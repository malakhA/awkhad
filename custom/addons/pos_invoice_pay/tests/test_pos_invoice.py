...
class TestUi(awkhad.tests.HttpCase):
    ...
    def test_pos_invoice_pay(self):
        ...
        self.phantom_js("/web",
                        "awkhad.__DEBUG__.services['web_tour.tour'].run('tour_pos_invoice_pay', 1000)",
                        "awkhad.__DEBUG__.services['web_tour.tour'].tours.tour_pos_invoice_pay.ready",
                        login="admin", timeout=200)


@awkhad.tests.common.at_install(True)
@awkhad.tests.common.post_install(True)
class TestModel(awkhad.tests.SingleTransactionCase):
