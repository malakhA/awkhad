...
class ProductProduct(models.Model):
    ...
    awkhad_module_version_id = fields.Many2one(
        'awkhad.module.version',
        'Awkhad Module Version',
    )
    app_license_id = fields.Many2one(
        comodel_name='awkhad.license',
        string='License',
        readonly=True,
        related="awkhad_module_version_id.license_id",
        store=True,
    )
    app_summary = fields.Char(
        'Summary (Manifest)',
        readonly=True,
        related="awkhad_module_version_id.summary",
        store=True,
    )
    app_website = fields.Char(
        'Website (Manifest)',
        readonly=True,
        related="awkhad_module_version_id.website",
        store=True,
    )
    ...
    app_version = fields.Char(
        'Version (Manifest)',
        readonly=True,
        related="awkhad_module_version_id.version",
        store=True,
    )
    app_author_ids = fields.Many2many(
        ...
        column2='author_id',
        multi='author',
        related="awkhad_module_version_id.author_ids",
        store=True,)
    app_github_url = fields.Char(
        'Github URL',
        readonly=True,
        related="awkhad_module_version_id.github_url",
        store=True,
    )
    app_development_status = fields.Selection(
        'Module maturity',
        readonly=True,
        related="awkhad_module_version_id.development_status",
        store=True,
    )

    @api.depends('awkhad_module_version_id',
                 'awkhad_module_version_id.description_rst_html')
    @api.multi
    def _compute_app_description_rst_html(self):
        for product in self:
            rst_desc = product.awkhad_module_version_id.description_rst_html
    ...
    def _manage_product_module(self, values):
        ...
        check if the product is used to save a module (by the awkhad_module_id
        field).
        Then check into given variant attributes if we find the variant used
        to save the module version (with XML ID:
        apps_product_creator.attribute_awkhad_version).
        ...
        if all([k in values_keys for k in check_keys]):
            ...
            module_version = self.env['awkhad.module.version']
            product_variant = self.env['product.product']
            product = product_obj.browse(values.get(product_tmpl_key, []))
            # If the product template is used to save an Awkhad module
            if self._check_related_to_module(product):
                ...
                versions = self._get_version_with_attribute(
                    product.awkhad_module_id.module_version_ids, attribute)
                for version in versions:
                    ...
                    for dep in dependency_modules:
                        product_variant_data = product_variant.search([
                            ('name', '=', dep.name),
                            ('awkhad_module_version_id.module_id', '=',
                             dep.module_id.id),
                    ...
                    values.update({
                        'awkhad_module_version_id': version.id,
                    })
        return True
    ...
    def _get_version_with_attribute(self, versions, attribute):
        """
        Get the version recordset (awkhad.module.version) into the given version
        (versions) based on the attribute name.
        :param versions: awkhad.module.version recordset
        :param attribute: product.attribute.value recordset
        :return: awkhad.module.version recordset
        """
    ...
    def _check_related_to_module(self, product):
        ...
        :return: bool
        """
        return bool(product.awkhad_module_id)
    ...
    def _get_version_attribute(self, values):
        ...
        attr_xml_id = "apps_product_creator.attribute_awkhad_version"
        attr_key = "attribute_value_ids"
    ...
    @api.depends('image_variant', 'product_tmpl_id.image', 'image_module',
                 'awkhad_module_version_id')
    def _compute_images(self):
        products = self.filtered(
            lambda p: p.awkhad_module_version_id and p.image_module)
