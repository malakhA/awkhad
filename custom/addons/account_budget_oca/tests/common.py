...
class TestAccountBudgetCommon(AccountingTestCase):
    ...
    def setUp(self):
        ...
        # In order to check account budget module in Awkhad I created a budget
        ...
        if not account_ids:
            account_ids = self.account_model.create({
                ...
                'tag_ids': [
                    (6, 0, [self.ref('account.account_tag_operating')])],
            }).ids
        self.account_budget_post_sales0 = self.env[
            'account.budget.post'].create({
                'name': 'Sales',
        ...
        self.account_budget_post_purchase0 = self.env[
            'account.budget.post'].create({
                'name': 'Purchases',
                'account_ids': [(6, None, account_ids)],
            })
