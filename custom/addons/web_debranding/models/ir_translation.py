...
def debrand_documentation_links(source, new_documentation_website):
    return re.sub(r'https://www.awkhad.com/documentation/',
                  new_documentation_website + 'documentation/',
                  source, flags=re.IGNORECASE)


def debrand_links(source, new_website):
    return re.sub(r'\bawkhad.com\b', new_website, source, flags=re.IGNORECASE)


def debrand(env, source, is_code=False):
    if not source or not re.search(r'\bawkhad\b', source, re.IGNORECASE):
        return source
    ...
    # Next regular expression exclude from substitution 'awkhad.SMTH', 'awkhad =', 'awkhad=', 'awkhadSMTH', 'awkhad['
    # Where SMTH is an any symbol or number or '_'. Option awkhad.com were excluded previously.
    # Examples:
    # awkhad.
    # xml file: https://github.com/awkhad/awkhad/blob/9.0/addons/im_livechat/views/im_livechat_channel_templates.xml#L148
    # awkhadSMTH
    # https://github.com/awkhad/awkhad/blob/11.0/addons/website_google_map/views/google_map_templates.xml#L14
    # awkhad =
    # https://github.com/awkhad/awkhad/blob/11.0/addons/web/views/webclient_templates.xml#L260
    # awkhad[
    # https://github.com/awkhad/awkhad/blob/11.0/addons/web_editor/views/iframe.xml#L43-L44
    # SMTH.awkhad
    # https://github.com/awkhad/awkhad/blob/11.0/addons/web_editor/views/iframe.xml#L43
    source = re.sub(r'\b(?<!\.)awkhad(?!\.\S|\s?=|\w|\[)\b', new_name, source, flags=re.IGNORECASE)
