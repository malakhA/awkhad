...
import copy
from awkhad import models, fields, _, api
from awkhad.tools import float_is_zero
...
class PosOrder(models.Model):
    ...
    def _process_order(self, order):
        ...
        if not float_is_zero(order['amount_paid'], account_precision):
            ...
            if postponed_payments:
                ...
                order['statement_ids'] = [p for p in payments if p not in postponed_payments]
                order['amount_paid'] = order['amount_paid'] - sum([p[2]['amount'] for p in postponed_payments] + [0])
                order['amount_return'] = max(order['amount_paid'] - order['amount_total'], 0)
        ...
        if postponed_payments:
            ...
            message = _("This Postponed invoice has been created from the point of sale session: <a href=# data-oe-model=pos.order data-oe-id=%d>%s</a>") % (
                res.id, res.name)
    ...
    def test_paid(self):
        ...
        for order in self:
            if (not order.lines) or (not order.postponed
                                     and (not order.statement_ids or (abs(order.amount_total - order.amount_paid) > 0.00001))):
                return False
        return True
