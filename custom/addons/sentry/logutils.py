...
class AwkhadSentryHandler(SentryHandler):
    '''
    Customized :class:`raven.handlers.logging.SentryHandler`.

    Allows to add additional Awkhad and HTTP request data to the event which is
    sent to Sentry.
    '''

    def __init__(self, include_extra_context, *args, **kwargs):
        super(AwkhadSentryHandler, self).__init__(*args, **kwargs)
    ...
    def emit(self, record):
        ...
        return super(AwkhadSentryHandler, self).emit(record)


class SanitizeAwkhadCookiesProcessor(SanitizePasswordsProcessor):
    '''
    Custom :class:`raven.processors.Processor`.

    Allows to sanitize sensitive Awkhad cookies, namely the "session_id" cookie.
    '''
