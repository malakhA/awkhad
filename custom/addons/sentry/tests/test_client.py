...
from ..logutils import AwkhadSentryHandler
...
class TestClientSetup(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # AwkhadSentryHandler handler so we can test with our in-memory client.
        remove_logging_handler('', AwkhadSentryHandler)
    ...
    def test_ignore_exceptions(self):
        config = {
            'sentry_enabled': True,
            'sentry_dsn': 'http://public:secret@example.com/1',
            'sentry_ignore_exceptions': 'awkhad.exceptions.UserError',
        }
        level, msg = logging.WARNING, 'Test UserError'
        client = initialize_raven(config, client_cls=InMemoryClient)

        handlers = list(
            log_handler_by_class(logging.getLogger(), AwkhadSentryHandler)
        )
