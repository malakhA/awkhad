...
from awkhad import api, exceptions, models, tools
from awkhad.modules.module import get_module_path
...
def ensure_module_state(env, modules, state):
    # read module states, bypassing any Awkhad cache
    if not modules:
        return
    env.cr.execute(
...
class Module(models.Model):
    ...
    def upgrade_changed_checksum(self, overwrite_existing_translations=False):
        ...
        then the normal Awkhad scheduled upgrade process
        is launched.
        ...
        nothing, otherwise the normal Awkhad upgrade process is launched.
