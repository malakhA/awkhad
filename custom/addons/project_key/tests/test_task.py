...
class TestTask(TestCommon):
    ...
    def test_01_key(self):
        self.assertEqual(self.task11.key, "ACA-1")
        self.assertEqual(self.task12.key, "ACA-2")
        self.assertEqual(self.task21.key, "AWKHAD-1")
    ...
    def test_05_switch_project(self):
        ...
        self.assertEqual(self.task11.key, 'AWKHAD-2')
        self.assertEqual(self.task12.key, 'AWKHAD-3')

    def test_06_name_search(self):
        oca_tasks = self.Task.name_search("ACA")
        self.assertEqual(len(oca_tasks), 2)

        non_oca_task_ids = [
            x[0] for x in self.Task.name_search("ACA", operator="not ilike")
        ]
