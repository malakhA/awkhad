...
class TestProject(TestCommon):
    ...
    def test_01_key(self):
        self.assertEqual(self.project_1.key, "ACA")
        self.assertEqual(self.project_2.key, "AWKHAD")
    ...
    def test_03_name_search(self):
        ...
        awkhad_projects = self.Project.browse(non_awkhad_projects).filtered(
            lambda x: x.id == self.project_2.id
        )

        self.assertEqual(len(awkhad_projects), 0)
