...
import re
import threading
from awkhad import models, api, tools
...
class IrMailServer(models.Model):
    ...
    def _tracking_headers_add(self, tracking_email_id, headers):
        ...
        headers = headers or {}
        headers['X-Awkhad-Database'] = getattr(
            threading.currentThread(), 'dbname', None),
        headers['X-Awkhad-Tracking-ID'] = tracking_email_id
        return headers
    ...
    def _tracking_email_id_body_get(self, body):
        ...
        match = re.search(
            r'<img[^>]*data-awkhad-tracking-email=["\']([0-9]*)["\']', body)
    ...
    def _tracking_email_get(self, message):
        ...
        if message.get('X-Awkhad-Tracking-ID', '').isdigit():
            tracking_email_id = int(message['X-Awkhad-Tracking-ID'])
