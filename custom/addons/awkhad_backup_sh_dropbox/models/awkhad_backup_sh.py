...
class BackupConfig(models.Model):
    ...
    def get_backup_list(self, cloud_params, service):
        ...
        try:
            ...
            return backup_list
        folder_path = self.env['ir.config_parameter'].get_param("awkhad_backup_sh_dropbox.dropbox_folder_path")
...
class BackupInfo(models.Model):
    ...
    def download_backup_action(self, backup=None):
        ...
        folder_path = self.env['ir.config_parameter'].get_param("awkhad_backup_sh_dropbox.dropbox_folder_path") or ""
        DropboxService = self.env['ir.config_parameter'].get_dropbox_service()
        return {
...
class BackupRemoteStorage(models.Model):
    ...
    def compute_dropbox_used_remote_storage(self):
        amount = sum(self.env['awkhad_backup_sh.backup_info'].search([('storage_service', '=', DROPBOX_STORAGE)]).mapped('backup_size'))
