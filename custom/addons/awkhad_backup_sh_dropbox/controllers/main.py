...
class NewBackupController(BackupController):
    ...
    def fetch_dashboard_data(self):
        ...
        dropbox_usage_values = {
            r.date: r.dropbox_used_remote_storage for r in values
        }
        for date in date_list:
            if date not in dropbox_usage_values:
                if date_list.index(date) == 0:
                    ...
                    dropbox_usage_values[date] = dropbox_usage_values.get(date_list[date_list.index(date) - 1], 0)
