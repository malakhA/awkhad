...
class ResConfigSettings(models.TransientModel):
    ...
    def get_values(self):
        ...
        config_parameters = self.env["ir.config_parameter"].sudo()
        sync_field_ids = config_parameters.sudo().get_param("pos_partner_sync.sync_field_ids", default=False)
        res.update(
            sync_field_ids=sync_field_ids and [int(x) for x in sync_field_ids.split(',')],
        )
        return res
