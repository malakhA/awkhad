...
from awkhad.addons.muk_utils.tests.common import multi_users
from awkhad.addons.muk_dms.tests.common import setup_data_function
from awkhad.addons.muk_dms.tests.test_storage import StorageTestCase
...
class StorageLObjectTestCase(StorageTestCase):
    ...
    def test_lobject_migrate(self):
        ...
        storage.write({'save_type': 'lobject'})
        file_02 = self.create_file(storage=storage)
        self.assertTrue(file_01.storage.id == storage.id)
        self.assertTrue(file_01.storage.save_type == 'lobject')
        self.assertTrue(file_01.save_type == 'database')
        self.assertTrue(file_02.storage.id == storage.id)
        self.assertTrue(file_02.storage.save_type == 'lobject')
        self.assertTrue(file_02.save_type == 'lobject')
        storage.action_storage_migrate()
        self.assertTrue(file_01.storage.id == storage.id)
        self.assertTrue(file_01.storage.save_type == 'lobject')
        self.assertTrue(file_01.save_type == 'lobject')
        self.assertTrue(file_02.storage.id == storage.id)
        self.assertTrue(file_02.storage.save_type == 'lobject')
        self.assertTrue(file_02.save_type == 'lobject')
