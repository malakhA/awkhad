...
class KnowledgeConfigSettings(models.TransientModel):
    ...
    module_cmis_read = fields.Boolean(
        'Attach files from an external DMS into Awkhad',
        help='Connect Awkhad with a CMIS compatible server to attach files\n'
             'to an Awkhad record.\n'
             '- This installs the module cmis_read.'
    )

    module_cmis_write = fields.Boolean(
        'Store attachments in an external DMS instead of the Awkhad Filestore',
        help='Connect Awkhad with a CMIS compatible server to store files.\n'
