...
import awkhad.tests.common as common
...
class TestSaleOrderLineDescriptionChange(common.TransactionCase):
    def setUp(self):
        ...
        self.group_only_sale_description = \
            self.env.ref('sale_order_line_description'
                         '.group_use_product_description_per_so_line')
        self.user_1 = self._create_user('TestUser1')
        self.user_2 = self._create_user('TestUser2',
                                        self.group_only_sale_description)
    ...
    def test_check_sale_order_line_description(self):
        ...
        sale_order_line = self.sale_order_line_model \
            .sudo(self.user_1).create(line_values.copy())
        self.assertEqual(
        ...
        sale_order_line = self.sale_order_line_model \
            .sudo(self.user_2).create(line_values.copy())
        self.assertEqual(
            sale_order_line.name,
