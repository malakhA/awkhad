...
class ExternalBinding(models.AbstractModel):
    ...
    An external binding is a binding between a backend and Awkhad.  For
    example, for a partner, it could be ``magento.res.partner`` or for a
    product, ``magento.product``.

    The final model, will be an ``_inherits`` of the Awkhad model and
    ...
    in ``magentoerpconnect`` you would not find that)::
        ...
        class MagentoResPartnerCategory(models.Model):
            ...
            _inherits = {'res.partner.category': 'awkhad_id'}

            awkhad_id = fields.Many2one(comodel_name='res.partner.category',
