...
from awkhad import fields, models, tools
...
class Binder(AbstractComponent):
    ...
    _awkhad_field = 'awkhad_id'  # override in sub-classes
    _sync_date_field = 'sync_date'  # override in sub-classes

    def to_internal(self, external_id, unwrap=False):
        """ Give the Awkhad recordset for an external ID
        ...
        if not bindings:
            if unwrap:
                return self.model.browse()[self._awkhad_field]
            return self.model.browse()
        bindings.ensure_one()
        if unwrap:
            bindings = bindings[self._awkhad_field]
        return bindings

    def to_external(self, binding, wrap=False):
        """ Give the external ID for an Awkhad binding ID

        :param binding: Awkhad binding for which we want the external id
    ...
    def bind(self, external_id, binding):
        """ Create the link between an external ID and an Awkhad ID

        :param external_id: external id to bind
        :param binding: Awkhad record to bind
        :type binding: int
        """
    ...
    def unwrap_binding(self, binding):
        ...
        return binding[self._awkhad_field]
    ...
    def unwrap_model(self):
        ...
        """
        try:
            column = self.model._fields[self._awkhad_field]
        except KeyError:
            raise ValueError(
                'Cannot unwrap model %s, because it has no %s fields'
                % (self.model._name, self._awkhad_field))
