...
external records into Awkhad records and conversely.
...
from awkhad.addons.component.exception import NoComponentError
...
def changed_by(*args):
    ...
    When fields are modified in Awkhad, we want to export only the
...
def external_to_m2o(field, binding=None):
    ...
    binding in Awkhad and returns it.
...
def follow_m2o_relations(field):
    ...
    :param field: field "path", using dots for relations as usual in Awkhad
    """
    def modifier(self, record, to_attr):
        attrs = field.split('.')
        value = record
...
class MapChild(AbstractComponent):
    ...
    * Format the output values to the format expected by Awkhad or the
    ...
    def format_items(self, items_values):
        ...
        It can be overridden for instance to add the Awkhad
...
class Mapper(AbstractComponent):
    """ A Mapper translates an external record to an Awkhad record and
    ...
    Submappings
        ...
        same call to :py:meth:`awkhad.models.BaseModel.create()`.
    ...
    def _build_mapper_component(cls):
        ...
        * "real" Python classes applied on top of existing components (here
        ...
        ::
            ...
            >>> cls.__bases__
            (<class 'awkhad.addons.connector.tests.test_mapper.ThirdMapper'>,
             <class 'awkhad.addons.component.core.second.mapper'>,
             <class 'awkhad.addons.component.core.base'>)
...
class ImportMapper(AbstractComponent):
    ...
    Transform a record from a backend to an Awkhad record
...
class ExportMapper(AbstractComponent):
    ...
    Transform a record from Awkhad to a backend record
