...
:py:class:`~connector.components.binder.Binder`
  The ``binders`` give the external ID or Awkhad ID from respectively an
  Awkhad ID or an external ID. A default implementation is available.
...
:py:class:`~connector.components.mapper.Mapper`
  The ``mappers`` transform a external record into an Awkhad record or
  conversely.
...
from awkhad.addons.queue_job.exception import RetryableJobError
...
class BaseConnectorComponent(AbstractComponent):
    ...
    def advisory_lock_or_retry(self, lock, retry_seconds=1):
        ...
        :exc:`awkhad.addons.queue_job.exception.RetryableJobError` so the job
        ...
        See :func:`awkhad.addons.connector.connector.pg_try_advisory_lock` for
        details.
