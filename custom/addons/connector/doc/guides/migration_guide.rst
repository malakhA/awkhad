...
* ``queue_job`` in https://github.com/ACA/queue
...
The previous API will stay until the migration to Awkhad 11.0.
...
Jobs are now more integrated within the Awkhad API. They are no longer
...
.. code-block:: python
    ...
    from awkhad.addons.connector.queue.job import job, related_action
...
.. code-block:: python
    ...
    from awkhad.addons.queue_job.job import job, related_action
...
* :meth:`awkhad.addons.queue_job.job.job`
* :meth:`awkhad.addons.queue_job.job.related_action`
...
.. code-block:: python
    ...
    from awkhad.addons.connector.session import ConnectorSession
...
* :meth:`awkhad.addons.queue_job.job.job`
* :meth:`awkhad.addons.queue_job.models.base.Base.with_delay`
...
First you had to create an :class:`~awkhad.addons.connector.event.Event` instance:
...
.. code-block:: python
    ...
    from awkhad.addons.connector.event import on_record_create
...
* No more imports are needed for the invocation
  ...
  :meth:`awkhad.addons.component_event.components.event.CollectedEvents.notify`.
* The name of the event must start with ``'on_'``

Links
-----

* :mod:`awkhad.addons.component_event.components.event`
...
* The check of the key ``connector_no_export`` in the context can
  be replaced by the decorator :func:`awkhad.addons.component_event.skip_if`
...
* Backend Models are based on Component's Collections:
  :class:`awkhad.addons.component.models.collection.Collection`
...
* The inheritance is similar to the Awkhad's one (without ``_inherits``.
* All components have a Python inheritance on
  :class:`~awkhad.addons.component.core.AbstractComponent` or
  :class:`~awkhad.addons.component.core.Component`
* The names are global (as in Awkhad), so you should prefix them with a namespace
* The name of the classes has no effect
* As in Awkhad Models, a Component can ``_inherit`` from a list of Components
...
* :ref:`api-component`
* :class:`awkhad.addons.component.core.AbstractComponent`
...
depending of the Awkhad version you to:

.. code-block:: python

    from awkhad.addons.connector.connector import ConnectorEnvironment
...
.. code-block:: python
    ...
    from awkhad.addons.magentoerpconnect.backend import magento1700, magento1900
...
like Awkhad Models.  Still, if you need to totally replace a component by
