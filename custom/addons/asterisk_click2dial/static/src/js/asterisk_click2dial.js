...
awkhad.define('asterisk_click2dial.systray.OpenCaller', function (require) {
"use strict";
...
var OpenCallerMenu = Widget.extend({
    ...
    on_open_caller: function (event) {
        ...
        if (r === false) {
             self.do_warn(
                _t('IPBX error'),
                _t('Calling party number not retreived from IPBX or IPBX unreachable by Awkhad'),
                false);
        }
            ...
            var action = {
                ...
                of Awkhad Enterprise edition, you have to change the line
                target: 'current',
                  to:
                target: 'new',
