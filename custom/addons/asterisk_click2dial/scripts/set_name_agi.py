...
"""
 Name lookup in Awkhad for incoming and outgoing calls with an
 Asterisk IPBX
 ...
 It's probably a good idea to create a user in Awkhad dedicated to this task.
 ...
 Note that this script can be used without Awkhad, with just the
 ...
 lookup in Awkhad's partners and other objects with phone numbers
 ...
 available from https://github.com/ACA/connector-telephony
 ...
 lookup in Awkhad the name corresponding to the phone number
 ...
 "awkhad-out-call" of the Outgoing Call of my Xivo server :
...
options = [
    {'names': ('-s', '--server'), 'dest': 'server', 'type': 'string',
        ...
        'help': 'DNS or IP address of the Awkhad server. Default = none '
        '(will not try to connect to Awkhad)'},
    {'names': ('-p', '--port'), 'dest': 'port', 'type': 'int',
        'action': 'store', 'default': False,
        'help': "Port of Awkhad's webservice interface. Default = 443 when SSL "
    ...
    {'names': ('-d', '--database'), 'dest': 'database', 'type': 'string',
        'action': 'store', 'default': 'awkhad',
        'help': "Awkhad database name. Default = 'awkhad'"},
    {'names': ('-u', '--user-id'), 'dest': 'userid', 'type': 'int',
        'action': 'store', 'default': 2,
        'help': "Awkhad user ID to use when connecting to Awkhad in "
        "XML-RPC. Default = 2"},
    {'names': ('-t', '--username'), 'dest': 'username', 'type': 'string',
        'action': 'store', 'default': 'demo',
        'help': "Awkhad username to use when connecting to Awkhad in "
        "JSON-RPC. Default = demo"},
    {'names': ('-w', '--password'), 'dest': 'password', 'type': 'string',
        'action': 'store', 'default': 'demo',
        'help': "Password of the Awkhad user. Default = 'demo'"},
    {'names': ('-a', '--ascii'), 'dest': 'ascii',
        'action': 'store_true', 'default': False,
        'help': "Convert name from UTF-8 to ASCII. Default = no, keep UTF-8"},
    {'names': ('-n', '--notify'), 'dest': 'notify',
        'action': 'store_true', 'default': False,
        'help': "Notify Awkhad users via a pop-up (requires the Awkhad "
        "module 'base_phone_popup'). If you use this option, you must pass "
        "the logins of the Awkhad users to notify as argument to the "
        "script. Default = no"},
    {'names': ('-g', '--geoloc'), 'dest': 'geoloc',
        'action': 'store_true', 'default': False,
        'help': "Try to geolocate phone numbers unknown to Awkhad. This "
...
def main(options, arguments):
    ...
    if options.outgoing:
        ...
        # connect to Awkhad or geoloc, we just keep it
    ...
    res = False
    # Yes, this script can be used without "-s awkhad_server" !
    if options.server and options.jsonrpc:
        import awkhadrpc
        proto = options.ssl and 'jsonrpc+ssl' or 'jsonrpc'
        agi.verbose(
            "Starting %s request on Awkhad %s:%d database %s username %s" % (
                proto.upper(), options.server, port, options.database,
                options.username))
        try:
            awkhad = awkhadrpc.AWKHAD(options.server, proto, port)
            awkhad.login(options.database, options.username, options.password)
            if options.notify:
                res = awkhad.execute(
                    'phone.common', method, phone_number, arguments)
            else:
                res = awkhad.execute('phone.common', method, phone_number)
            agi.verbose("Called method %s" % method)
        except:
            agi.verbose("Could not connect to Awkhad in JSON-RPC")
    elif options.server:
        proto = options.ssl and 'https' or 'http'
        agi.verbose(
            "Starting %s XML-RPC request on Awkhad %s:%d "
        ...
        try:
            ...
            agi.verbose("Could not connect to Awkhad in XML-RPC")
    ...
    if res:
        ...
        # if the number is not found in Awkhad, we try to geolocate
        agi.verbose(
        ...
        # if the number is not found in Awkhad and geoloc is off,
        # we put 'not_found_name' as Name
        agi.verbose("Phone number not found in Awkhad")
        res = not_found_name
...
if __name__ == '__main__':
    ...
    description = "This is an AGI script that sends a query to Awkhad. "
    "It can also be used without Awkhad to geolocate phone numbers "
    "of incoming calls."
