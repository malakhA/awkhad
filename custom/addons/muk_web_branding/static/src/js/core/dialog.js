...
awkhad.define('muk_web_branding.dialog', function (require) {
	"use strict";
	...
	Dialog.include({
		...
		_debrandOptions: function (options) {
			if (options.title && options.title.replace) {
				options.title = options.title.replace(/Awkhad/ig,
			...
			if (options.$content) {
				...
				options.$content.each(function () {
					...
					if (content) {
						content.replace(/Awkhad.com/ig,
							session.muk_branding_website);
						content.replace(/Awkhad/ig,
							session.muk_branding_system_name);
						$(this).html(content);
					}
				});
