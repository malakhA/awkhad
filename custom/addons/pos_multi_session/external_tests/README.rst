...
If **"No module named unittest2" Error** is given even if unittest2 is already installed, inside the awkhad docker open python in terminal and execute next::
...
* use *-dev* images of `install-awkhad <https://github.com/it-projects-llc/install-awkhad>`__, e.g. ``itprojectsllc/install-awkhad:10.0-dev``

::

    docker network create multi_session_test_network

    docker run --network=multi_session_test_network -d -e POSTGRES_USER=awkhad -e POSTGRES_PASSWORD=awkhad --name db_ms_test postgres:9.5


    docker run \
    -e DB_PORT_5432_TCP_ADDR=db_ms_test \
    --network=multi_session_test_network \
    -e AWKHAD_MASTER_PASS=admin \
    -e DATABASE=test_database \
    -e AWKHAD_DOMAIN=awkhad-nginx \
    -v /PATH/TO/pos-addons/:/mnt/addons/it-projects-llc/pos-addons/ \
    -v /PATH/TO/awkhad/:/mnt/awkhad-source/ \
    --name awkhad \
    -t itprojectsllc/install-awkhad:$AWKHAD_BRANCH-dev -- --workers=1 -d test_database --db-filter test_database
    ...
    --name awkhad-nginx \
    --network=multi_session_test_network \
    -t itprojectsllc/docker-awkhad-nginx

* Note. Name ``awkhad`` for docker container is mandatory for nginx container and cannot be changed
...
To run tests, you need to run awkhad server and then execute::
...
::
    ...
    docker exec -u awkhad -i -t awkhad /bin/bash -c "\
...
* open awkhad with localhost:8069
* open POS interface
...
To run tests on separated servers do what is written in previous paragraphs and create two dockers more. First docker below will process functionality of main server while 'awkhad' container will provide synchronization. The second docker provides longpolling support for the first one.
::
    ...
    -p 8072:8072 \
    -e AWKHAD_MASTER_PASS=admin \
    -e DATABASE=db_awkhad_main \
    -e AWKHAD_DOMAIN=awkhad-main-nginx \
    -e AWKHAD_PORT=8069 \
    -v /PATH/TO/pos-addons/:/mnt/addons/it-projects-llc/pos-addons/ \
    -v /PATH/TO/awkhad/:/mnt/awkhad-source/ \
    --name awkhad-main \
    -t itprojectsllc/install-awkhad:10.0-dev -- --workers=1 -d db_awkhad_main --db-filter db_awkhad_main


    docker run \
    -p 8888:80 \
    --name awkhad-main-nginx \
    --network=multi_session_test_network \
    -t itprojectsllc/docker-awkhad-nginx
...
* Configure main server according to instructions provided in module ``pos_multi_session_sync`` ``/doc/index.rst`` ``Configuration/Main Server``. Use ``awkhad-nginx`` like ``external server url``
...
awkhad-main-nginx Container Configuration
---------------------------------------
* Open ``awkhad-main-nginx`` container via::

    docker exec -i -u root -t awkhad-main-nginx /bin/bash
    ...
    http {
      ...
      server {
        ...
        add_header 'Access-Control-Allow-Origin' * always;
         ...
         location / {
            proxy_pass http://awkhad-main:8069;
          ...
          proxy_pass http://awkhad-main:8072;
          }
      }
    }

* Do not forget to restart your 'awkhad-main-nginx' container after all steps::

    docker restart awkhad-main-nginx

awkhad-nginx Container Configuration
----------------------------------
* Open ``awkhad-nginx`` container via::

    docker exec -i -u root -t awkhad-nginx /bin/bash
...
* Do not forget to restart your 'awkhad-main-nginx' container after all steps::
    ...
    docker restart awkhad-nginx

Run tests in Docker for separated servers
-----------------------------------------
::

    docker exec -u awkhad -i -t awkhad-main /bin/bash -c "\
    cd /mnt/addons/it-projects-llc/pos-addons/pos_multi_session; \
    AWKHAD_PORT=80 python -m unittest discover -t . -s external_tests"
