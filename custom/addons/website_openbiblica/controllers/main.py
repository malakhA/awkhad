...
from awkhad import http, modules, SUPERUSER_ID, _
...
class WebsiteBiblica(http.Controller):
    ...
    @http.route(['/biblica/<model("open.biblica"):biblica_id>',
                ...
                type='http', auth="public", website=True, csrf=False)
    def view_biblica(self, biblica_id=0, page=1, **kwargs):
        biblicas = request.env['open.biblica'].sudo().search([])
    ...
    @http.route(['/content/<model("open.content"):content_id>',
                ...
                type='http', auth="public", website=True, csrf=False)
    def view_content(self, content_id=0, page=1, **kwargs):
    ...
    @http.route(['/part/<model("open.part"):part_id>',
                 '/part/<model("open.part"):part_id>/page/<int:page>'],
                type='http', auth="public", website=True, csrf=False)
    def view_part(self, part_id=0, page=1, **kwargs):
        contents = request.env['open.content'].sudo().search([])
        biblicas = request.env['open.biblica'].sudo().search([('id', 'in', contents.mapped('biblica_id.id'))])
        languages = request.env['res.lang'].sudo().search([('id', 'in', biblicas.mapped('lang_id.id'))])
        if kwargs.get('src_id'):
    ...
    @http.route(['/line/<model("open.line"):line_id>',
                 '/line/<model("open.line"):line_id>/page/<int:page>'],
                type='http', auth="public", website=True, csrf=False)
    def view_line(self, line_id=0, page=1, **kwargs):
        contents = request.env['open.content'].sudo().search([])
        biblicas = request.env['open.biblica'].sudo().search([('id', 'in', contents.mapped('biblica_id.id'))])
        languages = request.env['res.lang'].sudo().search([('id', 'in', biblicas.mapped('lang_id.id'))])
        if kwargs.get('src_id'):
    ...
    @http.route(['/allsources/line/<model("open.line"):line_id>',
                 '/allsources/line/<model("open.line"):line_id>/page/<int:page>'],
                type='http', auth="public", website=True, csrf=False)
    def view_line_sources(self, line_id=0, page=1, **kwargs):
        contents = request.env['open.content'].sudo().search([])
        biblicas = request.env['open.biblica'].sudo().search([('id', 'in', contents.mapped('biblica_id.id'))])
        languages = request.env['res.lang'].sudo().search([('id', 'in', biblicas.mapped('lang_id.id'))])
    ...
    def post_part_comment(self, part_id=0, **kwargs):
        ...
        if not parent_id:
            if part_id.parent_id:
                name = part_id.parent_id.content_id.biblica_id.name + ' ' + part_id.parent_id.content_id.name + part_id.parent_id.name + ' ' + 'chapter' + ' ' + part_id.name
            else:
    ...
    @http.route(['/comment/p/<model("open.line"):line_id>'], type='http', auth="user", methods=['POST'], website=True)
    def post_line_comment(self, line_id=0, **kwargs):
        ...
        if not parent_id:
            if line_id.part_id.parent_id:
                name = line_id.part_id.parent_id.content_id.biblica_id.name + ' ' + line_id.part_id.parent_id.content_id.name + line_id.part_id.parent_id.name + ' ' + line_id.chapter + ' : ' + line_id.verse
            else:
