...
class TestStockPikingReturnRefundOption(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.partner = cls.env['res.partner'].create({
            ...
            'delivery_schedule_ids': [
                (6, 0, cls.schedule.ids + cls.schedule2.ids)],
        })

    def test_partner_schedule_name(self):
        self.assertEqual(
            self.schedule.name_get()[0][1], '08:00-10:00 (Mo, Tu)')
        ...
        self.assertEqual(
            self.schedule.name_get()[0][1], '08:00-10:00 (All days)')
    ...
    def test_partner_allow_delivery(self):
        self.assertTrue(
            self.partner.allow_delivery_date('2018-09-03 09:00:00')
        )
        self.assertTrue(
            self.partner.allow_delivery_date('2018-09-04 09:00:00')
        )
        self.assertFalse(
            self.partner.allow_delivery_date('2018-09-04 12:01:00')
        )
        self.assertFalse(
            self.partner.allow_delivery_date('2018-09-05 10:01:00')
        )
        ...
        self.schedule.update(day_update)
        self.assertTrue(
            self.partner.allow_delivery_date('2018-09-09 09:00:00')
        )
