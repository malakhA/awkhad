...
from awkhad import _, api, exceptions, fields, models, tools
...
class AbstractGithubModel(models.AbstractModel):
    ...
    def get_conversion_dict(self):
        """
        Prepare function that map Github fields to Awkhad fields
        :return: Dictionary {awkhad_field: github_field}
        """
        return {
    ...
    @api.model
    def get_awkhad_data_from_github(self, data):
        """Prepare function that map Github data to create in Awkhad"""
    ...
    def get_github_data_from_awkhad(self):
        """Prepare function that map Awkhad data to create in Github.
        Usefull only if your model implement creation in github"""
        self.ensure_one()
        raise exceptions.Warning(_(
            "Feature not Implemented : Please define"
            " 'get_github_data_from_awkhad' function in child model."))
    ...
    def get_from_id_or_create(self, data, extra_data=None):
        """Search if the awkhad object exists in database. If yes, returns the
            ...
            >>> self.env['github_organization'].get_from_id_or_create(
                {'id': 7600578, 'url': 'https://api.github.com/orgs/ACA'})
        """
    ...
    def create_from_name(self, name):
        """Call Github API, using a URL using github name. Load data and
            Create Awkhad object accordingly, if the awkhad object doesn't exist.
            ...
            >>> self.env['github_organization'].create_from_name('ACA')
            >>> self.env['github_repository'].create_from_name('ACA/web')
        """
        github_connector = self.get_github_connector(self.github_type())
    ...
    def update_from_github(self, child_update):
        """Call Github API, using a URL using github id. Load data and
            update Awkhad object accordingly, if the awkhad object is obsolete.
    ...
    def _create_from_github_data(self, data, extra_data=None):
        extra_data = extra_data and extra_data or {}
        vals = self.get_awkhad_data_from_github(data)
    ...
    def create_in_github(self, model_obj):
        ...
        response = github_connector.create(
            ...
            self.get_github_data_from_awkhad())
        # Create in Awkhad with the returned data and update object
