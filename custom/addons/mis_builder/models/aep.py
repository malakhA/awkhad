...
from awkhad import _, fields
from awkhad.exceptions import UserError
from awkhad.models import expression
...
def _is_domain(s):
    """ Test if a string looks like an Awkhad domain """
...
class AccountingExpressionProcessor(object):
    ...
    def get_aml_domain_for_dates(self, date_from, date_to, mode, target_move):
        ...
        elif mode == self.MODE_UNALLACATED:
            date_from_date = fields.Date.from_string(date_from)
            # TODO this takes the fy from the first company
            # make that user controllable (nice to have)?
            fy_date_from = self.companies[0].compute_fiscalyear_dates(date_from_date)[
    ...
    def do_queries(
        ...
        for key in self._map_account_ids:
            ...
            for acc in accs:
                ...
                if mode in (self.MODE_INITIAL, self.MODE_UNALLACATED) and float_is_zero(
    ...
    def replace_expr(self, expr):
        ...
        def f(mo):
            ...
            if (
                ...
                and mode in (self.MODE_INITIAL, self.MODE_UNALLACATED)
    ...
    def get_unallocated_pl(cls, companies, date, target_move="posted"):
        ...
        bals = cls._get_balances(
            cls.MODE_UNALLACATED, companies, date, date, target_move
        )
