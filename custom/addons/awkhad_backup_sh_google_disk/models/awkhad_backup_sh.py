...
from awkhad.addons.awkhad_backup_sh.models.awkhad_backup_sh import compute_backup_filename, compute_backup_info_filename, get_backup_by_id, ModuleNotConfigured
from awkhad.tools import DEFAULT_SERVER_DATE_FORMAT
...
class BackupConfig(models.Model):
    _inherit = "awkhad_backup_sh.config"
    ...
    def get_backup_list(self, cloud_params, service):
        ...
        try:
            ...
            return backup_list
        folder_id = self.env['ir.config_parameter'].get_param("awkhad_backup_sh_google_disk.google_disk_folder_id")
...
class BackupInfo(models.Model):
    _inherit = 'awkhad_backup_sh.backup_info'
    ...
    def download_backup_action(self, backup=None):
        ...
        file_id = self.env["awkhad_backup_sh.config"].get_google_drive_file_id(backup.backup_filename)
        return {
...
class BackupRemoteStorage(models.Model):
    _inherit = 'awkhad_backup_sh.remote_storage'
    ...
    def compute_google_drive_used_remote_storage(self):
        amount = sum(self.env['awkhad_backup_sh.backup_info'].search([('storage_service', '=', GOOGLE_DRIVE_STORAGE)]).mapped('backup_size'))
...
class DeleteRemoteBackupWizard(models.TransientModel):
    _inherit = "awkhad_backup_sh.delete_remote_backup_wizard"
    ...
    def delete_remove_backup_button(self):
        ...
        for record in backup_google_drive_info_records:
            for obj_name in [record.backup_filename, record.backup_info_filename]:
                file_id = self.env["awkhad_backup_sh.config"].get_google_drive_file_id(obj_name)
                try:
                    GoogleDriveService.files().delete(fileId=file_id).execute()
                except errors.HttpError as e:
                    _logger.exception(e)
