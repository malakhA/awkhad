...
from awkhad import fields, http
from awkhad.http import request
from awkhad.tools import DEFAULT_SERVER_DATE_FORMAT
from awkhad.addons.awkhad_backup_sh.controllers.main import BackupController
        ...
        for date in date_list:
            if date not in google_drive_usage_values:
                if date_list.index(date) == 0:
                    google_drive_usage_values[date] = 0
                else:
                    google_drive_usage_values[date] = google_drive_usage_values.get(date_list[date_list.index(date) - 1], 0)

        dashboard_data['services_storage_usage_graph_values'].update({
            'google_drive': [{
                'key': 'Remote Google Drive Storage Usage',
                'values': [{
                    0: fields.Date.to_string(day),
                    1: google_drive_usage_values[day]
