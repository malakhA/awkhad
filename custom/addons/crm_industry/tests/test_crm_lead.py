...
class TestCrmLead(TransactionCase):
    ...
    def test_check_industries(self):
        ...
        with self.assertRaises(ValidationError):
            self.env['crm.lead'].create(
                {'name': 'Test', 'industry_id': industry.id,
    ...
    def test_lead_create_contact(self):
        ...
        industry_2 = industry_pool.create({'name': 'Test 02',
                                           'parent_id': industry_1.id})
        industry_3 = industry_pool.create({'name': 'Test 03',
                                           'parent_id': industry_1.id})
        lead_vals = {
            'name': 'test',
            'partner_name': 'test',
            'industry_id': industry_1.id,
            'secondary_industry_ids': [(4, industry_2.id, 0),
                                       (4, industry_3.id, 0)],
        }
        ...
        self.assertEqual(
            partner.secondary_industry_ids, lead.secondary_industry_ids)
