...
awkhad.define('muk_dms_view.DocumentsMainView', function(require) {
"use strict";
...
var DocumentTreeMainView = ActionDocumentTreeView.extend({
    ...
    _preview_node: function(node) {
    	var self = this;
    	if(node.data && node.data.awkhad_model === "muk_dms.file") {
    		if (this.manager) {
    		...
    		var binary_url = session.url('/web/content', {
                model: "muk_dms.file",
                id: JSON.stringify(node.data.awkhad_id),
                filename: node.text,
                field: "content",
                download: true,
            });
    ...
    _create_selected_node: function() {
    	var node = this.controller.getSelectedItem();
    	if(node) {
    		var parent = node.data.awkhad_model === "muk_dms.file" ? this.controller.getParentNode(node) : node;
    		this.controller._createNode(parent, node.data.awkhad_model);
    	}
    },
