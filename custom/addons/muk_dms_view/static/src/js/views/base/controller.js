...
awkhad.define('muk_dms_view.DocumentsController', function(require) {
"use strict";
...
var DocumentsController = Widget.extend(FileUpload, {
    ...
    getSelectedDirectory: function() {
    	...
    	var directories = _.filter(selected, function(node) {
    		return node.data.awkhad_model === "muk_dms.directory";
    	});
    ...
    _checkCallback: function (operation, node, parent, position, more) {
    	if(operation === "copy_node" || operation === "move_node") {
    		...
    		if(parent.data && parent.data.awkhad_model === "muk_dms.settings") {
    			return false;
            }
    		// prevent moving a child to a file
    		if(parent.data && parent.data.awkhad_model === "muk_dms.file") {
    			return false;
            }
    	}
    	if(operation === "move_node") {
    		...
    		if(node.data && parent.data) {
    			...
    			_.each(parent.children, function(child, index, children) {
    				...
    				if(child_node.data && child_node.data.awkhad_model === parent.data.awkhad_model) {
    ...
    _checkSelect: function(node, event) {
    	if(this.params.filesOnly && node.data.awkhad_model !== "muk_dms.file") {
    		return false;
    	}
    	return !(node.parent === '#' && node.data.awkhad_model === "muk_dms.settings");
    },
...
//    	$("#menuContinenti").prop('disabled', function (_, val) { return ! val; });
    ...
    _dropItems: function(ev) {
		...
		if (node) {
			files.getFileTree(ev.data.items, true).then(
				this._uploadFiles.bind(this, node.data.awkhad_id) 
			);
		} else {
    ...
    _openInfo: function(node) {
		if(node.data.awkhad_model == "muk_dms.file") {
			new DocumentFileInfoDialog(this, {
				id: node.data.awkhad_id,
		    }).open();
    	} else {
	...
	_openNode: function(node) {
		...
		this.do_action({
            ...
            res_model: node.data.awkhad_model,
            res_id: node.data.awkhad_id,
            views: [[false, 'form']],
    ...
    _createNode: function(node, type) {
    	...
    	if(type == "muk_dms.file") {
    		context = $.extend(session.user_context, {
    			default_directory: node.data.awkhad_id
            });
    	} else if(type == "muk_dms.directory") {
    		context = $.extend(session.user_context, {
    			default_parent_directory: node.data.awkhad_id
            });
    	}
    	this.do_action({
    		type: 'ir.actions.act_window',
    ...
    _moveNode: function(ev) {
		...
		if(node.data.awkhad_model === "muk_dms.file") {
			vals.directory = parent.data.awkhad_id;
		} else if(node.data.awkhad_model === "muk_dms.directory") {
			vals.parent_directory = parent.data.awkhad_id;
		}
		this._rpc({
            model: node.data.awkhad_model,
            method: 'write',
            args: [node.data.awkhad_id, vals],
            context: session.user_context,
		}).done(function() {
    ...
    _deleteNodes: function(nodes) {
    	...
    	var data = _.chain(nodes).map(function(node) {
    		return {model: node.data.awkhad_model, id: node.data.awkhad_id};
    ...
    _copyNode: function(ev) {
		...
		if(original.data.awkhad_model === "muk_dms.file") {
			vals.directory = parent.data.awkhad_id;
		} else if(original.data.awkhad_model === "muk_dms.directory") {
			vals.parent_directory = parent.data.awkhad_id;
		}
		this._rpc({
            model: original.data.awkhad_model,
            method: 'copy',
            args: [original.data.awkhad_id, vals],
            context: session.user_context,
		}).done(function(copy_id) {
			node.data = original.data;
			node.id = original.data.awkhad_model.split(".") + "_" + copy_id;
    ...
    _renameNode: function(ev) {
		...
		this._rpc({
            model: node.data.awkhad_model,
            method: 'write',
            args: [node.data.awkhad_id, {'name': ev.data.text}],
            context: session.user_context,
		}).done(function() {
    ...
    _createDirecotry: function(node, name) {
		return this._rpc({
    		...
    		params: {
    			name: name,
            	parent_directory: node.data.awkhad_id,
            	context: _.extend({}, {
    ...
    _executeOperation: function(node, action_id) {
    	...
    	this._rpc({
            ...
            args: [[action_id], node.data.awkhad_id],
