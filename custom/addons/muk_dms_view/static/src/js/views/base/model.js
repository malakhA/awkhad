...
awkhad.define('muk_dms_view.DocumentsModel', function(require) {
"use strict";
...
var DocumentsModel = Class.extend(EventDispatcherMixin, ServicesMixin, {
    ...
    _loadNode: function(node, args) {
    	var result = $.Deferred();
    	if(node.data && node.data.awkhad_model === "muk_dms.storage") {
    		this._loadDirectoriesSingle(node.data.awkhad_id, args).then(function(directories) {
    			var directory_nodes = _.map(directories, function (directory) {
					return this._makeNodeDirectory(directory, args.file.show);
				}.bind(this));		
    			result.resolve(directory_nodes);
            }.bind(this));
    	} else if(node.data && node.data.awkhad_model === "muk_dms.directory") {
    		var files_loaded = $.Deferred();
    		var directories_loaded = $.Deferred();
    		this._loadSubdirectoriesSingle(node.data.awkhad_id, args).then(function(directories) {
    			var directory_nodes = _.map(directories, function (directory) {
					return this._makeNodeDirectory(directory, args.file.show);
				}.bind(this));		
    		...
    		if(args.file.show) {
    			this._loadFilesSingle(node.data.awkhad_id, args).then(function(files) {
    ...
    _searchNodes: function(val, node, args) {
		...
		var files_loaded = $.Deferred();
    	var directories_loaded = $.Deferred();
    	this._searchDirectories(node.data.awkhad_id, val, args).then(function(directories) {
		...
		if(args.directoriesOnly) {
			...
			this._searchFiles(node.data.awkhad_id, val, args).then(function(files) {
    ...
    _makeNodeStorage: function(storage, children) {
		return {
			...
			type: "storage",
			data: {
				awkhad_id: storage.id,
				awkhad_model: "muk_dms.storage",
				awkhad_record: storage,
			},
			children: children,
		};
    },
    _makeNodeDirectory: function(directory, showFiles) {
    	var directoryNode = {
			...
			type: "directory",
			data: {
				awkhad_id: directory.id,
				awkhad_model: "muk_dms.directory",
				awkhad_record: directory,
				name: directory.name,
