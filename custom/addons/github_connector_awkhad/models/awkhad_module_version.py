...
from awkhad import _, api, fields, models, tools
from awkhad.tools import html_sanitize
from awkhad.addons.base.models.ir_module import MyWriter
...
class AwkhadModuleVersion(models.Model):
    _name = 'awkhad.module.version'
    _description = 'Awkhad Module Version'
    ...
    module_id = fields.Many2one(
        comodel_name='awkhad.module', string='Module', required=True,
    ...
    license_id = fields.Many2one(
        comodel_name='awkhad.license', string='License', readonly=True,
    ...
    dependency_module_ids = fields.Many2many(
        comodel_name='awkhad.module', string='Dependencies',
    ...
    author_ids = fields.Many2many(
        string='Authors', comodel_name='awkhad.author',
    ...
    lib_python_ids = fields.Many2many(
        comodel_name='awkhad.lib.python', string='Python Lib Dependencies',
    ...
    lib_bin_ids = fields.Many2many(
        comodel_name='awkhad.lib.bin', string='Bin Lib Dependencies',
    ...
    awkhad_type = fields.Selection(
        string='Awkhad Type', selection=_AWKHAD_TYPE_SELECTION, store=True,
        compute='_compute_awkhad_type')
    ...
    category_id = fields.Many2one(
        comodel_name='awkhad.category', string='Category', readonly=True)
    ...
    def unlink(self):
        ...
        return super(AwkhadModuleVersion, self).unlink()
    ...
    def _compute_dependency_module_ids(self):
        module_obj = self.env['awkhad.module']
        for version in self:
            modules = []
    ...
    def _compute_lib(self):
        lib_python_obj = self.env['awkhad.lib.python']
        lib_bin_obj = self.env['awkhad.lib.bin']
        for version in self:
            python_libs = []
            bin_libs = []
    ...
    def _compute_license_id(self):
        license_obj = self.env['awkhad.license']
        for version in self:
            if version.license:
                version.license_id =\
    ...
    def _compute_author(self):
        awkhad_author_obj = self.env['awkhad.author']
        for version in self:
            authors = []
            for item in [x.strip() for x in version.author.split(',')]:
                if item and item != version.repository_id.organization_id.\
                        default_author_text:
                    authors.append(
                        awkhad_author_obj.create_if_not_exist(item))
            authors = set(authors)
    ...
    def get_module_category(self, info):
        ...
        """
        category_obj = self.env['awkhad.category']
    ...
    def create_or_update_from_manifest(
        ...
        if not module_version:
            ...
            vals = self.manifest_2_awkhad(info, repository_branch, module)
            vals['full_module_path'] = full_module_path
            module_version = self.create(vals)

        else:
            # Update module Version
            vals = self.manifest_2_awkhad(
        ...
        if icon_path:
            ...
            try:
                ...
                _logger.error(
                    'Unable to read the ACA icon image, error is %s', e)

    @api.model
    def cron_clean_awkhad_module_version(self):
        module_versions = self.search([])
        module_versions.clean_awkhad_module_version()

    @api.multi
    def clean_awkhad_module_version(self):
        for module_version in self:
