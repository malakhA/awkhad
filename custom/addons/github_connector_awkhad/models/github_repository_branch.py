...
from awkhad.modules import load_information_from_description_file
...
class GithubRepositoryBranch(models.Model):
    ...
    module_version_ids = fields.One2many(
        comodel_name='awkhad.module.version',
    ...
    def analyze_code_one(self):
        # Change log level to avoid warning, when parsing awkhad manifests
        logger1 = logging.getLogger('zgui.modules.module')
        logger2 = logging.getLogger('zgui.addons.base.module.module')
    ...
    def _analyze_module_name(self, path, module_name):
        self.ensure_one()
        module_version_obj = self.env['awkhad.module.version']
        try:
