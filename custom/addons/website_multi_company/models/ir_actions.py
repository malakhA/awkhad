...
class IrActionsActUrl(models.Model):
    ...
    def _action_website_url(self, data):
        ...
        if website.domain in ['localhost', '0.0.0.0'] \
           or website.domain.endswith('.example'):
            return False

        if self.env['ir.config_parameter'].sudo().get_param('web.base.url', '').startswith('https://'):
            scheme = 'https'
        else:
            scheme = 'http'
        url = '%s://%s/' % (scheme, website.domain)
        return url
