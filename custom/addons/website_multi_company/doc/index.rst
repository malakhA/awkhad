...
Awkhad version
------------
Please use up-to-date version of awkhad or at least be sure, that your awkhad has following updates:

* 26 Jun 2018: https://github.com/awkhad/awkhad/commit/5cecd0a197eba847e4a71bea3a31584d2b88ea6b
...
* if you use ``website_sale`` (eCommerce) module, then install `Real Multi Website (eCommerce extension) <https://www.awkhad.com/apps/modules/12.0/website_multi_company_sale/>`__ too
* if you use ``website_portal`` (Portal) module, then install `Real Multi Website (portal extension) <https://www.awkhad.com/apps/modules/12.0/website_multi_company_portal/>`__ too
...
Web Server
----------
Your webserver (e.g. Apache or Nginx) must pass header ``Host`` to awkhad, otherwise there is no way to define which website is used. Required configuration for Nginx and Apache looks as following:
...
No updates in awkhad config is required if you use only one database.

Multi database deployment 
-------------------------
For multi-database instance awkhad has to know which database to use when handle new request without
...
For PRODUCTION deployment with websites on subdomains you can use following patch. You need to update awkhad/http.py file as following::
    ...
    def db_filter(dbs, httprequest=None):
        ...
        if d == "www" and r:
            d, _, t = r.partition('.')
        r = awkhad.tools.config['dbfilter'].replace('%h', h).replace('%d', d).replace('%t', t)
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Most flexible way to deploy multi-database system is using `dbfilter_from_header <https://www.awkhad.com/apps/modules/10.0/dbfilter_from_header/>`__ (check module description for installation instruction).
...
Apache::
    ...
    Header add X-AWKHAD_DBFILTER [your filter regex]
    RequestHeader add X-AWKHAD_DBFILTER [your filter regex]
...
* Nginx::
      ...
      server {
        ...
        proxy_set_header Host $host;
        proxy_set_header X-Awkhad-dbfilter ^software_business\Z;
      ...
      server {
        ...
        proxy_set_header Host $host;
        proxy_set_header X-Awkhad-dbfilter ^delivery_business\Z;
...
Apache::
       ...
       <VirtualHost *:80>
		   ...
		   ProxyPreserveHost On
		   Header add X-AWKHAD_DBFILTER "software_business"
           RequestHeader add X-AWKHAD_DBFILTER "software_business"
       ...
       <VirtualHost *:80>
		   ...
		   ProxyPreserveHost On
		   Header add X-AWKHAD_DBFILTER "delivery_business"
           RequestHeader add X-AWKHAD_DBFILTER "delivery_business"
...
Awkhad.sh deployment
------------------

In the manager of your domain name registrar you need to add CNAME records for your domains (subdomains), for example:

* Create a CNAME record ``shop1.example.org`` pointing to <yourdatabase>.awkhad.com
* Create a CNAME record ``shop2.example.org`` pointing to <yourdatabase>.awkhad.com
* Create a CNAME record ``example.com`` pointing to <yourdatabase>.awkhad.com

Similar for dev and staging database, but use corresponding domain in awkhad.com, e.g. ``mywebsite-master-staging-12345689.dev.awkhad.com``

Translation issue
-----------------

Check this section if you use translations in awkhad.

Awkhad `may have wrong translations <https://github.com/awkhad/awkhad/issues/25550#issuecomment-401897456>`__, which leads to the error ``Translation is not valid``. You need either fix the translations or apply following workaround:

* open file ``awkhad/addons/base/ir/ir_translation.py``
...
Note. Awkhad doesn't share Website Menus (E.g. Homepage, Shop, Contact us, etc.) between websites. So, you need to have copies of them.
...
Follow the `instruction <https://www.awkhad.com/documentation/user/12.0/ecommerce/shopper_experience/paypal.html>`__ to know how to configure Paypal account and get Paypal Identity Token
