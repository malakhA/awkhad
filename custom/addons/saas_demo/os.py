...
# Some code is based on https://github.com/awkhad/awkhad-extra/blob/master/runbot/runbot.py
import subprocess
import os
import os.path
import logging
...
from awkhad.modules.module import module_manifest, README, MANIFEST_NAMES, adapt_version
...
def root_awkhad_path():
    path = os.path.abspath(os.path.expanduser(
        os.path.expandvars(os.path.dirname(awkhad.__file__))))
...
def update_addons_path(folder_of_folders, force=True):
    ...
    """
    if force:
        base_addons = os.path.join(root_awkhad_path(), 'awkhad/addons')
        main_addons = os.path.abspath(os.path.join(root_awkhad_path(), 'addons'))
        addons_path = [base_addons, main_addons]
    else:
        addons_path = awkhad.conf.addons_paths
