...
one Awkhad addon, and to write as many listeners as we want, in different places,
different addons.
...
For the creation of a record, it is in :meth:`awkhad.models.BaseModel.create`.
...
from awkhad.addons.component.core import AbstractComponent, Component
...
class EventCollecter(Component):
    ...
    the Awkhad's registry is rebuilt, hence the cache is cleared as well.

    An event always starts with ``on_``.

    Note that the special
    :class:`awkhad.addons.component_event.core.EventWorkContext` class should be
    used for this Component, because it can work
    without a collection.

    It is used by :meth:`awkhad.addons.component_event.models.base.Base._event`.
    ...
    def _complete_component_build(cls):
        ...
        # dynamically rebuild when awkhad registry is rebuild, we
        # are sure that the result is always the same for a lookup
        # until the next rebuild of awkhad's registry
