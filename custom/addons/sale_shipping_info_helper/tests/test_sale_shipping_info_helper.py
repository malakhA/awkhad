...
class TestDeliveryCost(common.TransactionCase):
    ...
    def test_00_shipping_info(self):
        ...
        line = self.SaleOrderLine.search([
            ...
            ('product_id', '=',
                self.sale.carrier_id.product_id.id)])
        self.assertEqual(len(line), 1, "Delivery cost is not Added")
        self.assertEqual(
            float_compare(line.price_subtotal, 10, precision_digits=2), 0,
            "Sale line delivery price subtotal is not correct")
        self.assertEqual(
            float_compare(line.price_total, 11, precision_digits=2), 0,
            "Sale line delivery price total is not correct")
        self.assertEqual(
            float_compare(line.price_tax, 1, precision_digits=2), 0,
            "Sale line delivery price tax is not correct")
        self.assertEqual(
            float_compare(
                self.sale.shipping_amount_tax, 1,
                precision_digits=2), 0,
            "Shipping amount tax is not correct")
        self.assertEqual(
            float_compare(
                self.sale.shipping_amount_untaxed, 10,
                precision_digits=2), 0,
            "Shipping amount untaxed is not correct")
        self.assertEqual(
            float_compare(
                self.sale.shipping_amount_total, 11,
                precision_digits=2), 0,
            "Shipping amount total is not correct")
        self.assertEqual(
            float_compare(
                self.sale.item_amount_tax, 75,
                precision_digits=2), 0,
            "Item amount tax is not correct")
        self.assertEqual(
            float_compare(
                self.sale.item_amount_untaxed, 750,
                precision_digits=2), 0,
            "Item amount untaxed is not correct")
        self.assertEqual(
            float_compare(
                self.sale.item_amount_total, 825.0,
                precision_digits=2), 0,
