awkhad.define('mail_base.base', function (require) {
"use strict";
...
chat_manager.notify_incoming_message = function (msg, options) {
    if (bus.is_awkhad_focused() && options.is_displayed) {
        // no need to notify
        return;
    }
...
chat_manager.make_message = function (data) {
    ...
    if ((!msg.author_id || !msg.author_id[0]) && msg.email_from) {
        ...
        msg.displayed_author = (msg.author_id === AWKHADBOT_ID) && "AwkhadBot" ||
                               msg.author_id && msg.author_id[1] ||
                               msg.email_from || _t('Anonymous');
    }

    // Don't redirect on author clicked of self-posted or AwkhadBot messages
    msg.author_redirect = !msg.is_author && msg.author_id !== AWKHADBOT_ID;

    // Compute the avatar_url
    if (msg.author_id === AWKHADBOT_ID) {
        msg.avatar_src = "/mail/static/src/img/awkhad_o.png";
...
chat_manager.on_transient_message_notification = function(data) {
    ...
    data.author_id = data.author_id || AWKHADBOT_ID;
