...
from awkhad.tools import OrderedSet, LastOrderedSet
...
# this is duplicated from awkhad.models.MetaModel._get_addon_name() which we
# unfortunately can't use because it's an instance method and should have been
# a @staticmethod
def _get_addon_name(full_name):
    # The (Awkhad) module name can be in the ``awkhad.addons`` namespace
    # or not. For instance, module ``sale`` can be imported as
    # ``awkhad.addons.sale`` (the right way) or ``sale`` (for backward
    # compatibility).
    module_parts = full_name.split('.')
    if len(module_parts) > 2 and module_parts[:2] == ['awkhad', 'addons']:
...
# it will be cleared and updated when the awkhad's registry is rebuilt
...
class WorkContext(object):
    ...
    .. attribute:: model_name
        ...
        as a `model` attribute to use directly with the Awkhad model from
        the components

    .. attribute:: collection

        The collection we are working with. The collection is an Awkhad
        Model that inherit from 'collection.base'. The collection attribute
        can be a record or an "empty" model.

    .. attribute:: model

        Awkhad Model for ``model_name`` with the same Awkhad
        :class:`~awkhad.api.Environment` than the ``collection`` attribute.
        ...
        if components_registry is not None:
            ...
            try:
                ...
                _logger.error(
                    ...
                    'Probably because the Awkhad registry has not been built '
                    'yet.', dbname
                )
                raise
    ...
    def component_by_name(self, name, model_name=None):
        ...
        A :exc:`awkhad.addons.component.exception.NoComponentError` is raised
        if:
    ...
    def component(self, usage=None, model_name=None):
        ...
        A :exc:`awkhad.addons.component.exception.SeveralComponentError` is
...
class AbstractComponent(object, metaclass=MetaComponent):
    ...
    Inheritance mechanism
        The inheritance mechanism is like the Awkhad's one for Models.  Each
        ...
        familiar if you already know Awkhad.  The first uses ``_inherit`` with
    ...
    Registration and lookups
        ...
        _apply_on
            List of names or name of the Awkhad model(s) for which the component
    ...
    * It is advised to namespace the names of the components (e.g.
    ...
    @property
    def env(self):
        """ Current Awkhad environment, the one of the collection record """
    ...
    def _build_component(cls, registry):
        ...
        This method is called at the end of the Awkhad's registry build.  The
        ...
        :meth:`awkhad.addons.connector.components.mapper.Mapper._complete_component_build`

        The following code is roughly the same than the Awkhad's one for
        building Models.
