...
class ComponentBuilder(models.AbstractModel):
    ...
    Every time an Awkhad registry is built, the know components are cleared and
    rebuilt as well.  The Component classes are built using the same mechanism
    than Awkhad's Models: a final class is created, taking every Components with
    a ``_name`` and applying Components with an ``_inherits`` upon them.

    The final Component classes are registered in global registry.

    This class is an Awkhad model, allowing us to hook the build of the
    components at the end of the Awkhad's registry loading, using
    ...
    def _register_hook(self):
        # This method is called by Awkhad when the registry is built,
    ...
    def load_components(self, module, components_registry=None):
        """ Build every component known by MetaComponent for an awkhad module
