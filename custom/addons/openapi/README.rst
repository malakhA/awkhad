...
with whatever system you need. All can be configured in Awkhad UI, no extra module
is needed.
...
As a workaround for multi-db Awkhad instances, system uses *Basic Authentication* with
...
* TODO: check whether module works in awkhad.sh and if not, document what exactly doesn't work and what a possible workarounds
...
* TODO: when user is not authenticated api returns 200 with the message below, instead of designed 401
  ...
  ::
    ...
    File "/opt/awkhad/vendor/it-projects-llc/sync-addons/openapi/controllers/pinguin.py", line 152, in authenticate_token_for_user
...
* TODO: ``.../swagger.json`` url doesn't work in multi-db mode in awkhad 12.0 at least: it make strange redirection to from ``/api/v1/demo/swagger.json?token=demo_token&db=source`` to ``/api/v1/demo/swagger.json?token%3Ddemo_token%26db%3Dsource``
...
* `IT-Projects LLC <https://it-projects.info>`__
      ...
      To get a guaranteed support you are kindly requested to purchase the module at `awkhad apps store <https://apps.awkhad.com/apps/modules/12.0/openapi/>`__.
...
HTML Description: https://apps.awkhad.com/apps/modules/12.0/openapi/
...
Tested on Awkhad 12.0 e1aee2aac2fd2c19dfd55734b1f51aedbc0393a6
