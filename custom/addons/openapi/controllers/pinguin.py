...
"""Pinguin module for Awkhad REST Api.
...
from awkhad.addons.web.controllers.main import ReportController
...
def get_data_from_auth_header(header):
    ...
    if len(decoded_token_parts) == 1:
        ...
        err_descrip = 'Basic auth header payload must be of the form "<%s>" (encoded to base64)' % 'user_token' if awkhad.tools.config['dbfilter'] else 'db_name:user_token'
...
def setup_db(httprequest, db_name):
    ...
    if httprequest.session.db:
        return
    if db_name not in awkhad.service.db.list_dbs(force=True):
...
def create_log_record(**kwargs):
    ...
    # transaction)
    if not test_mode:
        with awkhad.registry(request.session.db).cursor() as cr:
            # use new to save data even in case of an error in the old cursor
            env = awkhad.api.Environment(cr, request.session.uid, {})
...
def get_model_openapi_access(namespace, model):
    ...
    :returns: The error response object if namespace validation failed.
        A dictionary containing the model API configuration for this namespace.
            ...
            ```python
            {'context':                 (Dict)      awkhad context (default values through context),
...
def transform_dictfields_to_list_of_tuples(record, dct):
    ...
    :param awkhad.models.Model record: The model object.
...
def wrap__resource__create_one(modelname, context, data, success_code, out_fields):
    ...
    try:
        ...
        if not test_mode:
            ...
            # Probably, Awkhad (10.0 at least) uses different cursors
...
def get_model_for_read(model):
    ...
    if not test_mode:
        ...
        # Contrary to ISOLATION_LEVEL_SERIALIZABLE as per Awkhad Standard
...
def get_dict_from_record(record, spec, include_fields, exclude_fields):
    ...
    :param awkhad.models.Model record: The singleton record to load.
    ...
    for field in _spec:
        ...
        # Normal field, or unspecified relational
            ...
            if not hasattr(record, field):
                raise awkhad.exceptions.ValidationError(awkhad._('The model "%s" has no such field: "%s".') % (record._name, field))
...
def get_OAS_definitions_part(model_obj, export_fields_dict, definition_prefix='', definition_postfix=''):
    ...
    :param awkhad.models.Model model_obj: The model object.
