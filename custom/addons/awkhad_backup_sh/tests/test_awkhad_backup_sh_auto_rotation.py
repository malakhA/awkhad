...
import awkhad.tests.common


@awkhad.tests.common.at_install(False)
@awkhad.tests.common.post_install(True)
class TestAwkhadBackupSh(awkhad.tests.common.TransactionCase):
    ...
    def test_compute_auto_rotation_backup_dts(self):
        ...
        self.assertEqual(
            self.env['awkhad_backup_sh.config'].compute_auto_rotation_backup_dts(backup_dts, hourly=4),
            [dt for dt in backup_dts if backup_dts.index(dt) in [0, 1, 2, 3]]
        )
        self.assertEqual(
            self.env['awkhad_backup_sh.config'].compute_auto_rotation_backup_dts(backup_dts, daily=3),
            [dt for dt in backup_dts if backup_dts.index(dt) in [0, 2, 3]]
        )
        self.assertEqual(
            self.env['awkhad_backup_sh.config'].compute_auto_rotation_backup_dts(backup_dts, weekly=10),
            [dt for dt in backup_dts if backup_dts.index(dt) in [0, 2, 4, 5, 6, 7, 8]]
        )
        self.assertEqual(
            self.env['awkhad_backup_sh.config'].compute_auto_rotation_backup_dts(backup_dts, monthly=3),
            [dt for dt in backup_dts if backup_dts.index(dt) in [0, 2, 6]]
        )
        self.assertEqual(
            self.env['awkhad_backup_sh.config'].compute_auto_rotation_backup_dts(backup_dts, yearly=2),
            [dt for dt in backup_dts if backup_dts.index(dt) in [0, 7]]
        )
        self.assertEqual(
            self.env['awkhad_backup_sh.config'].compute_auto_rotation_backup_dts(backup_dts, hourly=2, daily=2, weekly=3),
        ...
        self.assertEqual(
            self.env['awkhad_backup_sh.config'].compute_auto_rotation_backup_dts(backup_dts, hourly=1000),
