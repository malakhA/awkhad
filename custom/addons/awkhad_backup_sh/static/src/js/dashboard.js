...
awkhad.define('awkhad_backup_sh.dashboard', function (require) {
'use strict';
...
var Dashboard = AbstractAction.extend(ControlPanelMixin, {
    template: 'awkhad_backup_sh.BackupDashboardMain',
    ...
    fetch_dashboard_data: function() {
        var self = this;
        return self._rpc({
                route: '/awkhad_backup_sh/fetch_dashboard_data',
    ...
    dashboard_can_backup: function(){
        return this.modules.awkhad_backup_sh.configured ||
            this.modules.awkhad_backup_sh_dropbox.configured ||
            this.modules.awkhad_backup_sh_google_disk.configured;
    },
    dashboard_basic: function(){
        return !this.modules.awkhad_backup_sh.configured &&
            !this.modules.awkhad_backup_sh_dropbox.installed &&
            !this.modules.awkhad_backup_sh_google_disk.installed;
    },
    dashboard_configure_dropbox: function(){
        return this.modules.awkhad_backup_sh_dropbox.installed &&
            !this.modules.awkhad_backup_sh_dropbox.configured;
    },
    dashboard_configure_google_disk: function(){
        return this.modules.awkhad_backup_sh_google_disk.installed &&
            !this.modules.awkhad_backup_sh_google_disk.configured;
    },
    dashboard_offer_iap: function(){
        return (this.modules.awkhad_backup_sh_dropbox.configured ||
                this.modules.awkhad_backup_sh_google_disk.configured) && !this.modules.awkhad_backup_sh.configured;
    },
    dashboard_iap: function(){
        return this.modules.awkhad_backup_sh.configured_iap;
    },
    dashboard_offer_extra_module: function(){
        return this.modules.awkhad_backup_sh.configured &&
            !this.modules.awkhad_backup_sh.configured_iap &&
            !this.modules.awkhad_backup_sh_dropbox.installed &&
            !this.modules.awkhad_backup_sh_google_disk.installed;
    },
    auth_via_awkhad: function(){
        if ('auth_link' in this.cloud_params) {
            self.do_action({
                name: "Auth via awkhad.com",
                target: 'self',
    ...
    render_backup_config_cards: function() {
        ...
        var $o_backup_dashboard_configs = self.$('.o_backup_dashboard_configs').append(
            QWeb.render('awkhad_backup_sh.config_cards', {configs: self.configs}));
    ...
    o_dashboard_get_s3_credentials: function(ev){
        window.location.href = '/awkhad_backup_sh/get_s3_credentials?redirect=' + encodeURIComponent(window.location.href);
    },
    o_dashboard_action_add_database: function (ev) {
        ...
        this.do_action({
            ...
            view_mode: 'form',
            views: [[false, "form"]],
            res_model: 'awkhad_backup_sh.config',
            target: 'current',
        },
        {
    ...
    close_dashboard_notification: function (ev) {
        ...
        this._rpc({
            model: 'awkhad_backup_sh.notification',
