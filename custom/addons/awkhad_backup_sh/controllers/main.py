...
import awkhad
from awkhad import exceptions, fields, http, _
from awkhad.exceptions import UserError
from awkhad.http import request
from awkhad.service import db
from awkhad.sql_db import db_connect
from awkhad.tools import config, DEFAULT_SERVER_DATETIME_FORMAT, DEFAULT_SERVER_DATE_FORMAT
from awkhad.tools.misc import str2bool
from awkhad.addons import web
from awkhad.addons.web.controllers.main import DBNAME_PATTERN
...
BACKUP_SERVICE_ENDPOINT = 'https://awkhad-backup.sh'

# module -> config parameter name
EXTRA_MODULES = {
    'awkhad_backup_sh_dropbox': lambda env: env['ir.config_parameter'].get_param('awkhad_backup_sh_dropbox.dropbox_access_token'),
    'awkhad_backup_sh_google_disk': lambda env: env['ir.config_parameter'].get_param('awkhad_backup_sh_google_disk.service_account_key'),
}
...
class BackupDatabase(web.controllers.main.Database):
    ...
    def _render_template(self, **d):
        ...
        # Show button 'Restore via Awkhad-backup.sh' on web/database/manager and web/database/selector pages
...
class BackupController(http.Controller):
    ...
    def get_cloud_params(cls, redirect=None, call_from='backend', update=True, env=None):
        mandadory_params = cloud_config_stores[call_from].get([
            'awkhad_backup_sh.s3_bucket_name',
            'awkhad_backup_sh.aws_access_key_id',
            'awkhad_backup_sh.aws_secret_access_key',
        ], env=env)
        cloud_params = cloud_config_stores[call_from].get([
            'awkhad_backup_sh.user_key',
            'awkhad_backup_sh.private_s3_dir',
            'awkhad_backup_sh.awkhad_oauth_uid'
        ], env=env)
    ...
    def update_cloud_params(cls, cloud_params, redirect, call_from):
        user_key = cloud_params['awkhad_backup_sh.user_key']
        if not user_key:
            user_key = ''.join(random.choice(string.hexdigits) for _ in range(30))
            cloud_config_stores[call_from].set('awkhad_backup_sh.user_key', user_key)
        ...
        if 'insufficient_credit_error' in cloud_params and call_from == 'backend':
            existing_msg = request.env['awkhad_backup_sh.notification'].sudo().search(
            ...
            if existing_msg:
                ...
                request.env['awkhad_backup_sh.notification'].sudo().create(notification_vals)
            ...
            if call_from == 'frontend':
                ...
                request.env['awkhad_backup_sh.notification'].sudo().search(
    ...
    def download_backup(self, backup_id, **kwargs):
        backup_info_record = request.env['awkhad_backup_sh.backup_info'].search([
            ('id', '=', backup_id),
            ('storage_service', '=', 'awkhad_backup_sh'),
        ])
    ...
    @http.route('/web/database/restore_via_awkhad_backup_sh', type='http', auth="none", methods=['POST'], csrf=False)
    def restore_via_awkhad_backup_sh(self, master_pwd, backup_file_name, name, encryption_password, copy=False):
            ...
            with closing(db_connect(name).cursor()) as cr:
                ...
                try:
                    cr.execute("""
                        ...
                        WHERE active=true AND id IN (SELECT ir_cron_id FROM awkhad_backup_sh_config_cron);

                        UPDATE awkhad_backup_sh_config SET active=false WHERE active=true;
                    """)
                except Exception:
                    pass
    ...
    @http.route('/awkhad_backup_sh/get_s3_credentials', type="http", auth='user')
    ...
    @http.route('/awkhad_backup_sh/fetch_dashboard_data', type="json", auth='user')
    def fetch_dashboard_data(self):
        ...
        values = request.env['awkhad_backup_sh.remote_storage'].search(last_month_domain).sorted(key='date')
        usage_values = {
        ...
        backup_configs = request.env['awkhad_backup_sh.config'].with_context({'active_test': False}).search_read(
            [], ['database', 'active', 'storage_service'])
        for b_config in backup_configs:
            graph_values = {date: 0 for date in last_week_dates}
            for backup in request.env['awkhad_backup_sh.backup_info'].search([
            ...
            b_config.update({
                'backups_number': request.env['awkhad_backup_sh.backup_info'].search_count([
        ...
        dashboard_data.update({
            ...
            'notifications': request.env['awkhad_backup_sh.notification'].search_read(
                [('is_read', '=', False)], ['id', 'date_create', 'message']),
            'up_balance_url': '%s/open_iap_recharge?user_key=%s' % (
                BACKUP_SERVICE_ENDPOINT,
                request.env['ir.config_parameter'].sudo().get_param('awkhad_backup_sh.user_key')
            ),
        })
        modules = {
            'awkhad_backup_sh': {
                'installed': True,
                'configured': bool(cloud_params.get('awkhad_backup_sh.aws_access_key_id') or False),
                'configured_iap': bool(cloud_params.get('awkhad_backup_sh.awkhad_oauth_uid') or False)
            }
        }
        version = '%s.%s' % (version_info[0], version_info[1])
        for m_name in EXTRA_MODULES.keys():
            modules[m_name] = {
                'url': "https://apps.awkhad.com/apps/modules/%s/%s/" % (version, m_name)
            }
    ...
    def wrapped(self, *args, **kwargs):
        try:
            ...
            if client_error.response['Error']['Code'] == 'InvalidAccessKeyId':
                ...
                # [request.env['ir.config_parameter'].set_param(option, None) for option in ('awkhad_backup_sh.aws_access_key_id', 'awkhad_backup_sh.aws_secret_access_key')]
...
# TODO: would it better to move this to awkhad_backup_sh.config ?
class BackupCloudStorage(http.Controller):

    @classmethod
    def get_amazon_s3_client(cls, cloud_params):
        s3_client = boto3.client('s3', aws_access_key_id=cloud_params['awkhad_backup_sh.aws_access_key_id'],
                                 aws_secret_access_key=cloud_params['awkhad_backup_sh.aws_secret_access_key'])
        return s3_client

    @classmethod
    def get_s3_dir(cls, cloud_params):
        if cloud_params.get('awkhad_backup_sh.awkhad_oauth_uid'):
            return cloud_params['awkhad_backup_sh.awkhad_oauth_uid']
        else:
            return cloud_params['awkhad_backup_sh.private_s3_dir'] or '.'
    ...
    def get_all_files(cls, cloud_params):
        ...
        list_objects = amazon_s3_client.list_objects_v2(
            Bucket=cloud_params['awkhad_backup_sh.s3_bucket_name'], Prefix=user_dir_name, Delimiter='/')
        all_files = [
            (obj['Key'].split('/')[-1], 'awkhad_backup_sh')
    ...
    def get_object(cls, cloud_params, filename=None, key=None):
        ...
        return amazon_s3_client.get_object(Bucket=cloud_params['awkhad_backup_sh.s3_bucket_name'], Key=key)
    ...
    def put_object(cls, cloud_params, obj, obj_key):
        ...
        amazon_s3_client.put_object(Body=obj, Bucket=cloud_params['awkhad_backup_sh.s3_bucket_name'], Key=obj_key)
    ...
    def delete_objects(cls, cloud_params, s3_keys):
        ...
        amazon_s3_client.delete_objects(
            Bucket=cloud_params['awkhad_backup_sh.s3_bucket_name'], Delete={'Objects': objs})
