...
from awkhad.exceptions import UserError, AccessError
from awkhad.tools import DEFAULT_SERVER_DATE_FORMAT, DEFAULT_SERVER_DATETIME_FORMAT
...
def get_backup_by_id(env, backup_id):
    backup = env['awkhad_backup_sh.backup_info'].browse(backup_id).exists()
    if len(backup) == 0:
...
class BackupConfig(models.Model):
    ...
    def _compute_database_names(self):
        if awkhad.tools.config['list_db']:
            return [(db, db) for db in awkhad.service.db.list_dbs() if db != 'session_store']
        else:
    ...
    config_cron_ids = fields.One2many('awkhad_backup_sh.config.cron', 'backup_config_id', string='Scheduled Auto Backups')
    ...
    def _compute_backup_count(self):
        self.backup_count = len(self.env['awkhad_backup_sh.backup_info'].search([
    ...
    def action_dashboard_redirect(self):
        return self.env.ref('awkhad_backup_sh.backup_dashboard').read()[0]
    ...
    def get_simulation_backup_list(self, config):
        info_records = self.env['awkhad_backup_sh.backup_info'].search([('backup_simulation', '=', True),
    ...
    def get_backup_list(self, cloud_params, service):
        ...
        try:
            ...
            configs = self.env['awkhad_backup_sh.config'].with_context({'active_test': False}).search([('backup_simulation', '=', True)])
    ...
    def delete_remote_objects(self, cloud_params, remote_objects):
        ...
        for obj in remote_objects:
            ...
            awkhad_backup_sh_objects.append('%s/%s' % (path, obj[0]))
        if awkhad_backup_sh_objects:
            return BackupCloudStorage.delete_objects(cloud_params, awkhad_backup_sh_objects)
        return {}
    ...
    def update_info(self, cloud_params, service=None):
        """
        * apply rotations
        * sync awkhad_backup_sh.backup_info records:
        ...
        for f in backup_list['all_files']:
            ...
            file_name = f[0]
            # service_name is awkhad_backup_sh, dropbox, etc.
            service_name = f[1]
        ...
        for r in self.env['awkhad_backup_sh.backup_info'].search([('active', '=', True)]):
        ...
        # Archive old records
        self.env['awkhad_backup_sh.backup_info'].browse(backup_info_ids_to_archive).write({
            'active': False
        })
        # Compute total remote storage after archive
        self.env['awkhad_backup_sh.remote_storage'].compute_total_used_remote_storage()
        ...
        for db_name, backup_dts in remote_backups.items():
            for backup_dt, files in backup_dts.items():
                ...
                record_exists = self.env['awkhad_backup_sh.backup_info'].search([
                    ('database', '=', db_name),
                ...
                self.env['awkhad_backup_sh.backup_info'].create(backup_info_vals)
        return backup_list
    ...
    def get_dump_stream_and_info_file(self, name, service, ts):
        ...
        if config_record.backup_simulation:
            ...
            dump_stream = awkhad.service.db.dump_db(name, None, 'zip')
    ...
    def make_backup(self, name, service, init_by_cron_id=None):
        ...
        self.env['awkhad_backup_sh.backup_info'].sudo().create(info_file_content)
        if init_by_cron_id:
            self.update_info(cloud_params, service=service)
        return None

    @api.model
    def make_backup_awkhad_backup_sh(self, ts, name, dump_stream, info_file, info_file_content, cloud_params):
    ...
    def install_demo_data(self):
        ...
        info_obj = self.env['awkhad_backup_sh.backup_info']
    ...
    def action_list_backups(self):
        return {
            ...
            'res_model': 'awkhad_backup_sh.backup_info',
            'domain': [
                ('database', '=', self.database),
            ],
            'search_view_id': self.env.ref('awkhad_backup_sh.awkhad_backup_sh_backup_search').id,
        }


class BackupConfigCron(models.Model):
    _name = 'awkhad_backup_sh.config.cron'
    _inherits = {'ir.cron': 'ir_cron_id'}
    _description = 'Backup Configuration Scheduled Actions'

    backup_config_id = fields.Many2one(
        'awkhad_backup_sh.config', string='Backup Configuration', required=True)
    ir_cron_id = fields.Many2one('ir.cron', string='Scheduled Action', required=True, ondelete='restrict')

    @api.model
    def create(self, vals):
        config = self.env['awkhad_backup_sh.config'].browse(vals['backup_config_id'])
        vals.update({
            'name': 'Awkhad-backup.sh: Backup the "%s" database, send it to %s storage, apply rotation' % (config.database, config.storage_service),
            'model_id': self.env.ref('awkhad_backup_sh.model_awkhad_backup_sh_config').id,
            'state': 'code',
            'numbercall': -1,
            'doall': False,
        })
...
class BackupInfo(models.Model):
    _name = 'awkhad_backup_sh.backup_info'
    ...
    def assert_user_can_download_backup(self):
        if not self.user_has_groups('awkhad_backup_sh.group_manager'):
...
class BackupNotification(models.Model):
    _name = 'awkhad_backup_sh.notification'
    ...
    def create_mail_activity_record(self):
        self.env['mail.activity'].create({
            'res_id': self.id,
            'res_model_id': self.env.ref('awkhad_backup_sh.model_awkhad_backup_sh_notification').id,
            'activity_type_id': self.env.ref('awkhad_backup_sh.mail_activity_data_notification').id,
    ...
    def fetch_notifications(self):
        ...
        data = {'params': {
            'user_key': config_params.get_param('awkhad_backup_sh.user_key'),
            'date_last_request': config_params.get_param('awkhad_backup_sh.date_last_request', None)
        }}
        response = requests.post(BACKUP_SERVICE_ENDPOINT + '/fetch_notifications', json=data).json()['result']
        config_params.set_param('awkhad_backup_sh.date_last_request', response['date_last_request'])
...
class BackupRemoteStorage(models.Model):
    _name = 'awkhad_backup_sh.remote_storage'
    ...
    s3_used_remote_storage = fields.Integer(string='Awkhad Backup sh Usage, MB', readonly=True)
    ...
    def compute_total_used_remote_storage(self):
        ...
        amount = sum(self.env['awkhad_backup_sh.backup_info'].search([]).mapped('backup_size'))
    ...
    def compute_s3_used_remote_storage(self):
        amount = sum(self.env['awkhad_backup_sh.backup_info'].search([('storage_service', '=', S3_STORAGE)]).mapped('backup_size'))
        today_record = self.search([('date', '=', datetime.strftime(datetime.now(), DEFAULT_SERVER_DATE_FORMAT))])
        if today_record:
...
class DeleteRemoteBackupWizard(models.TransientModel):
    _name = "awkhad_backup_sh.delete_remote_backup_wizard"
    _description = "Delete Remote Backups Wizard"

    @api.multi
    def delete_remove_backup_button(self):
        record_ids = []
        if self._context.get('active_model') == 'awkhad_backup_sh.backup_info':
            record_ids = self._context.get('active_ids')

        backup_info_records = self.env['awkhad_backup_sh.backup_info'].search([('id', 'in', record_ids)])
