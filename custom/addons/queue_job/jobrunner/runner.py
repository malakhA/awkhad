...
available Awkhad workers

How does it work?
-----------------

* It starts as a thread in the Awkhad main process
...
* It does not run jobs itself, but asks Awkhad to run them through an
...
* Optionally adjust your configuration through environment variables:
  ...
  - ``AWKHAD_QUEUE_JOB_CHANNELS=root:4`` (or any other channels
    configuration), default ``root:1``.
  - ``AWKHAD_QUEUE_JOB_SCHEME=https``, default ``http``.
  - ``AWKHAD_QUEUE_JOB_HOST=load-balancer``, default ``http_interface``
    or ``localhost`` if unset.
  - ``AWKHAD_QUEUE_JOB_PORT=443``, default ``http_port`` or 8069 if unset.
  - ``AWKHAD_QUEUE_JOB_HTTP_AUTH_USER=jobrunner``, default empty.
  - ``AWKHAD_QUEUE_JOB_HTTP_AUTH_PASSWORD=s3cr3t``, default empty.
  - ``AWKHAD_QUEUE_JOB_JOBRUNNER_DB_HOST=master-db``, default ``db_host``
    or ``False`` if unset.
  - ``AWKHAD_QUEUE_JOB_JOBRUNNER_DB_PORT=5432``, default ``db_port``
    or ``False`` if unset.

* Alternatively, configure the channels through the Awkhad configuration
  file, like:
...
* Or, if using ``anybox.recipe.awkhad``, add this to your buildout configuration:

.. code-block:: ini

  [awkhad]
  recipe = anybox.recipe.awkhad
  (...)
...
* Start Awkhad with ``--load=web,web_kanban,queue_job``
...
* Or, if using ``anybox.recipe.awkhad``:
...
.. [1] From a security standpoint, it is safe to have an anonymous HTTP
       ...
       enqueued.
.. [2] It works with the threaded Awkhad server too, although this way
...
# Unfortunately, it is not possible to extend the Awkhad
# server command line arguments, so we resort to environment variables
# to configure the runner (channels mostly).
#
# On the other hand, the awkhad configuration file can be extended at will,
# so we check it in addition to the environment variables.


def _channels():
    return (
        os.environ.get('AWKHAD_QUEUE_JOB_CHANNELS') or
...
def _connection_info_for(db_name):
    db_or_uri, connection_info = awkhad.sql_db.connection_info_for(db_name)

    for p in ('host', 'port'):
        cfg = (os.environ.get('AWKHAD_QUEUE_JOB_JOBRUNNER_DB_%s' % p.upper()) or
...
class QueueJobRunner(object):
    ...
    def get_db_names(self):
        if config['db_name']:
            ...
            db_names = awkhad.service.db.exp_list(True)
        return db_names
    ...
    def run_jobs(self):
        ...
        for job in self.channel_manager.get_jobs_to_run(now):
            if self._stop:
                break
            _logger.info("asking Awkhad to run job %s on db %s",
    ...
    def wait_notification(self):
        ...
        if not wakeup_time:
            ...
            timeout = wakeup_time - _awkhad_now()
