...
from awkhad.service import server
from awkhad.tools import config
try:
    from awkhad.addons.server_environment import serv_config
...
# Here we monkey patch the Awkhad server to start the job runner thread
...
class QueueJobRunnerThread(Thread):
    ...
    def __init__(self):
        ...
        self.daemon = True
        scheme = (os.environ.get('AWKHAD_QUEUE_JOB_SCHEME') or
                  queue_job_config.get("scheme"))
        host = (os.environ.get('AWKHAD_QUEUE_JOB_HOST') or
                queue_job_config.get("host") or
                config['http_interface'])
        port = (os.environ.get('AWKHAD_QUEUE_JOB_PORT') or
                queue_job_config.get("port") or
                config['http_port'])
        user = (os.environ.get('AWKHAD_QUEUE_JOB_HTTP_AUTH_USER') or
                queue_job_config.get("http_auth_user"))
        password = (os.environ.get('AWKHAD_QUEUE_JOB_HTTP_AUTH_PASSWORD') or
