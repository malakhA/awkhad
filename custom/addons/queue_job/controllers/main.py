...
import awkhad
from awkhad import _, http, tools
from awkhad.service.model import PG_CONCURRENCY_ERRORS_TO_RETRY
...
class RunJobController(http.Controller):
    ...
    def runjob(self, db, job_uuid, **kw):
        ...
        env = http.request.env(user=awkhad.SUPERUSER_ID)

        def retry_postpone(job, message, seconds=None):
            job.env.clear()
            with awkhad.api.Environment.manage():
                with awkhad.registry(job.env.cr.dbname).cursor() as new_cr:
