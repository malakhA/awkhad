...
class DelayableRecordset(object):
    ...
    :func:`~awkhad.addons.queue_job.job.job`.
    ...
    by :meth:`~awkhad.addons.queue_job.models.base.Base.with_delay`
    """
    ...
    def __getattr__(self, name):
        ...
        if not getattr(recordset_method, 'delayable', None):
            raise AttributeError(
                ...
                'it should be decorated with awkhad.addons.queue_job.job.job' %
                (name, self.recordset)
            )
...
def identity_exact(job_):
    ...
    .. python::
        ...
        from awkhad.addons.queue_job.job import identity_exact
...
class Job(object):
    ...
    Jobs are stored in the ``queue.job`` Awkhad Model, but they are handled
    through this class.
    ...
    .. attribute:: model_name
        ...
        Awkhad model on which the job will run.
    ...
    .. attribute:: user_id
        ...
        Awkhad user id which created the job
...
def _is_model_method(func):
    return (inspect.ismethod(func) and
            isinstance(func.__self__.__class__, awkhad.models.MetaModel))
...
def related_action(action=None, **kwargs):
    ...
    A *Related Action* will appear as a button on the Awkhad view.
