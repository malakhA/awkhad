...
from awkhad.addons.point_of_sale.tests.common import TestPointOfSaleCommon
...
class TestMicropay(TestPointOfSaleCommon):
    ...
    def test_micropay_backend(self):
        ...
        * Cashier scanned buyer's QR and upload it to awkhad server,
        awkhad server sends information to wechat servers and wait for response with result.

        * Once user authorize the payment, awkhad receives result syncroniosly from
        previously sent request.

        * Awkhad sends result to POS via longpolling.
