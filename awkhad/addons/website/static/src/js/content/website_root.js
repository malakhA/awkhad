...
awkhad.define('website.WebsiteRoot', function (require) {
'use strict';
...
require("website.content.zoomawkhad");
...
var WebsiteRoot = BodyManager.extend({
    ...
    start: function () {
        ...
        this.$('.zoomable img[data-zoom]').zoomAwkhad();
            ...
            return new Dialog(self, {
                ...
                $content: $('<div/>', {
                    html: (data.data ? data.data.arguments[1] : data.statusText)
                        + '<br/>'
                        + _.str.sprintf(
                            _t('It might be possible to edit the relevant items or fix the issue in <a href="%s">the classic Awkhad interface</a>'),
    ...
    _multiCompanySwitch: function (ev) {
        ...
        this._rpc({model: 'res.users',
            method: 'write',
            args: [awkhad.session_info.user_id, {'company_id': parseInt(company_id_to_switch_to, 10)}],
        }).then(function () {
