...
from awkhad.addons.base.models.ir_http import RequestUID, ModelConverter
from awkhad.http import request
from awkhad.tools import config, ustr, pycompat
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if awkhad._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            awkhad._geoip_resolver = GeoIPResolver.open(geofile) or False
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if awkhad._geoip_resolver and request.httprequest.remote_addr:
                record = awkhad._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except awkhad.exceptions.MissingError:
