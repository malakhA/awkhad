...
from awkhad.tests.common import HttpCase, TransactionCase
from awkhad.tools import DotDict
...
class TestWebsitePriceList(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('awkhad.addons.website_sale.models.website.Website.get_pricelist_available', wraps=self._get_pricelist_available)
        patcher.start()
...
def simulate_frontend_context(self, website_id=1):
    ...
    patcher = patch('awkhad.addons.website.models.ir_http.get_request_website', wraps=get_request_website)
    patcher.start()
    self.addCleanup(patcher.stop)
