awkhad.define('partner.autocomplete.core', function (require) {
'use strict';
...
return {
    _dropPreviousAwkhad: new concurrency.DropPrevious(),
    ...
    autocomplete: function (value) {
        ...
        var awkhadPromise = this._getAwkhadSuggestions(value, isVAT).then(function (suggestions){
            awkhadSuggestions = suggestions;
        });
        ...
        var concatResults = function () {
            // Add Clearbit result with Awkhad result (with unique domain)
            if (clearbitSuggestions && clearbitSuggestions.length) {
                var websites = awkhadSuggestions.map(function (suggestion) {
                ...
                clearbitSuggestions.forEach(function (suggestion) {
                    if (websites.indexOf(suggestion.domain) < 0) {
                        ...
                        awkhadSuggestions.push(suggestion);
                    }
                });
            }

            awkhadSuggestions = _.filter(awkhadSuggestions, function (suggestion) {
                return !suggestion.ignored;
            });
            _.each(awkhadSuggestions, function(suggestion){
              delete suggestion.ignored;
            });
            return def.resolve(awkhadSuggestions);
        };

        this._whenAll([awkhadPromise, clearbitPromise]).then(concatResults, concatResults);
    ...
    /**
     * Use Awkhad Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Deferred}
     * @private
     */
    _getAwkhadSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousAwkhad.add(def);
    },
    /**
