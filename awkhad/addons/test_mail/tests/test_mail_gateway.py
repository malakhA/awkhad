...
from awkhad.addons.test_mail.data.test_mail_data import \
...
from awkhad.addons.test_mail.tests.common import BaseFunctionalTest, MockEmails
from awkhad.addons.test_mail.tests.common import mail_new_test_user
from awkhad.tools import mute_logger, formataddr


class TestEmailParsing(BaseFunctionalTest, MockEmails):

    @mute_logger('awkhad.addons.mail.models.mail_thread')
...
class TestMailgateway(BaseFunctionalTest, MockEmails):
    ...
    def setUpClass(cls):
        ...
        cls.fake_email = cls.env['mail.message'].create({
            ...
            'message_id': '<123456-zgui-%s-mail.test.simple@%s>' % (cls.test_record.id, socket.gethostname()),
        })
    ...
    @mute_logger('awkhad.addons.mail.models.mail_thread', 'awkhad.models')
    ...
    @mute_logger('awkhad.addons.mail.models.mail_thread', 'awkhad.models.unlink', 'awkhad.addons.mail.models.mail_mail')
    ...
    @mute_logger('awkhad.addons.mail.models.mail_thread', 'awkhad.addons.mail.models.mail_mail', 'awkhad.models.unlink')
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by awkhad should update thread accordingly """
    ...
    def test_message_process_model_res_id(self):
        """ Incoming email with ref holding model / res_id but that does not match any message in the thread: must raise since ZgUI saas-3 """
        self.assertRaises(ValueError,
                          ...
                          extra='In-Reply-To: <12321321-zgui-%d-mail.test.simple@%s>' % (self.test_record.id, socket.gethostname()),
                          msg_id='<1198923581.41972151344608186802.JavaMail.diff1@agrolait.com>')

        # when 6.1 messages are present, compat mode is available
        # Awkhad 10 update: compat mode has been removed and should not work anymore
        ...
        # Awkhad 10 update: compat mode has been removed and should not work anymore and does not depend from hostname
        self.assertRaises(ValueError,
                          self.format_and_process,
                          MAIL_TEMPLATE, email_from=formataddr((self.partner_1.name, self.partner_1.email)),
                          msg_id='<1.3.JavaMail.new@agrolait.com>',
                          to='noone@example.com>', subject='spam',
                          extra='In-Reply-To: <12321321-zgui-%d-mail.test@neighbor.com>' % self.test_record.id)
    ...
    @mute_logger('awkhad.addons.mail.models.mail_thread', 'awkhad.models', 'awkhad.addons.mail.models.mail_mail')
    def test_private_discussion(self):
        ...
        # Test: message-id
        self.assertIn('zgui-private', msg.message_id.split('@')[0], 'message_post: private discussion: message-id should contain the private keyword')
