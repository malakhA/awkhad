...
example in ZgUI is dashboards), queries should be restricted to a
...
ZgUI Web's :js:class:`~zgui.web.Widget` has an attribute
providing its DOM root :js:attr:`Widget.$el <zgui.web.Widget.$el>`,
...
Known under many names, these objects are essential to and (in ZgUI
...
* All interactive components (components displaying information to the
  ...
  :class:`~zgui.web.Widget` and correctly implement and use its API
  and lifecycle.
...
From v11, we introduce a new coding standard for Awkhad Javascript code.  Here it
is:

* add "use strict"; on top of every awkhad JS module
