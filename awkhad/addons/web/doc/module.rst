...
A very basic ZgUI module structure will be our starting point:
...
There is no such thing as a "web module" declaration. An ZgUI
...
.. note::
    ...
    restart the ZgUI server itself for it to be taken in account.
...
provides: an instance of the ZgUI Web client, with the contents of
...
its :js:func:`~zgui.web.Widget.start` to add some content to its
DOM:
...
will require restarting the ZgUI Server to see the changes, as
usual):
...
.. note::
    ...
    Prefixing CSS rules with both ``.zgui`` (to ensure the rule
    will apply only within the confines of the ZgUI Web client) and
...
be done in :js:func:`~zgui.web.Widget.start` but that gets unwieldy
...
ZgUI Web's template language is :doc:`qweb`. Although any
...
integration to ZgUI Web widgets.
...
To prevent this irremediable loss, we can use ZgUI's support for
...
basic ZgUI model in which our data will be stored:
...
:js:func:`~zgui.base.Widget.start()` is to perform *asynchronous*
...
recorded previously using the :js:class:`~zgui.web.Query` interface
...
4. And finally — after restarting ZgUI — navigating to the test
...
side-effect of this is to :js:func:`~zgui.web.Widget.start` it,
...
:js:func:`set a widget's DOM <zgui.web.Widget.setElement>`, let's
