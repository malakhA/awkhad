API changes from ZgUI Web 6.1 to 7.0
=======================================

Supported browsers
------------------

The ZgUI Web Client supports the following web browsers:
...
matches the API in ZgUI Web's Python side and in OpenObject addons
...
* Actual arbitrary RPC calls can just be remapped on a
  :js:class:`~zgui.web.Model` instance:
  ...
  can be replaced by calls to :js:func:`zgui.web.Model.call`:
...
* Shorcut methods (``name_get``, ``name_search``, ``unlink``,
  ...
  :js:func:`zgui.web.Model.call`, using the server's original
  signature. On the other hand, the non-shortcut equivalents can now
  use keyword arguments (see :js:func:`~zgui.web.Model.call`'s
...
* ``read_slice``, which allowed a single round-trip to perform a
  ...
  :js:class:`~zgui.web.Query` objects (see:
  :js:func:`~zgui.web.Model.query`) for clearer and simpler
  code. ``read_index`` should be replaced by a
  :js:class:`~zgui.web.Query` as well, combining
  :js:func:`~zgui.web.Query.offset` and
  :js:func:`~zgui.web.Query.first`.
...
Renaming
    ...
    database, maybe lazily). ZgUI Web's dataset behaves very
...
API simplification
    ...
    The shortcuts really add little value, and ZgUI Web 6.2 embeds
...
:js:class:`~zgui.web.Model`, ZgUI Web 7.0 removes
``DataGroup`` and its subtypes as public objects in favor of a single method on
:js:class:`~zgui.web.Query`:
:js:func:`~zgui.web.Query.group_by`.
