...
Client actions are the client-side version of ZgUI's "Server
...
In the ZgUI Web code, a client action only requires two pieces of
information:
...
* Providing said object. Two different types of objects can be mapped
  ...
  * An ZgUI Web widget, which must inherit from
    :js:class:`zgui.web.Widget`
...
* if, on the other hand, the client action maps to a
  :js:class:`~zgui.web.Widget`, that
  :js:class:`~zgui.web.Widget` will be instantiated and added to
  the web client's canvas, with the usual
  :js:class:`~zgui.web.Widget` lifecycle (essentially, it will
...
object::
    ...
    // Registers the object 'zgui.web_dashboard.Widget' to the client
...
At this point, the generic :js:class:`~zgui.web.Widget` lifecycle
...
should be performed in the :js:func:`~zgui.web.Widget.start()`
method.

.. note::

    As required by :js:class:`~zgui.web.Widget`'s contract, if
    :js:func:`~zgui.web.Widget.start()` executes any asynchronous
