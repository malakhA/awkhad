awkhad.define('web.data', function (require) {
"use strict";
...
var Query = Class.extend({
     ...
     * @returns {jQuery.Deferred<Array<zgui.web.QueryGroup>> | null}
     */
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of ZgUI records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the ZgUI model this dataset will manage
     */
    ...
    get_id_index: function (id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // ZgUI ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // ZgUI ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, ZgUI domain
     ...
     * an SQL-like form usable by ZgUI's ORM.
     *
     ...
     * Awkhad's ORM (such as 'name desc', 'product_id', 'order_date asc')
     */
...
var Model = Class.extend({
    /**
    new zgui.web.Model(name[, context[, domain]])

    @constructs instance.web.Model
    @extends instance.web.Class

    @param {String} name name of the ZgUI model this object is bound to
    @param {Object} [context]
    @param {Array} [domain]
    */
    ...
    /**
     * Call a method (over RPC) on the bound ZgUI model.
     *
