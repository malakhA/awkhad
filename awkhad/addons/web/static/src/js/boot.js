...
(function() {
    ...
    var awkhad = window.awkhad = window.awkhad || {};
    _.extend(awkhad, {
            ...
            if (jobs.length) {
                ...
                for (var k=0; k<jobs.length; k++) {
                    debug_jobs[jobs[k].name] = job = {
                        ...
                        dependents: awkhad.__DEBUG__.get_dependents(jobs[k].name),
                        name: jobs[k].name
                    };
                    if (jobs[k].error) {
                    ...
                    var deps = awkhad.__DEBUG__.get_dependencies( job.name );
                ...
                missing = awkhad.__DEBUG__.get_missing_jobs();
                failed = awkhad.__DEBUG__.get_failed_jobs();
                var unloaded = _.filter(debug_jobs, function (job) { return job.missing; });

                if (awkhad.debug || !_.isEmpty(failed) || !_.isEmpty(unloaded)) {
                    ...
                    if (awkhad.debug && !_.isEmpty(debug_jobs)) {
                        log('Debug:                   ', debug_jobs);
                    }
                }
            }
            awkhad.__DEBUG__.js_modules = {
                missing: missing,
                failed: _.pluck(failed, 'name'),
            };
            awkhad.__DEBUG__.didLogInfo.resolve();
        },
                ...
                if (!job.error) {
                    $.when(job_exec).then(
                        function (data) {
                            ...
                            def.resolve();
                            awkhad.process_jobs(jobs, services);
                        }, function (e) {
                            job.rejected = e || true;
                            jobs.push(job);
                            def.resolve();
