awkhad.define('web.Session', function (require) {
"use strict";
...
var Session = core.Class.extend(mixins.EventDispatcherMixin, {
    ...
    init: function (parent, origin, options) {
        ...
        options = options || {};
        this.module_list = (options.modules && options.modules.slice()) || (window.awkhad._modules && window.awkhad._modules.slice()) || [];
        this.server = null;
    ...
    on_modules_loaded: function () {
        ...
        for(var j=0; j<this.module_list.length; j++) {
            ...
            zgui[mod] = {};
            // init module mod
            var fct = zgui._openerp[mod];
            if(typeof(fct) === "function") {
                zgui._openerp[mod] = {};
                for (var k in fct) {
                    zgui._openerp[mod][k] = fct[k];
                }
                fct(zgui, zgui._openerp[mod]);
            }
    ...
    session_reload: function () {
        var result = _.extend({}, window.awkhad.session_info);
        delete result.session_id;
        ...
        var promise = deferred.then(function () {
            ...
            if (self.origin_server) {
                ...
                if (self.override_session) {
                    options.headers["X-Zgui-Session-Id"] = self.session_id || '';
                }
