awkhad.define('web.mixins', function (require) {
"use strict";
...
function AwkhadEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
AwkhadEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};

AwkhadEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
var EventDispatcherMixin = _.extend({}, ParentedMixin, {
     ...
     * ZgUI Web where developers may want to replace existing callbacks with
     * theirs.
     *
    ...
    trigger_up: function (name, info) {
        var event = new AwkhadEvent(this, name, info);
