...
import awkhad
import awkhad.modules.registry
from awkhad.api import call_kw, Environment
from awkhad.modules import get_resource_path
from awkhad.tools import crop_image, topological_sort, html_escape, pycompat
from awkhad.tools.mimetypes import guess_mimetype
from awkhad.tools.translate import _
from awkhad.tools.misc import str2bool, xlwt, file_open
from awkhad.tools.safe_eval import safe_eval
from awkhad import http
from awkhad.http import content_disposition, dispatch_rpc, request, \
    serialize_exception as _serialize_exception, Response
from awkhad.exceptions import AccessError, UserError, AccessDenied
from awkhad.models import check_method_name
from awkhad.service import db, security
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('awkhad.addons.web', "views")
...
def module_installed_bypass_session(dbname):
    try:
        registry = awkhad.registry(dbname)
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, awkhad.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = awkhad.conf.server_wide_modules or []
    serverside = ['base', 'web']
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("awkhad.addons.web.main.manifest_list(): debug parameter is deprecated")
...
def fix_view_modes(action):
    """ For historical reasons, Awkhad has weird dealings in relation to
...
def _local_web_translations(trans_file):
    ...
    try:
        ...
        return
    for x in po:
        if x.id and x.string and "zgui-web" in x.auto_comments:
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = awkhad.SUPERUSER_ID
        ...
        if request.httprequest.method == 'POST':
            ...
            except awkhad.exceptions.AccessDenied as e:
                request.uid = old_uid
                if e.args == awkhad.exceptions.AccessDenied().args:
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = awkhad.SUPERUSER_ID
...
class WebClient(http.Controller):
    ...
    def translations(self, mods=None, lang=None):
        ...
        messages = request.env['ir.translation'].sudo().search_read([
            ...
            ('comments', 'like', 'zgui-web'), ('value', '!=', False),
            ('value', '!=', '')],
    ...
    def version_info(self):
        return awkhad.service.common.exp_version()
...
class Proxy(http.Controller):
    ...
    def post(self, path):
        ...
        with request.session.load_request_data() as data:
            ...
            headers = {'X-Zgui-Session-Id': request.session.sid}
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = awkhad.tools.config.verify_admin_password('admin')
        d['list_db'] = awkhad.tools.config['list_db']
        d['langs'] = awkhad.service.db.exp_list_lang()
        d['countries'] = awkhad.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = awkhad.service.db.list_db_incompatible(d['databases'])
        except awkhad.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        try:
            awkhad.service.db.check_super(master_pwd)
            ...
            dump_stream = awkhad.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=awkhad.SUPERUSER_ID))
    ...
    def account(self):
        ...
        return 'https://accounts.awkhad.com/oauth2/auth?' + werkzeug.url_encode(params)
    ...
    def content_image(self, xmlid=None, model='ir.attachment', id=None, field='datas',
        ...
        if not content:
            ...
            if not (width or height):
                ...
                if suffix in ('small', 'medium', 'big'):
                    content = getattr(awkhad.tools, 'image_resize_image_%s' % suffix)(content)
        ...
        if crop and (width or height):
            ...
            if not upper_limit:
                ...
                if height > 500:
                    height = 500
            content = awkhad.tools.image_resize_image(base64_source=content, size=(width or None, height or None),
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = awkhad.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: (field[0] not in ['id', '.id', 'display_name', 'name'], awkhad.tools.ustr(field[1].get('string', ''))))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from Awkhad's export data to whatever the
