...
def get_awkhad_server_url():
    return read_file_first_line('awkhad-remote-server.conf')
...
class StatusController(http.Controller):
    ...
    def connect_box(self, token):
        """
        This route is called when we want that a IoT Box will be connected to a Awkhad DB
        token is a base 64 encoded string and have 2 argument separate by |
        1 - url of awkhad DB
        2 - token. This token will be compared to the token of Awkhad. He have 1 hour lifetime
        """
...
class Manager(Thread):
    ...
    def load_drivers(self):
        """
        This method loads local files: 'awkhad/addons/hw_drivers/drivers'
    ...
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to Awkhad database
        """
        server = get_awkhad_server_url()
        if server:
            iot_box = {
            ...
            _logger.warning('Awkhad server not set')
    ...
    def run(self):
        """
        Thread that will check connected/disconnected device, load drivers if needed and contact the awkhad server with the updates
        """
        devices = {}
        updated_devices = {}
