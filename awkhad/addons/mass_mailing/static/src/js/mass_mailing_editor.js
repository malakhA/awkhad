awkhad.define('mass_mailing.editor', function (require) {
"use strict";
...
var awkhad_top = window.top.awkhad;
...
snippets_editor.Class.include({
    _computeSnippetTemplates: function (html) {
        ...
        $mail_themes_upgrade.on("click", function (e) {
            ...
            e.preventDefault();
            awkhad_top[window.callback+"_do_action"]("mass_mailing.action_mass_mailing_configuration");
        });
        ...
        $dropdown.on("click", ".dropdown-item", function (e) {
            ...
            // Notify form view
            awkhad_top[window.callback+"_downup"]($editable_area.addClass("o_dirty").html());
        });
...
awkhad_top[callback+"_updown"] = function (value, fields_values, field_name) {
    if (!window || window.closed) {
        delete awkhad_top[callback+"_updown"];
        return;
    }
    ...
    if (fields_values.mailing_model && editor_enable) {
        if (value.indexOf('on_change_model_and_list') !== -1) {
            awkhad_top[callback+"_downup"](_val);
        }
    }
};
