awkhad.define('web_editor.backend', function (require) {
'use strict';
...
var FieldTextHtmlSimple = basic_fields.DebouncedField.extend(TranslatableFieldMixin, {
     ...
     *
     * @private
     * @returns {Array} "ir.attachment" awkhad domain.
     */
...
var FieldTextHtml = AbstractField.extend({
    ...
    start: function () {
        ...
        window.awkhad[this.callback+"_editor"] = function (EditorBar) {
            setTimeout(function () {
                self.on_editor_loaded(EditorBar);
            },0);
        };
        window.awkhad[this.callback+"_content"] = function () {
            self.on_content_loaded();
        };
        window.awkhad[this.callback+"_updown"] = null;
        window.awkhad[this.callback+"_downup"] = function () {
            self.resize();
        };
    ...
    on_editor_loaded: function (EditorBar) {
        ...
        if (this.value && window.awkhad[self.callback+"_updown"] && !(this.$content.html()||"").length) {
            this.render();
        }
    ...
    render: function () {
        ...
        if (this.mode === "edit") {
            if (window.awkhad[this.callback+"_updown"]) {
                // FIXME
                // window.awkhad[this.callback+"_updown"](value, this.view.get_fields_values(), this.name);
                this.resize();
            }
        } else {
    ...
    destroy: function () {
        ...
        delete window.awkhad[this.callback+"_editor"];
        delete window.awkhad[this.callback+"_content"];
        delete window.awkhad[this.callback+"_updown"];
        delete window.awkhad[this.callback+"_downup"];
    },
