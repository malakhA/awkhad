...
from awkhad import fields as awkhad_fields, tools, _
from awkhad.exceptions import ValidationError, AccessError, MissingError, UserError
from awkhad.http import content_disposition, Controller, request, route
from awkhad.tools import consteq
...
class CustomerPortal(Controller):
    ...
    def _get_archive_groups(self, model, domain=None, fields=None, groupby="create_date", order="create_date desc"):
        ...
        for group in request.env[model]._read_group_raw(domain, fields=fields, groupby=groupby, orderby=order):
            ...
            groups.append({
                'date_begin': awkhad_fields.Date.to_string(awkhad_fields.Date.from_string(date_begin)),
                'date_end': awkhad_fields.Date.to_string(awkhad_fields.Date.from_string(date_end)),
                'name': label,
