awkhad.define('sms.sms_widget_tests', function (require) {
"use strict";
...
QUnit.module('fields', {
    ...
    QUnit.test('Sms widgets are correctly rendered', function (assert) {
        ...
        // GSM-7
        form.$('.o_input').val("Hello from Awkhad").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 chars, fits in 1 SMS (GSM7) ', 'Should be "15 chars, fits in 1 SMS (GSM7) " for "Hello from Awkhad"');
        // GSM-7 with \n => this one count as 2 characters
        form.$('.o_input').val("Hello from Awkhad\n").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '17 chars, fits in 1 SMS (GSM7) ', 'Should be "17 chars, fits in 1 SMS (GSM7) " for "Hello from Awkhad\\n"');
        // Unicode => ê
        form.$('.o_input').val("Hêllo from Awkhad").trigger('input');
        assert.strictEqual(form.$('.o_sms_count').text(), '15 chars, fits in 1 SMS (UNICODE) ', 'Should be "15 chars, fits in 1 SMS (UNICODE) " for "Hêllo from Awkhad"');
        // GSM-7 with 160c
