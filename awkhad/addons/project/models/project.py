...
from awkhad.exceptions import UserError, AccessError, ValidationError
...
class Task(models.Model):
    ...
    def _notify_specific_email_values(self, message):
        ...
        try:
            ...
            headers = {}
        if self.project_id:
            current_objects = [h for h in headers.get('X-Awkhad-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Awkhad-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Awkhad-Tags'] = ','.join(self.tag_ids.mapped('name'))
