awkhad.define('web_settings_dashboard.settings_dashboard_tests', function (require) {
"use strict";
...
QUnit.module('settings_dashboard', function () {
    QUnit.test('Dashboard: Invite new user', function (assert) {
        ...
        // add email to invite
        dashboard.$('.o_user_emails').val('lagan@awkhad.com').trigger($.Event('keydown', {
            which: $.ui.keyCode.ENTER,
        }));
        assert.strictEqual(dashboard.$('.o_badge_text').text().trim(), 'lagan@awkhad.com',
        ...
        assert.strictEqual(dashboard.$('.o_web_settings_dashboard_user').text().trim(), 'lagan@awkhad.com',
    ...
    QUnit.test('Dashboard: Invite new user (warnings)', function (assert) {
                ...
                if (route === '/web_settings_dashboard/data') {
                    return $.when({
                        ...
                        users_info: {
                            active_users: 4,
                            pending_counts: 1,
                            pending_users: [[3, 'xyz@awkhad.com']],
                        },
                    });
                }
                return this._super.apply(this, arguments);
        ...
        dashboard.$('.o_user_emails').val('xyz@awkhad.com').trigger($.Event('keydown', {
            which: $.ui.keyCode.ENTER,
        }));
        assert.strictEqual(dashboard.$('.o_badge_text').length, 0,
            'should not have generated any badge');
        assert.strictEqual(dashboard.$('.o_user_emails').val(), 'xyz@awkhad.com',
            'input should not have been cleared');
    ...
    QUnit.test('Dashboard: Invite a list of users', function (assert) {
        ...
        var emails = ['a@awkhad.com', 'b@awkhad.com', 'c@awkhad.com', 'd@awkhad.com'];
    ...
    QUnit.test('Dashboard: Invite a list of users (with warnings)', function (assert) {
                ...
                if (route === '/web_settings_dashboard/data') {
                    return $.when({
                        ...
                        users_info: {
                            active_users: 4,
                            pending_counts: 1,
                            pending_users: [[4, 'a@awkhad.com'], [5, 'd@awkhad.com']],
                        },
                    });
                }
                return this._super.apply(this, arguments);
        ...
        // simulate a copy paste of multiple email addresses
        var emails = ['a@awkhad.com', 'b@awkhad.com', 'x@y', 'd@awkhad.com'];
        dashboard.$('.o_user_emails').val(emails.join(' ')).trigger($.Event('keydown', {
            which: $.ui.keyCode.ENTER,
        }));
