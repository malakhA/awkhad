...
from awkhad.addons.payment.models.payment_acquirer import ValidationError
from awkhad.addons.payment_ogone.controllers.main import OgoneController
from awkhad.addons.payment_ogone.data import ogone
from awkhad.http import request
from awkhad.tools import DEFAULT_SERVER_DATE_FORMAT, ustr
from awkhad.tools.float_utils import float_compare, float_repr, float_round
...
class PaymentAcquirerOgone(models.Model):
    ...
    def _ogone_generate_shasign(self, inout, values):
        ...
        :param string inout: 'in' (awkhad contacting ogone) or 'out' (ogone
                             contacting awkhad). In this last case only some
    ...
    def ogone_form_generate_values(self, values):
        ...
        if self.save_token in ['ask', 'always']:
            temp_ogone_tx_values.update({
                'ALIAS': 'AWKHAD-NEW-ALIAS-%s' % time.time(),    # something unique,
...
class PaymentTxOgone(models.Model):
    ...
    def ogone_s2s_do_transaction(self, **kwargs):
        ...
        reference = self.reference or "AWKHAD-%s-%s" % (datetime.datetime.now().strftime('%y%m%d_%H%M%S'), self.partner_id.id)
    ...
    def _ogone_s2s_get_tx_status(self):
        account = self.acquirer_id
        #reference = tx.reference or "AWKHAD-%s-%s" % (datetime.datetime.now().strftime('%Y%m%d_%H%M%S'), tx.partner_id.id)
...
class PaymentToken(models.Model):
    ...
    def ogone_create(self, values):
        if values.get('cc_number'):
            ...
            alias = 'AWKHAD-NEW-ALIAS-%s' % time.time()
