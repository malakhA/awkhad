...
import awkhad
from awkhad import api, fields, models, SUPERUSER_ID
...
class ImBus(models.Model):
    ...
    def sendmany(self, notifications):
        ...
        if channels:
            ...
            def notify():
                with awkhad.sql_db.db_connect('postgres').cursor() as cr:
...
class ImDispatch(object):
    ...
    def poll(self, dbname, channels, last, options=None, timeout=TIMEOUT):
        ...
        if not awkhad.evented:
            current = threading.current_thread()
            current._Thread__daemonic = True # PY2
            current._daemonic = True         # PY3
            # rename the thread to avoid tests waiting for a longpolling
            current.setName("zgui.longpolling.request.%s" % current.ident)
...
dispatch = None
if not awkhad.multi_process or awkhad.evented:
