...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, record, values, command=None):
        ...
        The logic will only be applied if awkhadbot is in a chat with a user or
        if someone pinged awkhadbot.

         :param record: the mail_thread (or mail_channel) where the user
            message was posted/awkhadbot will answer.
        ...
        if len(record) != 1 or values.get("author_id") == awkhadbot_id:
            return
        if self._is_bot_pinged(values) or self._is_bot_in_private_channel(record):
            ...
            if answer:
                ...
                record.with_context(mail_create_nosubscribe=True).sudo().message_post(body=answer, author_id=awkhadbot_id, message_type=message_type, subtype_id=subtype_id)
    ...
    def _get_answer(self, record, body, values, command=False):
        ...
        if self._is_bot_in_private_channel(record):
            # main flow
            if awkhadbot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.awkhadbot_state = "onboarding_attachement"
                return _("Great! 👍<br/>Now, try to <b>send an attachment</b>, like a picture of your cute dog...")
            elif awkhadbot_state == 'onboarding_attachement' and values.get("attachment_ids"):
                self.env.user.awkhadbot_state = "onboarding_command"
                return _("Not a cute dog, but you get it 😊<br/>To access special features, <b>start your sentence with '/'</b>. Try to get help.")
            elif awkhadbot_state == 'onboarding_command' and command == 'help':
                self.env.user.awkhadbot_state = "onboarding_ping"
                return _("Wow you are a natural!<br/>Ping someone to grab its attention with @nameoftheuser. <b>Try to ping me using @AwkhadBot</b> in a sentence.")
            elif awkhadbot_state == 'onboarding_ping' and self._is_bot_pinged(values):
                self.env.user.awkhadbot_state = "idle"
                return _("Yep, I am here! 🎉 <br/>You finished the tour, you can <b>close this chat window</b>. Enjoy discovering Awkhad.")
            elif awkhadbot_state == "idle" and (_('start the tour') in body.lower()):
                self.env.user.awkhadbot_state = "onboarding_emoji"
                return _("To start, try to send me an emoji :)")
            # easter eggs
            elif awkhadbot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            elif awkhadbot_state == "idle" and (('help' in body) or _('help') in body):
                return _("I'm just a bot... :( You can check <a href=\"https://www.awkhad.com/page/docs\">our documentation</a>) for more information!")
            elif _('fuck') in body or "fuck" in body:
                return _("That's not nice! I'm a bot but I have feelings... 💔")
            else:
                #repeat question
                if awkhadbot_state == 'onboarding_emoji':
                    return _("Not exactly. To continue the tour, send an emoji, <b>type \":)\"</b> and press enter.")
                elif awkhadbot_state == 'onboarding_attachement':
                    return _("To <b>send an attachment</b>, click the 📎 icon on the right, and select a file.")
                elif awkhadbot_state == 'onboarding_command':
                    return _("Not sure wat you are doing. Please press / and wait for the propositions. Select \"help\" and press enter")
                elif awkhadbot_state == 'onboarding_ping':
                    return _("Sorry, I am not listening. To get someone's attention, <b>ping him</b>. Write \"@awkhadbot\" and select me.")
            ...
            return random.choice([_("Yep, AwkhadBot is in the place!"), _("Pong.")])
        return False
    ...
    def _is_bot_pinged(self, values):
        awkhadbot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        return (4, awkhadbot_id) in values.get('partner_ids', [])

    def _is_bot_in_private_channel(self, record):
        awkhadbot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        if record._name == 'mail.channel' and record.channel_type == 'chat':
            return awkhadbot_id in record.with_context(active_test=False).channel_partner_ids.ids
        return False
