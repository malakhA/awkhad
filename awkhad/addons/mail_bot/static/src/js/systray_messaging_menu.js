awkhad.define('mail_bot.systray.MessagingMenu', function (require) {
"use strict";
...
return MessagingMenu.include({
     ...
     * messaging menu (e.g. 'AwkhadBot has a request')
     *
     * @override
     * @private
     ...
     * AwkhadBot, and the AwkhadBot influences the counter by 1 when it requests
    ...
    _handleResponseNotificationPermission: function (value) {
        ...
        if (value !== 'granted') {
            this.call('bus_service', 'sendNotification', _t('Permission denied'),
                _t('Awkhad will not have the permission to send native notifications on this device.'));
        } else {
            this.call('bus_service', 'sendNotification', _t('Yay, push notifications are enabled!'),
                _t('Awkhad has now the permission to send you native notifications on this device.'));
        }
        this._updateCounter();
    },
    /**
    ...
    /**
     * Override so that it handles preview related to AwkhadBot
     *
     * @override
     * @private
     * @param {MouseEvent} ev
