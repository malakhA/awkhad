awkhad.define('mail_bot.MailBotService', function (require) {
"use strict";
...
var MailBotService =  AbstractService.extend({
    ...
    start: function () {
        ...
        if ('awkhadbot_initialized' in session && ! session.awkhadbot_initialized) {
            this._showAwkhadbotTimeout();
        }
    },
    ...
    /**
     * Get the previews related to the AwkhadBot (conversation not included).
     * For instance, when there is no conversation with AwkhadBot and AwkhadBot has
    ...
    getPreviews: function (filter) {
        ...
        if (this.hasRequest() && (filter === 'mailbox_inbox' || !filter)) {
            previews.push({
                title: _t("AwkhadBot has a request"),
                imageSRC: "/mail/static/src/img/awkhadbot.png",
                status: 'bot',
    ...
    /**
     * Tell whether AwkhadBot has a request or not.
     *
     * @returns {boolean}
     */
    hasRequest: function () {
     ...
     * @private
     */
    _showAwkhadbotTimeout: function () {
        var self = this;
        setTimeout(function () {
            session.awkhadbot_initialized = true;
            self._rpc({
                model: 'mail.channel',
                method: 'init_awkhadbot',
            });
        }, 2*60*1000);
    },
});
