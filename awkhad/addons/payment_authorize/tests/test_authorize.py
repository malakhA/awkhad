...
from awkhad.addons.payment.tests.common import PaymentAcquirerCommon
from awkhad.addons.payment_authorize.controllers.main import AuthorizeController
...
@awkhad.tests.tagged('post_install', '-at_install', '-standard', 'external')
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('awkhad.addons.payment_authorize.models.payment', 'ValidationError')
...
@awkhad.tests.tagged('post_install', '-at_install', '-standard')
class AuthorizeForm(AuthorizeCommon):
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on awkhad/awkhad + create sandbox account
        authorize.write({
