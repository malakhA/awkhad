...
from awkhad.addons.payment.models.payment_acquirer import _partner_split_name
from awkhad.exceptions import ValidationError, UserError
...
class AuthorizeAPI():
    ...
    def create_customer_profile(self, partner, cardnumber, expiration_date, card_code):
        ...
        a corresponding payment profile to it. Note that a single partner in the Awkhad
        ...
        # merchantCustomerId is AWKHAD-{partner.id}-{random hex string} truncated to maximum 20 characters
        etree.SubElement(profile, "merchantCustomerId").text = ('AWKHAD-%s-%s' % (partner.id, uuid4().hex[:8]))[:20]
    ...
    def create_customer_profile_from_tx(self, partner, transaction_id):
        ...
        # merchantCustomerId is AWKHAD-{partner.id}-{random hex string} truncated to maximum 20 characters
        etree.SubElement(customer, "merchantCustomerId").text = ('AWKHAD-%s-%s' % (partner.id, uuid4().hex[:8]))[:20]
