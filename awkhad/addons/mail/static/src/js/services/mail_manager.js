awkhad.define('mail.Manager', function (require) {
"use strict";
...
var MailManager =  AbstractService.extend({
    ...
    _AWKHADBOT_ID: "AWKHADBOT", // default authorID for transient messages
    ...
    /**
     * Get the AwkhadBot ID, which is the default authorID for transient messages
     *
     * @returns {string}
     */
    getAwkhadbotID: function () {
        return this._AWKHADBOT_ID;
    },
    /**
            ...
            if (thread) {
                if (
                    ...
                    if (thread.isTwoUserThread() && options.showNotification) {
                        ...
                        if (!self.call('bus_service', 'isAwkhadFocused') || !query.isVisible) {
    ...
    _initializeInternalState: function () {
        ...
        // # of message received when awkhad is out of focus
    ...
    _notifyIncomingMessage: function (message) {
        if (this.call('bus_service', 'isAwkhadFocused')) {
            // no need to notify
            return;
        }
