...
import awkhad.tests
import time
from awkhad.addons.account.tests.test_reconciliation import TestReconciliation
...
class TestUi(awkhad.tests.HttpCase):
    ...
    def test_01_admin_bank_statement_reconciliation(self):
        ...
        self.phantom_js("/web#statement_ids=" + str(bank_stmt.id) + "&action=bank_statement_reconciliation_view",
            "awkhad.__DEBUG__.services['web_tour.tour'].run('bank_statement_reconciliation')",
            "awkhad.__DEBUG__.services['web_tour.tour'].tours.bank_statement_reconciliation.ready", login="admin")
