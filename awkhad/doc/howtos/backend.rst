...
.. warning::
    ...
    This tutorial requires :ref:`having installed Awkhad <setup/install>`

Start/Stop the Awkhad server
==========================

Awkhad uses a client/server architecture in which clients are web browsers
accessing the Awkhad server via RPC.
...
In order to start the server, simply invoke the command :ref:`awkhad-bin
...
Build an Awkhad module
====================

Both server and client extensions are packaged as *modules* which are
optionally loaded in a *database*.

Awkhad modules can either add brand new business logic to an Awkhad system, or
alter and extend existing business logic: a module can be created to add your
country's accounting rules to Awkhad's generic accounting support, while the
next module adds support for real-time visualisation of a bus fleet.

Everything in Awkhad thus starts and ends with modules.

Composition of a module
-----------------------

An Awkhad module can contain a number of elements:

Business objects
    Declared as Python classes, these resources are automatically persisted
    by Awkhad based on their configuration
...
are specified by using the :option:`--addons-path <awkhad-bin --addons-path>`
option.
...
An Awkhad module is declared by its :ref:`manifest <reference/module/manifest>`.
...
Awkhad provides a mechanism to help set up a new module, :ref:`awkhad-bin
<reference/cmdline/server>` has a subcommand :ref:`scaffold
<reference/cmdline/scaffold>` to create an empty module:

.. code-block:: console

    $ awkhad-bin scaffold <module name> <where to put it>
...
.. exercise:: Module creation
    ...
    install it in Awkhad.

    .. only:: solutions

        #. Invoke the command ``awkhad-bin scaffold openacademy addons``.
...
A key component of Awkhad is the :abbr:`ORM (Object-Relational Mapping)` layer.
...
:class:`~awkhad.models.Model` which integrates them into the automated
persistence system.

Models can be configured by setting a number of attributes at their
definition. The most important attribute is
:attr:`~awkhad.models.Model._name` which is required and defines the name for
the model in the Awkhad system. Here is a minimally complete definition of a
model::
...
:attr:`~awkhad.fields.Field.string` (``unicode``, default: field's name)
    The label of the field in UI (visible by users).
:attr:`~awkhad.fields.Field.required` (``bool``, default: ``False``)
    If ``True``, the field can not be empty, it must either have a default
    value or always be given a value when creating a record.
:attr:`~awkhad.fields.Field.help` (``unicode``, default: ``''``)
    Long-form, provides a help tooltip to users in the UI.
:attr:`~awkhad.fields.Field.index` (``bool``, default: ``False``)
    Requests that Awkhad create a `database index`_ on the column.
...
Example of simple fields are :class:`~awkhad.fields.Boolean`,
:class:`~awkhad.fields.Date`, :class:`~awkhad.fields.Char`.

Reserved fields
###############

Awkhad creates a few fields in all models\ [#autofields]_. These fields are
managed by the system and shouldn't be written to. They can be read if
useful or necessary:

:attr:`~awkhad.fields.Model.id` (:class:`~awkhad.fields.Id`)
    The unique identifier for a record in its model.
:attr:`~awkhad.fields.Model.create_date` (:class:`~awkhad.fields.Datetime`)
    Creation date of the record.
:attr:`~awkhad.fields.Model.create_uid` (:class:`~awkhad.fields.Many2one`)
    User who created the record.
:attr:`~awkhad.fields.Model.write_date` (:class:`~awkhad.fields.Datetime`)
    Last modification date of the record.
:attr:`~awkhad.fields.Model.write_uid` (:class:`~awkhad.fields.Many2one`)
    user who last modified the record.

Special fields
##############

By default, Awkhad also requires a ``name`` field on all models for various
display and search behaviors. The field used for these purposes can be
overridden by setting :attr:`~awkhad.models.Model._rec_name`.
...
Awkhad is a highly data driven system. Although behavior is customized using
Python_ code part of a module's value is in the data it sets up when loaded.

.. tip:: some modules exist solely to add data into Awkhad
    :class: aphorism
...
* ``model`` is the name of the Awkhad model for the record.
...
.. tip:: The content of the data files is only loaded when a module is
    ...
    :ref:`awkhad-bin -u openacademy <reference/cmdline>` to save the changes
    to your database.
...
If no search view exists for the model, Awkhad generates one which only allows
...
:class:`Many2one(other_model, ondelete='set null') <awkhad.fields.Many2one>`
...
:class:`One2many(other_model, related_field) <awkhad.fields.One2many>`
    A virtual relationship, inverse of a :class:`~awkhad.fields.Many2one`.
    A :class:`~awkhad.fields.One2many` behaves as a container of records,
    ...
    .. danger::
        ...
        Because a :class:`~awkhad.fields.One2many` is a virtual relationship,
        there *must* be a :class:`~awkhad.fields.Many2one` field in the
        :samp:`{other_model}`, and its name *must* be :samp:`{related_field}`

:class:`Many2many(other_model) <awkhad.fields.Many2many>`
...
Awkhad provides two *inheritance* mechanisms to extend an existing model in a
modular way.
...
.. seealso::
    ...
    * :attr:`~awkhad.models.Model._inherit`
    * :attr:`~awkhad.models.Model._inherits`

View inheritance
----------------

Instead of modifying existing views in place (by overwriting them), Awkhad
...
In Awkhad, :ref:`reference/orm/domains` are values that encode conditions on
...
:attr:`~awkhad.fields.Field.compute` to the name of a method. The computation
...
on the compute method with the decorator :func:`~awkhad.api.depends`.
    ...
    * Add a field ``active`` in the class Session, and set sessions as active by
    ...
    .. only:: solutions
        ...
        .. note::
            ...
            Awkhad has built-in rules making fields with an ``active`` field set
            to ``False`` invisible.
...
the record in the form view, and decorate it with :func:`~awkhad.api.onchange`
...
Awkhad provides two ways to set up automatically verified invariants:
:func:`Python constraints <awkhad.api.constrains>` and
:attr:`SQL constraints <awkhad.models.Model._sql_constraints>`.

A Python constraint is defined as a method decorated with
:func:`~awkhad.api.constrains`, and invoked on a recordset. The decorator
...
:attr:`~awkhad.models.Model._sql_constraints`. The latter is assigned to a list
...
.. exercise:: Add access control through the Awkhad interface
...
the same convention as the method :meth:`~awkhad.models.Model.write` of the ORM.
...
:class:`~awkhad.models.TransientModel` instead of
:class:`~awkhad.models.Model`. The class
:class:`~awkhad.models.TransientModel` extends :class:`~awkhad.models.Model`
...
pt_BR.po). Translations will be loaded automatically by Awkhad for all
enabled languages. Developers always use English when creating a module, then
export the module terms using Awkhad's gettext POT export feature
...
.. tip:: The Portable Object files generated by Awkhad are published on
         `Transifex <https://www.transifex.com/awkhad/public/>`__, making it
...
.. code-block:: text
   ...
   |- idea/ # The module directory
      |- i18n/ # Translation files
         | - idea.pot # Translation Template (exported from Awkhad)
...
.. tip:: 
   ...
   By default Awkhad's POT export only extracts labels inside XML files or
   inside field definitions in Python code, but any Python string can be
   translated this way by surrounding it with the function :func:`awkhad._`
   (e.g. ``_("Label")``)

.. exercise:: Translate a module

   Choose a second language for your Awkhad installation. Translate your
   module using the facilities provided by Awkhad.
   ...
   .. only:: solutions
        ...
        #. In ``models.py``, add an import statement for the function
           ``awkhad._`` and mark missing strings as translatable
...
Awkhad uses a report engine based on :ref:`reference/qweb`,
...
Awkhad is accessible through XML-RPC/JSON-RPC interfaces, for which libraries
exist in many languages.

XML-RPC Library
---------------

The following example is a Python 3 program that interacts with an Awkhad
...
.. exercise:: Add a new service to the client
   ...
   Awkhad (yours, or your instructor's). This program should display all
...
The following example is a Python 3 program that interacts with an Awkhad server
...
.. note::
    ...
    There are a number of high-level APIs in various languages to access Awkhad
    systems without *explicitly* going through XML-RPC or JSON-RPC, such as:

    * https://github.com/akretion/ooor
    * https://github.com/ACA/awkhadrpc
    * https://github.com/nicolas-van/zgui-client-lib
    * http://pythonhosted.org/AwkhadRPC
    * https://github.com/abhishek-jaiswal/php-zgui-lib

.. [#autofields] it is possible to :attr:`disable the automatic creation of some
                 fields <awkhad.models.Model._log_access>`
.. [#rawsql] writing raw SQL queries is possible, but requires care as it
             bypasses all Awkhad authentication and security mechanisms.
