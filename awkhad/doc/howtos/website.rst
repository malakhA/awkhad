...
.. warning::
    ...
    * This guide assumes :ref:`an installed Awkhad <setup/install>`

Creating a basic module
=======================

In Awkhad, tasks are performed by creating modules.

Modules customize the behavior of an Awkhad installation, either by adding new
behaviors or by altering existing ones (including behaviors added by other
modules).

:ref:`Awkhad's scaffolding <reference/cmdline/scaffold>` can setup a basic
module. To quickly get started simply invoke:

.. code-block:: console

    $ ./awkhad-bin scaffold Academy my-modules
...
* start the Awkhad server

  .. code-block:: console

      $ ./awkhad-bin --addons-path addons,my-modules
...
display logic. Awkhad allows any Python templating system, but provides its
...
Finally restart Awkhad and update the module's data (to install the template)
by going to :menuselection:`Settings --> Modules --> Modules -->
Academy` and clicking :guilabel:`Upgrade`.

.. tip::

    Alternatively, Awkhad can be restarted :option:`and update modules at
    the same time<awkhad-bin -u>`:

    .. code-block:: console

        $ awkhad-bin --addons-path addons,my-modules -d academy -u academy

Going to http://localhost:8069/academy/academy/ should now result in:

.. image:: website/basic-list.png

Storing data in Awkhad
====================

:ref:`Awkhad models <reference/orm/model>` map to database tables.
...
#. Because :meth:`~awkhad.models.Model.search` returns a set of records
...
Awkhad bundles a module dedicated to building websites.

So far we've used controllers fairly directly, but Awkhad 8 added deeper
...
You're now in Awkhad "proper": the administrative interface. For now click on
...
:func:`~awkhad.http.route` decorator which takes a routing string and a
...
restart Awkhad, access http://localhost:8069/academy/Alice/ and
...
Restart Awkhad, access http://localhost:8069/academy/2, note how the old value
...
Awkhad provides an additional converter called ``model`` which provides records
...
Restart Awkhad and upgrade the module, then you can visit each teacher's page.
...
Restart Awkhad and update the views, reload the teacher's page and… the field
...
Restart Awkhad and upgrade the module, there is now a placeholder under the
...
A brief and incomplete introduction to the Awkhad administration
--------------------------------------------------------------

The Awkhad administration was briefly seen during the `website support`_ section.
We can go back to it using :menuselection:`Administrator --> Administrator` in
the menu (or :guilabel:`Sign In` if you're signed out).

The conceptual structure of the Awkhad backend is simple:

#. first are menus, a tree (menus can have sub-menus) of records. Menus
   without children map to…
#. actions. Actions have various types: links, reports, code which Awkhad should
   execute or data display. Data display actions are called *window actions*,
   and tell Awkhad to display a given *model* according to a set of views…
...
Editing in the Awkhad administration
----------------------------------

By default, an Awkhad model is essentially invisible to a user. To make it
...
:ref:`view <reference/views>`) Awkhad will automatically create a basic one
...
:class:`the inverse relationship <awkhad.fields.One2many>` to the *teachers*
model:
...
Awkhad provides technical models, which don't directly fulfill business needs
but which add capabilities to business objects without having to build
them by hand.

One of these is the *Chatter* system, part of Awkhad's email and messaging
system, which can add notifications and discussion threads to any model.
The model simply has to :attr:`~awkhad.models.Model._inherit`
...
Awkhad also provides business models which allow using or opting in business
needs more directly. For instance the ``website_sale`` module sets up an
e-commerce site based on the products in the Awkhad system. We can easily make
...
restart Awkhad, update your module, there is now a :guilabel:`Shop` section in
...
.. note::
    ...
    * to extend a model in-place, it's :attr:`inherited
      <awkhad.models.Model._inherit>` without giving it a new
      :attr:`~awkhad.models.Model._name`
