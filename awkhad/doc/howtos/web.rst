...
This guide is about creating modules for Awkhad's web client.

To create websites with Awkhad, see :doc:`website`; to add business capabilities
or extend existing business systems of Awkhad, see :doc:`backend`.
...
.. warning::
    ...
    * `Underscore.js`_
    ...
    It also requires :ref:`an installed Awkhad <setup/install>`, and Git_.


A Simple Module
===============

Let's start with a simple Awkhad module holding basic web component
...
.. code-block:: console
    ...
    $ git clone http://github.com/awkhad/petstore

This will create a ``petstore`` folder wherever you executed the command.
You then need to add that folder to Awkhad's
:option:`addons path <awkhad-bin --addons-path>`, create a new database and
...
Files used in the "web" side of an Awkhad module must be placed in a ``static``
...
``oepetstore/static/js/petstore.js``
    ...
    currently look like::
        ...
        awkhad.oepetstore = function(instance, local) {
...
.. warning::
    ...
    "developer mode": log into your Awkhad instance (user *admin* password
    *admin* by default) open the user menu (in the top-right corner of the
    Awkhad screen) and select :guilabel:`About Awkhad` then :guilabel:`Activate
    the developer mode`:

    .. image:: web/about_awkhad.png
        :align: center
...
The Awkhad framework uses one such pattern to define modules within web addons,
...
In Awkhad web, modules are declared as functions set on the global ``awkhad``
...
* the first parameter is the current instance of the Awkhad web client, it gives
  access to various capabilities defined by the Awkhad (translations,
...
* the second parameter is your own local namespace automatically created by
  ...
  outside your module (either because the Awkhad web client needs to call them
...
For simplicity and developer-friendliness Awkhad web provides a class
system based on John Resig's `Simple JavaScript Inheritance`_.

New classes are defined by calling the :func:`~awkhad.web.Class.extend`
method of :class:`awkhad.web.Class`::
...
The :func:`~awkhad.web.Class.extend` method takes a dictionary describing
...
by calling :func:`~awkhad.web.Class.extend` on the parent class, as is done
to subclass :class:`~awkhad.web.Class`::
...
The Awkhad web client bundles jQuery_ for easy DOM manipulation. It is useful
...
Awkhad Web makes specific components responsible for sections of a page. In
Awkhad web, the base for such components is the :class:`~awkhad.Widget`
...
It extends :class:`~awkhad.Widget` and overrides the standard method
:func:`~awkhad.Widget.start`, which — much like the previous ``MyClass``
— does little for now.
...
The ``HomePage`` widget already has a :func:`~awkhad.Widget.start`
...
All widgets have a :attr:`~awkhad.Widget.$el` which represents the
section of page they're in charge of (as a jQuery_ object). Widget content
should be inserted there. By default, :attr:`~awkhad.Widget.$el` is an
empty ``<div>`` element.
...
Let's add some content to the widget's root element, using jQuery::
    ...
    local.HomePage = instance.Widget.extend({
        start: function() {
            this.$el.append("<div>Hello dear Awkhad user!</div>");
        },
    });
...
.. note::
    ...
    to refresh the javascript code loaded in Awkhad Web, you will need to reload
    the page. There is no need to restart the Awkhad server.

The ``HomePage`` widget is used by Awkhad Web and managed automatically.
...
``GreetingsWidget``'s :func:`~awkhad.Widget.appendTo` method::
...
* Finally it tells ``GreetingsWidget`` where to insert itself, delegating part
  of its :attr:`~awkhad.Widget.$el` to the ``GreetingsWidget``.

When the :func:`~awkhad.Widget.appendTo` method is called, it asks the
widget to insert itself at the specified position and to display its content.
The :func:`~awkhad.Widget.start` method will be called during the call
to :func:`~awkhad.Widget.appendTo`.
...
.. code-block:: html
    ...
    <div class="oe_petstore_homepage">
        <div>Hello dear Awkhad user!</div>
...
:class:`~awkhad.Widget`, because we added some classes on them.
...
:func:`~awkhad.Widget.getChildren`
...
When overriding the :func:`~awkhad.Widget.init` method of a widget it is
...
it. This is done via the :func:`~awkhad.Widget.destroy` method::

    greeting.destroy();

When a widget is destroyed it will first call
:func:`~awkhad.Widget.destroy` on all its children. Then it erases itself
from the DOM. If you have set up permanent structures in
:func:`~awkhad.Widget.init` or :func:`~awkhad.Widget.start` which
must be explicitly cleaned up (because the garbage collector will not handle
them), you can override :func:`~awkhad.Widget.destroy`.

.. danger::

    when overriding :func:`~awkhad.Widget.destroy`, ``_super()``
...
As many other environments, Awkhad's solution is to use a `template engine`_.
Awkhad's template engine is called :ref:`reference/qweb`.
...
* It has special support in Awkhad Web's :class:`~awkhad.Widget`, though it
  can be used outside of Awkhad's web client (and it's possible to use
  :class:`~awkhad.Widget` without relying on QWeb)
...
.. note::
    ...
    like Awkhad :ref:`views <reference/views>`.
    ...
    characteristic of Awkhad, and losing it was considered unacceptable.
...
However, because :class:`~awkhad.Widget` has special integration for QWeb
the template can be set directly on the widget via its
:attr:`~awkhad.Widget.template` attribute::
...
* with the second version, the template is rendered right before
  :func:`~awkhad.Widget.start` is called
...
.. warning::
    ...
    they're set as a widget's :attr:`~awkhad.Widget.template`. If there are
...
When using :class:`~awkhad.Widget`'s integration it is not possible to
...
before :func:`~awkhad.Widget.start` is called (the widget's state will
essentially be that set up by :func:`~awkhad.Widget.init`):
...
But because it's a common operation, :class:`~awkhad.Widget` provides an
equivalent shortcut through the :func:`~awkhad.Widget.$` method::
...
:attr:`~awkhad.Widget.events` is an object (mapping) of an event to the
...
:func:`~awkhad.Widget.trigger` takes the name of the event to trigger as
...
listening to the ``user_chose`` event using :func:`~awkhad.Widget.on`::
...
:func:`~awkhad.Widget.on` binds a function to be called when the
...
additional arguments of :func:`~awkhad.Widget.trigger` if it has
any. Example::
...
.. note::
    ...
    exception to that rule is ``awkhad.web.bus`` which exists specifically
    to broadcasts evens in which any widget could be interested throughout
    the Awkhad web application.
...
* :func:`~awkhad.Widget.set` sets the value of a property and triggers
  :samp:`change:{propname}` (where *propname* is the property name passed as
  first parameter to :func:`~awkhad.Widget.set`) and ``change``
* :func:`~awkhad.Widget.get` retrieves the value of a property.
...
The class system of the Awkhad web framework allows direct modification of
existing classes using the :func:`~awkhad.web.Class.include` method::
...
defined in the call to :func:`~awkhad.web.Class.include`. This will also work
if some instances of the class (or of any of its sub-classes) were created
prior to the call to :func:`~awkhad.Widget.include`.
...
In Awkhad, translations files are automatically generated by scanning the source
...
:func:`~awkhad.web._t` (and also :func:`~awkhad.web._lt`).
...
:func:`~awkhad.web._lt` ("lazy translate") is similar but somewhat more
...
Communication with the Awkhad Server
==================================

Contacting Models
-----------------

Most operations with Awkhad involve communicating with *models* implementing
...
communicating with Awkhad requires additional metadata whose setup before every
call would be verbose and error-prone. As a result, Awkhad web provides
...
The class used to call Awkhad models is :class:`awkhad.Model`. It is
instantiated with the Awkhad model's name as first parameter
(``oepetstore.message_of_the_day`` here).

:func:`~awkhad.web.Model.call` can be used to call any (public) method of an
Awkhad model. It takes the following positional arguments:
...
:func:`~awkhad.Widget.call` returns a deferred resolved with the value
...
the time zone of the user, used to compute correctly dates and times if Awkhad
...
solution to achieve that is to use :class:`awkhad.web.CompoundContext`.

:class:`~awkhad.web.CompoundContext` is a class used to pass the user's
...
dictionaries or other :class:`~awkhad.web.CompoundContext` instances. It will
...
are related to the configuration of the current user in Awkhad plus the
``new_key`` key that was added when instantiating
:class:`~awkhad.web.CompoundContext`.

Queries
-------

While :func:`~awkhad.Model.call` is sufficient for any interaction with Awkhad
models, Awkhad Web provides a helper for simpler and clearer querying of models
(fetching of records based on various conditions):
:func:`~awkhad.Model.query` which acts as a shortcut for the common
combination of :py:meth:`~awkhad.models.Model.search` and
::py:meth:`~awkhad.models.Model.read`. It provides a clearer syntax to search
and read models::
...
* :func:`~awkhad.web.Model.query` takes an optional list of fields as
  parameter (if no field is provided, all fields of the model are fetched). It
  returns a :class:`awkhad.web.Query` which can be further customized before
  being executed
* :class:`~awkhad.web.Query` represents the query being built. It is
  ...
  :class:`~awkhad.web.Query` for its customization options.

When the query is set up as desired, simply call
:func:`~awkhad.web.Query.all` to execute it and return a
deferred to its result. The result is the same as
:py:meth:`~awkhad.models.Model.read`'s, an array of dictionaries where each
...
.. exercise:: Pet Toys List
    ...
    In Awkhad, images are generally stored in regular fields encoded as
...
In Awkhad, many operations start from an :ref:`action <reference/actions>`:
...
In Awkhad Web, the component responsible for handling and reacting to these
...
:func:`~awkhad.Widget.do_action` is a shortcut of :class:`~awkhad.Widget`
...
the Awkhad web know to start this widget? Because the widget is registered as
a *client action*.

A client action is (as its name implies) an action type defined almost
entirely in the client, in javascript for Awkhad web. The server simply sends
...
``instance.web.client_actions`` is a :class:`~awkhad.web.Registry` in which
the action manager looks up client action handlers when it needs to execute
one. The first parameter of :class:`~awkhad.web.Registry.add` is the name
(tag) of the client action, and the second parameter is the path to the widget
from the Awkhad web client root.
...
Much of Awkhad web's usefulness (and complexity) resides in views. Each view
...
Most :ref:`Awkhad views <reference/views>` are implemented through a subclass
of :class:`awkhad.web.View` which provides a bit of generic basic structure
for handling events and displaying model information.

The *search view* is considered a view type by the main Awkhad framework, but
...
:py:class:`~awkhad.models.Model.fields_view_get`) and any other data source
it needs. To that purpose, views are provided with an optional view
identifier set as the :attr:`~awkhad.web.View.view_id` attribute.

Views are also provided with a :class:`~awkhad.web.DataSet` instance which
...
:func:`~awkhad.web.View.do_search`, and updating their
:class:`~awkhad.web.DataSet` as necessary.
...
:py:class:`~awkhad.fields.Char` fields which should contain email addresses
...
* It must correctly implement the 3 field attributes available in all fields
  of Awkhad. The ``AbstractField`` class already implements an algorithm that
  ...
  * ``readonly``: When ``true``, the field must not be editable by the
    user. Most fields in Awkhad have a completely different behavior depending
...
* Fields have two methods, ``set_value()`` and ``get_value()``, which are
  ...
  given by the Awkhad server when a ``read()`` is performed on a model and give
  ...
  ``write()`` is not necessarily the same in Awkhad. As example, when you read a
...
interface and the ``AbstractField`` class directly in the code of the Awkhad web
client.
...
user to modify it can be useful, but most fields in Awkhad also allow editing.
...
their :attr:`~awkhad.web.form.FormWidget.field_manager` attribute::
...
:attr:`~awkhad.web.form.FormWidget` is generally the
:class:`~awkhad.web.form.FormView` itself, but features used from it should
be limited to those defined by :class:`~awkhad.web.form.FieldManagerMixin`,
the most useful being:

* :func:`~awkhad.web.form.FieldManagerMixin.get_field_value(field_name)`
  which returns the value of a field.
* :func:`~awkhad.web.form.FieldManagerMixin.set_values(values)` sets multiple
