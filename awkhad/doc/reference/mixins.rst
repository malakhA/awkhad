...
Awkhad implements some useful classes and mixins that make it easy for you to add
...
``_mail_post_access`` - :class:`~awkhad.models.Model`  attribute
...
database without needing to connect to Awkhad directly.
...
catchall domain will be sufficient since all routing will be done inside Awkhad.
...
.. note:: Aliases can also be created manually and supported by a simple
    :class:`~awkhad.fields.Many2one` field. This guide assumes you wish a
...
* ``campaign_id``: :class:`~awkhad.fields.Many2one` field to a ``utm.campaign``
  object (i.e. Christmas_Special, Fall_Collection, etc.)
* ``source_id``: :class:`~awkhad.fields.Many2one` field to a ``utm.source``
  object (i.e. Search Engine, mailing list, etc.)
* ``medium_id``: :class:`~awkhad.fields.Many2one` field to a ``utm.medium``
...
(i.e. https://www.awkhad.com/?campaign_id=mixin_talk&source_id=www.awkhad.com&medium_id=website),
    ...
    class MyModel(models.Models):
        ...
        def tracking_fields(self):
            ...
            # ("URL_PARAMETER", "FIELD_NAME_MIXIN", "NAME_IN_COOKIES")
                ('my_field', 'my_field', 'awkhad_utm_my_field')
            ])
            return result

This will tell the system to create a cookie named *awkhad_utm_my_field* with the
...
* ``helpdesk.ticket`` in the Helpdesk (*helpdesk* - Awkhad Enterprise only) Application
...
* ``website_published``: :class:`~awkhad.fields.Boolean` field which represents
  the status of the publication
* ``website_url``: :class:`~awkhad.fields.Char` field which represents
...
* ``website_meta_title``: :class:`~awkhad.fields.Char` field that allow you to set
  an additional title to your page
* ``website_meta_description``: :class:`~awkhad.fields.Char` field that contains a
  short description of the page (sometimes used in search engines results)
* ``website_meta_keywords``: :class:`~awkhad.fields.Char` field that contains some
