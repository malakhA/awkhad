:banner: banners/awkhad_guideline.jpg
...
This page introduces the Awkhad Coding Guidelines. Those aim to improve the
quality of Awkhad Apps code. Indeed proper code improves readability, eases
...
.. warning::
    ...
    details, see our `pull request guide <https://awkhad.com/submit-pr>`_.
...
File naming is important to quickly find information through all awkhad addons.
This section explains how to name files in a standard awkhad module. As an
example we use a `plant nursery <https://github.com/tivisse/awkhaddays-2018/tree/master/plant_nursery>`_ application.
...
.. code-block:: text
    ...
    |   |-- res_partner.py (inherited Awkhad model)
...
contained in a file named ``<module_name>.py``. An old convention in Awkhad is to
...
data (image, libraries) outside Awkhad: do not use an URL to an image but copy
...
directory. This naming comes from old awkhad applications using the wizard
...
The complete tree of our Awkhad module therefore looks like
...
- The tag *<data>* is only used to set not-updatable data with ``noupdate=1``.
  ...
  set on the ``<awkhad>`` tag and do not set a ``<data>`` tag.
...
Awkhad supports custom tags acting as syntactic sugar:
...
Using a linter can help show syntax and semantic warnings or errors. Awkhad
...
#. Imports of ``awkhad``
#. Imports from Awkhad modules (rarely, and only if necessary)
...
.. code-block:: python
    ...
    # 2 : imports of awkhad
    import awkhad
    from awkhad import api, fields, models, _ # alphabetically ordered
    from awkhad.tools.safe_eval import safe_eval as eval
    # 3 : imports from awkhad addons
    from awkhad.addons.website.models.website import slug
    from awkhad.addons.web.controllers.main import login_redirect
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The Awkhad framework is in charge of providing the transactional context for
...
Awkhad uses a GetText-like method named "underscore" ``_( )`` to indicate that
...
In general in Awkhad, when manipulating strings, prefer ``%`` over ``.format()``
...
- Model name (using the dot notation, prefix by the module name) :
    - When defining an Awkhad Model : use singular form of the name (*res.partner*
      and *sale.order* instead of *res.partnerS* and *saleS.orderS*)
    - When defining an Awkhad Transient (wizard) : use ``<related_base_model>.<action>``
...
- Awkhad Python Class : use camelcase (Object-oriented style).
...
Awkhad addons have some conventions on how to structure various files. We explain
here in more details how web assets are supposed to be organized.

The first thing to know is that the Awkhad server will serve (statically) all files
...
statically available at the url *your-awkhad-server.com/web/static/src/js/some_file.js*
...
More precise JS guidelines are detailed in the `github wiki  <https://github.com/awkhad/awkhad/wiki/Javascript-coding-guidelines>`_.
...
- **[CLA]** for signing the Awkhad Individual Contributor License;
...
For Awkhad R&D developers "PO team asked me to do it" is not a valid why, by the way.
...
If you are an Awkhad R&D developer the WHY should be the purpose of the task you
