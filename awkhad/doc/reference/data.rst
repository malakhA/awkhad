...
Awkhad is greatly data-driven, and a big part of modules definition is thus
...
The main way to define data in Awkhad is via XML data files: The broad structure
of an XML data file is the following:

* Any number of operation elements within the root element ``awkhad``
...
.. note::
    ...
    can specify the awkhad flag ``noupdate`` set to 1.  If part of
    ...
    .. code-block:: xml
      ...
      <awkhad>
          <data noupdate="1">
              <!-- Only loaded when installing the module (awkhad-bin -i module) -->
              <operation/>
          </data>

          <!-- (Re)Loaded at install and update (awkhad-bin -i/-u) -->
          <operation/>
      </awkhad>
...
``search``
    ...
    the field is a :class:`~awkhad.fields.Many2one`
``ref``
    ...
    Mostly for :class:`~awkhad.fields.Many2one` and
    :class:`~awkhad.fields.Reference` fields
``type``
...
Because some important structural models of Awkhad are complex and involved,
...
.. literalinclude:: ../../awkhad/addons/base/data/res.country.state.csv
    :language: text
    :lines: 1-15
...
.. csv-table::
    :file: ../../awkhad/addons/base/data/res.country.state.csv
    :header-rows: 1
