...
There are many ways to test an application.  In Awkhad, we have three kinds of
tests
...
Awkhad provides support for testing modules using unittest.
...
`unittest documentation`_, but Awkhad provides a number of utilities and helpers
related to testing Awkhad content (modules, mainly):

.. autoclass:: awkhad.tests.common.TransactionCase
    :members: browse_ref, ref

.. autoclass:: awkhad.tests.common.SingleTransactionCase
    :members: browse_ref, ref

.. autoclass:: awkhad.tests.common.SavepointCase

.. autoclass:: awkhad.tests.common.HttpCase
    :members: browse_ref, ref, url_open, phantom_js

.. autofunction:: awkhad.tests.common.tagged
...
.. autofunction:: awkhad.tests.common.at_install

.. autofunction:: awkhad.tests.common.post_install

The most common situation is to use
:class:`~awkhad.tests.common.TransactionCase` and test a property of a model
in each method::
...
.. autoclass:: awkhad.tests.common.Form
    :members:

.. autoclass:: awkhad.tests.common.M2MProxy
    :members: add, remove, clear

.. autoclass:: awkhad.tests.common.O2MProxy
...
:option:`--test-enable <awkhad-bin --test-enable>` was enabled when starting the
Awkhad server.
...
In Awkhad, Python tests can be tagged to facilitate the test selection when
running tests.

Subclasses of :class:`awkhad.tests.common.BaseCase` (usually through
:class:`~awkhad.tests.common.TransactionCase`,
:class:`~awkhad.tests.common.SavepointCase` or
:class:`~awkhad.tests.common.HttpCase`) are automatically tagged with
``standard``, ``at_install`` and their source module's name by default.

Invocation
^^^^^^^^^^

:option:`--test-tags <awkhad-bin --test-tags>` can be used to select/filter tests
to run on the command-line.

This option defaults to ``+standard`` meaning tests tagged ``standard``
(explicitly or implicitly) will be run by default when starting Awkhad
with :option:`--test-enable <awkhad-bin --test-enable>`.

When writing tests, the :func:`~awkhad.tests.common.tagged` decorator can be
used on **test classes** to add or remove tags.

The decorator's arguments are tag names, as strings.

.. danger:: :func:`~awkhad.tests.common.tagged` is a class decorator, it has no
...
.. code-block:: python
    ...
    from awkhad.tests import TransactionCase, tagged
...
.. code-block:: console
    ...
    $ awkhad-bin --test-enable --test-tags nice
...
:option:`--test-tags <awkhad-bin --test-tags>`: on the command-line, values
are *additive* (you're selecting all tests with *any* of the specified tags)

.. code-block:: console

    $ awkhad-bin --test-enable --test-tags nice,standard
...
.. code-block:: console
    ...
    $ awkhad-bin --test-enable --test-tags 'standard,-slow'

When you write a test that does not inherit from the
:class:`~awkhad.tests.common.BaseCase`, this test will not have the default tags,
...
- ``standard``: All Awkhad tests that inherit from
  :class:`~awkhad.tests.common.BaseCase` are implicitely tagged standard.
  :option:`--test-tags <awkhad-bin --test-tags>` also defaults to ``standard``.
...
- *module_name*: Awkhad tests classes extending
  :class:`~awkhad.tests.common.BaseCase` are implicitely tagged with the
  ...
  .. code-block:: console
      ...
      $ awkhad-bin --test-enable --test-tags stock_account
...
.. important::
    ...
    modules have to be selected with the :option:`-u <awkhad-bin -u>` or
    :option:`-i <awkhad-bin -i>` switches.  For simplicity, those switches are
...
.. code-block:: console
    ...
    $ awkhad-bin --test-enable --test-tags sale

Run the tests from the sale module but not the ones tagged as slow:

.. code-block:: console

    $ awkhad-bin --test-enable --test-tags 'sale,-slow'

Run only the tests from stock or tagged as slow:

.. code-block:: console

    $ awkhad-bin --test-enable --test-tags '-standard, slow, stock'
...
Awkhad Web includes means to unit-test both the core code of
Awkhad Web and your own javascript modules. On the javascript side,
unit-testing is based on QUnit_ with a number of helpers and
extensions for better integration with Awkhad.

To see what the runner looks like, find (or start) an Awkhad server
...
.. code-block:: console
  ...
  '/tmp/awkhad_tests/{db_name}/screenshots/'

Two new command line arguments were backported from Awkhad 13.0 to control this behavior:
:option:`--screenshots <awkhad-bin --screenshots>` and :option:`--screencasts <awkhad-bin --screencasts>`
