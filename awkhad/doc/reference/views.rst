...
``groups_id``
    :class:`~awkhad.fields.Many2many` field to the groups allowed to view/use
    the current view
``inherit_id``
...
``button``
    ...
    ``type``
        type of button, indicates how it clicking it affects Awkhad:
    ...
    ``context``
        merged into the view's context when performing the button's Awkhad call
    ``confirm``
        confirmation message to display (and for the user to accept) before
        performing the button's Awkhad call
...
Semantic components tie into and allow interaction with the Awkhad
system. Available semantic components are:

``button``
  call into the Awkhad system, similar to :ref:`list view buttons
...
``field``
  ...
  ``widget``
    ...
    (e.g. :class:`~awkhad.fields.Char`,
    :class:`~awkhad.fields.Many2one`). The ``widget`` attributes allows using
  ...
  ``on_change``
    ...
    .. deprecated:: 8.0
       ...
       Use :func:`awkhad.api.onchange` on the model
  ...
  ``mode``
    for :class:`~awkhad.fields.One2many`, display mode (view type) to use for
  ...
  ``filename``
    ...
    file
  ``password``
    indicates that a :class:`~awkhad.fields.Char` field stores a password and
...
``templates``
  ...
  ``record``
    ...
    a :meth:`~awkhad.models.Model.read` (except for date and datetime fields
    that are `formatted according to user's locale
    <https://github.com/awkhad/awkhad/blob/a678bd4e/addons/web_kanban/static/src/js/kanban_record.js#L102>`_)
  ``read_only_mode``
    self-explanatory
    ...
    * buttons and links with a ``type`` attribute become perform Awkhad-related
      operations rather than their standard HTML function. Possible types are:

      ``action``, ``object``
        standard behavior for :ref:`Awkhad buttons
        <reference/views/list/button>`, most attributes relevant to standard
        Awkhad buttons can be used.
      ``open``
...
``arrow`` (required, 1)
    ...
    ``object`` (required)
      the edge's Awkhad model
    ``source`` (required)
      :class:`~awkhad.fields.Many2one` field of the edge's model pointing to
...
.. warning::
   ...
   The Dashboard view is only available in Awkhad Enterprise.
...
``aggregate``
    ...
    - ``group_operator`` (optional)
        ...
        .. note:: The special aggregate function ``count_distinct`` (defined in awkhad) can also be used here
...
.. warning::
   ...
   The Cohort view is only available in Awkhad Enterprise.
...
``field``
    ...
    ``widget``
        ...
        standard Awkhad 8.0 is a ``selection`` widget for
        :class:`~awkhad.fields.Many2one` fields)
    ``domain``
        if the field can provide an auto-completion
        (e.g. :class:`~awkhad.fields.Many2one`), filters the possible
        completion results.
...
``filter``
    ...
    ``domain`` (optional)
        an Awkhad :ref:`domain <reference/orm/domains>`, will be appended to the
