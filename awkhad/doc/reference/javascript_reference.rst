...
This document presents the Awkhad Javascript framework. This
...
.. note:: An html version of all js docstrings in Awkhad is available at:
      :ref:`JS API <api/js>`
...
- the *website*: this is the public part of Awkhad.  It allows an unidentified
...
It means that while a user is working on Awkhad, the web client class (and the
...
Managing assets in Awkhad is not as straightforward as it is in some other apps.
...
a collection of files (javascript, css, scss). In Awkhad, the most important
...
Main bundles
------------
When the Awkhad server is started, it checks the timestamp of each file in a bundle,
...
- *web.assets_common*: this bundle contains most assets which are common to the
  ...
  lower level building blocks for the awkhad framework.  Note that it contains the
  *boot.js* file, which defines the awkhad module system.
...
.. note ::
    ...
    awkhad web client.  This means that the files are transferred through the network
...
sure they are loaded in the correct order.  In order to do that, Awkhad has defined
a small module system (located in the file *addons/web/static/src/js/boot.js*,
which needs to be loaded first).

The Awkhad module system, inspired by AMD, works by defining the function *define*
on the global awkhad object. We then define each javascript module by calling that
function.  In the Awkhad framework, a module is a piece of code that will be executed
...
.. code-block:: javascript
    ...
    // in file a.js
    awkhad.define('module.A', function (require) {
        "use strict";
    ...
    // in file b.js
    awkhad.define('module.B', function (require) {
        "use strict";
...
.. code-block:: javascript
    ...
    awkhad.define('module.Something', ['module.A', 'module.B'], function (require) {
        "use strict";
...
The *awkhad.define* method is given three arguments:

- *moduleName*: the name of the javascript module.  It should be a unique string.
  The convention is to have the name of the awkhad addon followed by a specific
...
.. code-block:: javascript
    ...
    awkhad.define('module.Something', ['web.ajax'], function (require) {
        "use strict";
...
Awkhad was developed before ECMAScript 6 classes were available.  In Ecmascript 5,
...
For these reasons, Awkhad decided to use its own class system, inspired by John
...
The awkhad Class system does not support multiple inheritance, but for those cases
...
the way Awkhad is structured, it is sometimes necessary in one addon to modify
...
* Global selectors should be avoided. Because a component may be used several
  times in a single page (an example in Awkhad is dashboards), queries should be
  ...
  lead to unintended or incorrect behavior.  Awkhad Web's
...
There are currently two event systems supported by Awkhad: a simple system which
...
- *trigger_up*: this is the method that will create a small *AwkhadEvent* and
...
The AwkhadEvent class is very simple.  It has three public attributes: *target*
...
A common need in the Awkhad ecosystem is to extend/change the behaviour of the
...
There are typically two usecases when working on Awkhad: one may need to call a
...
The Awkhad framework has a standard way to communicate various informations to the
...
The notification system in Awkhad is designed with the following components:
...
In 2017, Awkhad replaced the previous view code with a new architecture.  The
...
handled by awkhad and correspond to italic and bold, respectively).  Note that the
