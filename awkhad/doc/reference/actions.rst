...
``views``
    ...
    :meth:`~awkhad.models.Model.fields_view_get`). The first type of the
...
Allow opening a URL (website/web page) via an Awkhad action. Can be customized
via two fields:
...
::

    {
        ...
        "url": "https://awkhad.com",
        "target": "self",
    }

will replace the current content section by the Awkhad home page.
...
.. autoclass:: awkhad.addons.base.models.ir_actions.IrActionsServer
...
``model_id``
    Awkhad model linked to the action.
...
``fields_lines`` (create/write)
    ...
    :class:`~awkhad.fields.One2many` with the fields:
...
``paperformat_id``
    :class:`~awkhad.fields.Many2one` field to the paper format you wish to
