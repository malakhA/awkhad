...
.. note:: If you want to contribute to the translation of Awkhad itself, please refer to the
  `Awkhad Wiki page <https://github.com/awkhad/awkhad/wiki/Translations>`_.
...
automatically loaded by Awkhad when the corresponding language is installed (via
...
Awkhad automatically exports translatable strings from "data"-type content:
...
* QWeb templates (both server-side and client-side), all text nodes are
  ...
  exported
* for :class:`~awkhad.fields.Field`, unless their model is marked with
  ``_translate = False``:
...
* help/error messages of :attr:`~awkhad.models.Model._constraints` and
  :attr:`~awkhad.models.Model._sql_constraints` are exported

Explicit exports
================

When it comes to more "imperative" situations in Python code or Javascript
code, Awkhad cannot automatically export translatable terms so they
must be marked explicitly for export. This is done by wrapping a literal
string in a function call.

In Python, the wrapping function is :func:`awkhad._`::

    title = _("Bank Accounts")

In JavaScript, the wrapping function is generally :js:func:`awkhad.web._t`:
