...
There are many ways to solve a problem in JavaScript, and in Awkhad.  However, the
Awkhad framework was designed to be extensible (this is a pretty big constraint),
and some common problems have a nice standard solution.  The standard solution
has probably the advantage of being easy to understand for an awkhad developers,
and will probably keep working when Awkhad is modified.
...
First of all, remember that the first rule of customizing awkhad with JS is:
...
example, the voip addon in awkhad need to modify the FieldPhone widget to add the
