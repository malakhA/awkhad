...
In-App Purchase (IAP) allows providers of ongoing services through Awkhad apps to
be compensated for ongoing service use rather than — and possibly instead of
— a sole initial purchase.

In that context, Awkhad acts mostly as a *broker* between a client and an Awkhad
App Developer:

* Users purchase service tokens from Awkhad.
* Service providers draw tokens from the user's Awkhad account when service
  is requested.

.. attention::

    This document is intended for *service providers* and presents the latter,
    which can be done either via direct JSON-RPC2_ or if you are using Awkhad
...
.. figure:: images/players.png
    ...
    * The Client installed your Awkhad App, and from there will request services.
    * Awkhad brokers crediting, the Client adds credit to their account, and you
    ...
    * The External Service is an optional player: *you* can either provide a
      ...
      bridge/translator between an Awkhad system and the actual service.
...
.. note:: The credits went from integer to float value starting **October 2018**.
    ...
    * for a postage service: 1 credit = 1 post stamp.
    ...
    that the clients can buy on https://iap.awkhad.com (see :ref:`Packs <iap-packages>`).
...
.. figure:: images/normal.png
    ...
    2. The Service Provider asks Awkhad if there are enough credits for the
    ...
    4. The Service Provider goes back to Awkhad to capture (if the service could
...
.. figure:: images/no-credit.png
    ...
    2. The Service Provider asks Awkhad if there are enough credits on the
       Client's account and gets a negative reply.
    3. This is signaled back to the Client.
    4. Who is redirected to their Awkhad account to credit it and re-try.
...
Register the service on Awkhad
----------------------------
...
go to your *Portal Account* on the IAP endpoint (https://iap.awkhad.com for
production, https://iap-sandbox.awkhad.com for testing, the endpoints are
*independent* and *not synchronized*). Alternatively, you can go to your portal
on Awkhad (https://iap.awkhad.com/my/home) and select *In-App Services*.
...
* :samp:`name` - :class:`ServiceName`: This is the string you will need to provide inside
  the client's :ref:`app <iap-awkhad-app>` when requesting a transaction from Awkhad. (e.g.
...
.. warning::
   ...
   :class:`ServiceName` should usually match the name of your Awkhad Client App.
...
.. note::
    ...
    Awkhad takes a 25% commission on all pack sales. Adjust your selling price accordingly.
...
The second step is to develop an `Awkhad App`_ which clients can install in their
Awkhad instance and through which they can *request* the services you provide.
Our app will just add a button to the Partners form which lets a user request
burning some CPU time on the server.

First, we will create an *awkhad module* depending on ``iap``. IAP is a standard
...
:func:`~awkhad.addons.iap.jsonrpc` helper to call a JSON-RPC2_ endpoint on an
other Awkhad instance and transparently re-raise relevant Awkhad exceptions
(:class:`~awkhad.addons.iap.models.iap.InsufficientCreditError`,
:class:`awkhad.exceptions.AccessError` and :class:`awkhad.exceptions.UserError`).
...
.. note::
    ...
    :class:`~awkhad.addons.iap.models.iap.InsufficientCreditError` coming from the action
    and prompts the user to add credits to their account.

    :func:`~awkhad.addons.iap.jsonrpc` takes care of re-raising
    :class:`~awkhad.addons.iap.models.iap.InsufficientCreditError` for you.

.. danger::

    If you are not using :func:`~awkhad.addons.iap.jsonrpc` you *must* be
    careful to re-raise
    :class:`~awkhad.addons.iap.models.iap.InsufficientCreditError` in your handler
...
for JSON-RPC2_ calls (:func:`~awkhad.addons.iap.jsonrpc`) and a service helper
for transactions (:class:`~awkhad.addons.iap.models.iap.charge`) we will also be
implementing the service side as an Awkhad module:

.. patch::

Since the query from the client comes as JSON-RPC2_ we will need the
corresponding controller which can call :class:`~awkhad.addons.iap.models.iap.charge` and
perform the service within:

.. patch::

.. todo:: for the actual IAP will the "portal" page be on awkhad.com or iap.awkhad.com?

.. todo:: "My Account" > "Your InApp Services"?


The :class:`~awkhad.addons.iap.models.iap.charge` helper will:
...
.. danger::
    ...
    By default, :class:`~awkhad.addons.iap.models.iap.charge` contacts the *production*
    IAP endpoint, https://iap.awkhad.com. While developing and testing your
    service you may want to point it towards the *development* IAP endpoint
    https://iap-sandbox.awkhad.com.

    To do so, set the ``iap.endpoint`` config parameter in your service
    Awkhad: in debug/developer mode, :menuselection:`Setting --> Technical -->
    Parameters --> System Parameters`, just define an entry for the key
    ``iap.endpoint`` if none already exists).

The :class:`~awkhad.addons.iap.models.iap.charge` helper has two additional optional
...
* The IAP transaction API does not require using Awkhad when implementing your
...
.. function:: /iap/1/authorize
    ...
    *and creates a hold (pending transaction) on that amount*.
    ...
    :raises: :class:`~awkhad.exceptions.AccessError` if the service token is invalid
    :raises: :class:`~awkhad.addons.iap.models.iap.InsufficientCreditError` if the account does not have enough credits
    :raises: ``TypeError`` if the ``credit`` value is not an integer or a float

.. code-block:: python

    r = requests.post(AWKHAD + '/iap/1/authorize', json={
        'jsonrpc': '2.0',
        'id': None,
        'method': 'call',
        'params': {
...
.. function:: /iap/1/capture
    ...
    :raises: :class:`~awkhad.exceptions.AccessError`

.. code-block:: python
  :emphasize-lines: 8

    r2 = requests.post(AWKHAD + '/iap/1/capture', json={
        'jsonrpc': '2.0',
        'id': None,
        'method': 'call',
        'params': {
...
.. code-block:: python
    ...
    r2 = requests.post(AWKHAD + '/iap/1/cancel', json={
        'jsonrpc': '2.0',
        'id': None,
        'method': 'call',
        'params': {
...
.. class:: ServiceName
    ...
    String identifying your service on https://iap.awkhad.com (production) as well
...
.. exception:: awkhad.addons.iap.models.iap.InsufficientCreditError
...
.. exception:: awkhad.exceptions.AccessError
    :noindex:

    Raised by:

    * any operation to which a service token is required, if the service token is invalid; or
    * any failure in an inter-server call. (typically, in :func:`~awkhad.addons.iap.jsonrpc`).

.. exception:: awkhad.exceptions.UserError
    :noindex:
...
* Token ``000000``: Represents a non-existing account. Returns
  an :class:`~awkhad.addons.iap.models.iap.InsufficientCreditError` on authorize attempt.
* Token ``000111``: Represents an account without sufficient credits to perform any service.
  Returns an :class:`~awkhad.addons.iap.models.iap.InsufficientCreditError` on authorize attempt.
...
For convenience, if you are implementing your service using Awkhad the ``iap``
...
.. class:: awkhad.addons.iap.models.iap.charge(env, key, account_token, credit[, dbuuid, description, credit_template])
    ...
    * otherwise cancels it.
    ...
    :param awkhad.api.Environment env: used to retrieve the ``iap.endpoint``
                                     configuration key
    :param ServiceKey key:
...
.. class:: awkhad.addons.iap.models.iap.authorize(env, key, account_token, credit[, dbuuid, description, credit_template])
...
.. class:: awkhad.addons.iap.models.iap.cancel(env, transaction_token, key)
...
.. class:: awkhad.addons.iap.models.iap.capture(env, transaction_token, key, credit)
...
.. _Awkhad App: https://www.awkhad.com/apps
