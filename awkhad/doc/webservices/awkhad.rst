...
Awkhad is usually extended internally via modules, but many of its features and
all of its data are also available from the outside for external analysis or
integration with various tools. Part of the :ref:`reference/orm/model` API is
easily available over XML-RPC_ and accessible from a variety of languages.

.. Awkhad XML-RPC idiosyncracies:
   * uses multiple endpoint and a nested call syntax instead of a
     "hierarchical" server structure (e.g. ``awkhad.res.partner.read()``)
...
.. only:: html
    ...
    .. rst-class:: setupcode hidden
        ...
        .. code-block:: python3
            ...
            info = xmlrpc.client.ServerProxy('https://demo.awkhad.com/start').start()
        ...
        .. code-block:: ruby
            ...
            info = XMLRPC::Client.new2('https://demo.awkhad.com/start').call('start')
            url, db, username, password = \
                info['host'], info['database'], info['user'], info['password']
        ...
        .. code-block:: php
            ...
            $info = ripcord::client('https://demo.awkhad.com/start')->start();
        ...
        .. code-block:: java
            ...
            start_config.setServerURL(new URL("https://demo.awkhad.com/start"));
...
If you already have an Awkhad server installed, you can just use its
parameters

.. warning::

    For Awkhad Online instances (<domain>.awkhad.com), users are created without a
    *local* password (as a person you are logged in via the Awkhad Online
    authentication system, not by the instance itself). To use XML-RPC on Awkhad
    ...
    *https://mycompany.awkhad.com*), the *database name* is the name of the
...
To make exploration simpler, you can also ask https://demo.awkhad.com for a test
database:
...
Awkhad requires users of the API to be authenticated before they can query most
data.
...
The second endpoint is ``xmlrpc/2/object``, is used to call methods of awkhad
...
Records can be listed and filtered via :meth:`~awkhad.models.Model.search`.

:meth:`~awkhad.models.Model.search` takes a mandatory
...
:meth:`~awkhad.models.Model.search_count` can be used to retrieve
only the number of records matching the query. It takes the same
:ref:`domain <reference/orm/domains>` filter as
:meth:`~awkhad.models.Model.search` and no other parameter.
...
Record data is accessible via the :meth:`~awkhad.models.Model.read` method,
which takes a list of ids (as returned by
:meth:`~awkhad.models.Model.search`) and optionally a list of fields to
...
:meth:`~awkhad.models.Model.fields_get` can be used to inspect
...
Because it is a very common task, Awkhad provides a
:meth:`~awkhad.models.Model.search_read` shortcut which as its name suggests is
equivalent to a :meth:`~awkhad.models.Model.search` followed by a
:meth:`~awkhad.models.Model.read`, but avoids having to perform two requests
and keep ids around.

Its arguments are similar to :meth:`~awkhad.models.Model.search`'s, but it
can also take a list of ``fields`` (like :meth:`~awkhad.models.Model.read`,
...
Records of a model are created using :meth:`~awkhad.models.Model.create`. The
method will create a single record and return its database identifier.

:meth:`~awkhad.models.Model.create` takes a mapping of fields to values, used
...
.. warning::
    ...
    :class:`~awkhad.fields.Integer`, string for :class:`~awkhad.fields.Char`
    or :class:`~awkhad.fields.Text`),

    * :class:`~awkhad.fields.Date`, :class:`~awkhad.fields.Datetime` and
      :class:`~awkhad.fields.Binary` fields use string values
    * :class:`~awkhad.fields.One2many` and :class:`~awkhad.fields.Many2many`
      use a special command protocol detailed in :meth:`the documentation to
      the write method <awkhad.models.Model.write>`.

Update records
--------------

Records can be updated using :meth:`~awkhad.models.Model.write`, it takes
a list of records to update and a mapping of updated fields to values similar
to :meth:`~awkhad.models.Model.create`.
...
:meth:`~awkhad.models.Model.unlink`.
...
While we previously used :meth:`~awkhad.models.Model.fields_get` to query a
model and have been using an arbitrary model from the start, Awkhad stores
...
Provides information about Awkhad models via its various fields
...
``field_id``
    list of the model's fields through a :class:`~awkhad.fields.One2many` to
    :ref:`reference/webservice/inspection/fields`
``view_ids``
    :class:`~awkhad.fields.One2many` to the :ref:`reference/views` defined
    for the model
``access_ids``
    :class:`~awkhad.fields.One2many` relation to the
...
Provides information about the fields of Awkhad models and allows adding
custom fields without using Python code

``model_id``
    :class:`~awkhad.fields.Many2one` to
...
``groups``
    ...
    :class:`~awkhad.fields.Many2many` to ``res.groups``
