:banner: banners/deploying_awkhad.jpg

==============
Deploying Awkhad
==============

This document describes basic steps to set up Awkhad in production or on an
...
Awkhad is a multi-tenant system: a single Awkhad system may run and serve a number
...
bound to a database: Awkhad needs to know which database should be used to load
...
accessible Awkhad needs a rule to know which one it should use.

That is one of the purposes of :option:`--db-filter <awkhad-bin --db-filter>`:
...
.. note::
  Setting a proper :option:`--db-filter <awkhad-bin --db-filter>` is an important part
...
UNIX socket is fine if you want Awkhad and PostgreSQL to execute on the same
machine, and is the default when no host is provided, but if you want Awkhad and
...
* Only accept loopback connections and `use an SSH tunnel`_ between the
  machine on which Awkhad runs and the one on which PostgreSQL runs, then
  configure Awkhad to connect to its end of the tunnel
* Accept connections to the machine on which Awkhad is installed, possibly
  over ssl (see `PostgreSQL connection settings`_ for details), then configure
  Awkhad to connect over the network
...
.. _setup/deploy/awkhad:

Configuring Awkhad
----------------

Out of the box, Awkhad connects to a local postgres over UNIX socket via port
...
create a new user (``awkhad``) and set it as the database user.
...
SSL Between Awkhad and PostgreSQL
-------------------------------

Since Awkhad 11.0, you can enforce ssl connection between Awkhad and PostgreSQL.
in Awkhad the db_sslmode control the ssl security of the connection
...
Awkhad includes built-in HTTP servers, using either multithreading or
multiprocessing.
...
* Multiprocessing is enabled by configuring :option:`a non-zero number of
  worker processes <awkhad-bin --workers>`, the number of workers should be based
...
listening on :option:`the longpolling port <awkhad-bin --longpolling-port>` but
the client will not connect to it.

Instead you must have a proxy redirecting requests whose URL starts with
``/longpolling/`` to the longpolling port. Other request should be proxied to
the :option:`normal HTTP port <awkhad-bin --http-port>`

To achieve such a thing, you'll need to deploy a reverse proxy in front of Awkhad,
like nginx or apache. When doing so, you'll need to forward some more http Headers
to Awkhad, and activate the proxy_mode in Awkhad configuration to have Awkhad read those
headers.
...
* RAM = 9 * ((0.8*150) + (0.2*1024)) ~= 3Go RAM for Awkhad
...
Whether it's accessed via website/web client or web service, Awkhad transmits
authentication information in cleartext. This means a secure deployment of
Awkhad must use HTTPS\ [#switching]_. SSL termination can be implemented via
just about any SSL termination proxy, but requires the following setup:

* Enable Awkhad's :option:`proxy mode <awkhad-bin --proxy-mode>`. This should only be enabled when Awkhad is behind a reverse proxy
...
in ``/etc/nginx/sites-enabled/awkhad.conf`` set:
...
.. code-block:: nginx
  ...
  server {
     listen 80;
     server_name awkhad.mycompany.com;
  ...
  server {
   ...
   # Add Headers for awkhad proxy mode
   ...
   # log
   access_log /var/log/nginx/awkhad.access.log;
   error_log /var/log/nginx/awkhad.error.log;

   # Redirect longpoll requests to awkhad longpolling port
   location /longpolling {
   proxy_pass http://awkhadchat;
   }

   # Redirect requests to awkhad backend server
   location / {
     proxy_redirect off;
     proxy_pass http://awkhad;
   }
...
Awkhad as a WSGI Application
==========================

It is also possible to mount Awkhad as a standard WSGI_ application. Awkhad
provides the base for a WSGI launcher script as ``awkhad-wsgi.example.py``. That
script should be customized (possibly after copying it from the setup directory) to correctly set the
configuration directly in :mod:`awkhad.tools.config` rather than through the
command-line or a configuration file.

However the WSGI server will only expose the main HTTP endpoint for the web
client, website and webservice API. Because Awkhad does not control the creation
of workers anymore it can not setup cron or livechat workers

Cron Workers
------------

To run cron jobs for an Awkhad deployment as a WSGI application requires

* A classical Awkhad (run via ``awkhad-bin``)
...
* Which should not be exposed to the network. To ensure cron runners are not
  ...
  entirely with :option:`awkhad-bin --no-http` or setting ``http_enable = False``
...
* Deploy a threaded version of Awkhad (instread of a process-based preforking
  one) and redirect only requests to URLs starting with ``/longpolling/`` to
  that Awkhad, this is the simplest and the longpolling URL can double up as
  the cron instance.
* Deploy an evented Awkhad via ``awkhad-gevent`` and proxy requests starting
  with ``/longpolling/`` to
  :option:`the longpolling port <awkhad-bin --longpolling-port>`.

Serving Static Files
====================

For development convenience, Awkhad directly serves all static files in its
...
Awkhad static files live in each module's ``static/`` folder, so static files
...
- Use appropriate database filters ( :option:`--db-filter <awkhad-bin --db-filter>`)
  to restrict the visibility of your databases according to the hostname.
  See :ref:`db_filter`.
  You may also use :option:`-d <awkhad-bin -d>` to provide your own (comma-separated)
...
- Once your ``db_name`` and ``db_filter`` are configured and only match a single database
  ...
  (this is also exposed as the :option:`--no-database-list <awkhad-bin --no-database-list>`
  command-line option)

- Make sure the PostgreSQL user (:option:`--db_user <awkhad-bin --db_user>`) is *not* a super-user,
  ...
  See also :ref:`setup/deploy/awkhad`.
...
- Keep installations updated by regularly installing the latest builds,
  ...
  https://www.awkhad.com/page/download or http://nightly.awkhad.com
...
- Run Awkhad behind a web server providing HTTPS termination with a valid SSL certificate,
  ...
  and then enable the :option:`proxy mode <awkhad-bin --proxy-mode>` option.
...
threat should not be neglected for Awkhad servers. Awkhad emits a log entry whenever a login attempt
...
Failed login::
      ...
      2018-07-05 14:56:31,506 24849 INFO db_name awkhad.addons.base.res.res_users: Login failed for db:db_name login:admin from 127.0.0.1

Successful login::

      2018-07-05 14:56:31,506 24849 INFO db_name awkhad.addons.base.res.res_users: Login successful for db:db_name login:admin from 127.0.0.1
...
10 failed login attempts are detected from the same IP within 1 minute::
    ...
    logpath = /var/log/awkhad.log  ;  set the actual awkhad log path here
...
:ref:`setup/deploy/awkhad` mentioned ``admin_passwd`` in passing.
...
Awkhad is supported by multiple browsers for each of its versions. No
distinction is made according to the browser version in order to be
up-to-date. Awkhad is supported on the current browser version. The list
...
.. [#different-machines]
    to have multiple Awkhad installations use the same PostgreSQL database,
...
.. _POSBox: https://www.awkhad.com/page/point-of-sale-hardware#part_2
