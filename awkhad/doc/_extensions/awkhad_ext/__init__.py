...
def setup(app):
    if hasattr(app, 'set_translator'):
        ...
        if getattr(app.config, 'html_translator_class', None):
            app.warn("Overriding the explicitly set  html_translator_class setting",
                     location="awkhad extension")
        app.config.html_translator_class = 'awkhad_ext.translator.BootstrapTranslator'
    ...
    switcher.setup(app)
    app.add_config_value('awkhad_cover_default', None, 'env')
    app.add_config_value('awkhad_cover_external', {}, 'env')
    app.add_config_value('awkhad_cover_default_external', lambda conf: conf.awkhad_cover_default, 'env')
...
def update_meta(app, pagename, templatename, context, doctree):
    ...
    if meta is None:
        meta = context['meta'] = {}
    meta.setdefault('banner', app.config.awkhad_cover_default)
