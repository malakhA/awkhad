        ...
        for title, ref in ((e[0], e[1]) for e in node['entries']):
            ...
            for subtitle, subref in entries:
                ...
                if subref in env.metadata:
                    cover = env.metadata[subref].get('banner', conf.awkhad_cover_default)
                elif subref in conf.awkhad_cover_external:
                    cover = conf.awkhad_cover_external[subref]
                else:
                    cover = conf.awkhad_cover_default_external
