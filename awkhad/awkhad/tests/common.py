# -*- coding: utf-8 -*-
"""
The module :mod:`awkhad.tests.common` provides unittest test cases and a few
...
from awkhad.models import BaseModel
from awkhad.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from awkhad.tools import pycompat
from awkhad.tools import single_email_re
from awkhad.tools.misc import find_in_path
...
from awkhad.service import security



_logger = logging.getLogger(__name__)

# The awkhad library is supposed already configured.
ADDONS_PATH = awkhad.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = awkhad.tools.config['http_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = awkhad.SUPERUSER_ID


def get_db_name():
    db = awkhad.tools.config['db_name']
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most awkhad operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('awkhad.addons.'):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    """
    Subclass of TestCase for common ZgUI-specific code.

    This class is abstract and expects self.registry, self.cr and self.uid to be
    initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.awkhad.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~awkhad.models.BaseModel`
        """
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/awkhad/addons/" in filename:
                        filename = filename.rsplit("/awkhad/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = awkhad.registry(get_db_name())
        #: current transaction's cursor
        self.cr = self.cursor()
        #: :class:`~awkhad.api.Environment` for the current test case
        self.env = api.Environment(self.cr, awkhad.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.registry = awkhad.registry(get_db_name())
        cls.cr = cls.registry.cursor()
        cls.env = api.Environment(cls.cr, awkhad.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_awkhad')
        self.chrome_process = None

        otc = awkhad.tools.config
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = awkhad.http.root.session_store.new()
        self.session_id = self.session.sid
        self.session.db = get_db_name()
        awkhad.http.root.session_store.save(self.session)
    ...
    def _wait_remaining_requests(self):
        ...
        for thread in threading.enumerate():
            if thread.name.startswith('awkhad.service.http.request.'):
                join_retry_count = 10
    ...
    def authenticate(self, user, password):
        ...
        if user is None:
            if self.session:
                awkhad.http.root.session_store.delete(self.session)
        ...
        # ZgUISession.authenticate accesses the current request, which we
        ...
        awkhad.http.root.session_store.save(session)
        if self.browser:
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~awkhad.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~awkhad.fields.Many2many` fields, the field itself is a
    :class:`~awkhad.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~awkhad.fields.One2many` are reified as
    :class:`~awkhad.tests.common.O2MProxy`.

    Because the :class:`~awkhad.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~awkhad.tests.common.O2MProxy.new` and
    :meth:`~awkhad.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: awkhad.models.Model
    ...
    :type view: int | str | awkhad.model.Model
    ...
    def _init_from_defaults(self, model):
        ...
        def cleanup(k, v):
            if fields[k]['type'] == 'one2many':
                ...
                return awkhad.fields.Datetime.to_string(v)
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~awkhad.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~awkhad.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, descr in fields.items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = awkhad.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = awkhad.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return awkhad.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return awkhad.fields.Date.to_string(value)
...
def tagged(*tags):
    ...
    By default, all Test classes from awkhad.tests.common have a test_tags
