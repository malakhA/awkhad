...
from awkhad.tools import frozendict, classproperty, StackMap, pycompat
from awkhad.exceptions import CacheMiss
...
def constrains(*args):
    ...
    Should raise :class:`~awkhad.exceptions.ValidationError` if the
    validation failed.
...
def one(method):
    """ Decorate a record-style method where ``self`` is expected to be a
        ...
        .. deprecated:: 9.0
            ...
            is a single record with :meth:`~awkhad.models.Model.ensure_one`.
    """
...
from awkhad.exceptions import UserError, AccessError, MissingError
