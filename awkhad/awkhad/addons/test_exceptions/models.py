...
class m(models.Model):
    ...
    def generate_except_osv(self):
        ...
        raise awkhad.osv.osv.except_osv('title', 'description')

    @api.multi
    def generate_except_orm(self):
        # title is ignored in the new (6.1) exceptions
        raise awkhad.exceptions.except_orm('title', 'description')

    @api.multi
    def generate_warning(self):
        raise awkhad.exceptions.Warning('description')

    @api.multi
    def generate_redirect_warning(self):
        action = self.env.ref('test_exceptions.action_test_exceptions')
        raise awkhad.exceptions.RedirectWarning('description', action.id, 'Go to the redirection')
    ...
    def generate_access_error(self):
        raise awkhad.exceptions.AccessError('description')
    ...
    def generate_user_error(self):
        raise awkhad.exceptions.UserError('description')

    @api.multi
    def generate_missing_error(self):
        raise awkhad.exceptions.MissingError('description')

    @api.multi
    def generate_validation_error(self):
        raise awkhad.exceptions.ValidationError('description')
