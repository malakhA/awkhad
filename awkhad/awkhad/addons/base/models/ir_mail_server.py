...
from awkhad.exceptions import except_orm, UserError
from awkhad.tools import ustr, pycompat, formataddr
...
class IrMailServer(models.Model):
    ...
    def test_smtp_connection(self):
        for server in self:
            ...
            try:
                ...
                email_from, email_to = self.env.user.email, 'noreply@awkhad.com'
                if not email_from:
    ...
    def connect(self, host=None, port=None, user=None, password=None, encryption=None,
        ...
        if smtp_encryption == 'ssl':
            if 'SMTP_SSL' not in smtplib.__all__:
                raise UserError(
                    _("Your Awkhad Server does not support SMTP-over-SSL. "
    ...
    def _get_default_bounce_address(self):
        ...
        If "mail.bounce.alias" is not set it defaults to "postmaster-awkhad".

        If "mail.catchall.domain" is not set, return None.

        '''
        get_param = self.env['ir.config_parameter'].sudo().get_param
        postmaster = get_param('mail.bounce.alias', default='postmaster-awkhad')
