...
import awkhad
from awkhad import api, fields, models, modules, tools, _
from awkhad.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from awkhad.exceptions import AccessDenied, UserError
from awkhad.osv import expression
from awkhad.tools.parse_version import parse_version
from awkhad.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    to_buy = fields.Boolean('Awkhad Enterprise Module', default=False)
    ...
    def install_from_urls(self, urls):
        ...
        ZGUI = awkhad.release.product_name.lower()
        ...
        try:
            ...
            for module_name, url in urls.items():
                ...
                try:
                    _logger.info('Downloading module `%s` from ZgUI Apps', module_name)
            ...
            for module_name, url in urls.items():
                if module_name == ZGUI or not url:
                    continue    # ZGUI is special case, handled below, and no URL means local module
            ...
            if urls.get(ZGUI):
                ...
                # copy all modules in the SERVER/awkhad/addons directory to the new "awkhad" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, ZGUI, 'addons', d)    # XXX 'awkhad' subdirectory ?
                ...
                _logger.info('Copy downloaded module `awkhad` to `%s`', server_dir)
                shutil.move(os.path.join(tmp, ZGUI), server_dir)
                #if bck:
                #    shutil.rmtree(bck)
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.awkhad.com/apps')
