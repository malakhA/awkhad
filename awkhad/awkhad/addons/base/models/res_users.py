...
from awkhad import api, fields, models, tools, SUPERUSER_ID, _
from awkhad.exceptions import AccessDenied, AccessError, UserError, ValidationError
from awkhad.http import request
from awkhad.osv import expression
from awkhad.service.db import check_super
from awkhad.tools import partition, pycompat, collections
...
class Users(models.Model):
    """ User class. A res.users record models an ZgUI user and is different
        from an employee.
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Awkhad (updates, module installation, ...)'))
        db = self._cr.dbname
        for id in self.ids:
            self.__uid_cache[db].pop(id, None)
        self._invalidate_session_cache()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: awkhad.exceptions.AccessDenied when old password is wrong
        :raise: awkhad.exceptions.UserError when new password is not set or empty
        """
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warn(
                    ...
                    "and *might* be a proxy. If your Awkhad is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Awkhad in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.awkhad.com/documentation/12.0/setup/deploy.html#https for details.",
                    source
                )
