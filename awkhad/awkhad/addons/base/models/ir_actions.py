...
from awkhad.exceptions import MissingError, UserError, ValidationError, AccessError
from awkhad.osv import expression
from awkhad.tools.safe_eval import safe_eval, test_python_expr
from awkhad.tools import pycompat, wrap_module
...
class IrActionsServer(models.Model):
    ...
    Since Awkhad 8.0 a button 'Create Menu Action' button is available on the
...
#  - env: Awkhad Environment on which the action is triggered
#  - model: Awkhad Model of the record on which the action is triggered; is a void recordset
...
# To return an action, assign: action = {...}\n\n\n\n"""
    ...
    def _get_eval_context(self, action=None):
        ...
        eval_context.update({
            ...
            'model': model,
            # Exceptions
            'Warning': awkhad.exceptions.Warning,
            # record
            'record': record,
            'records': records,
            # helpers
