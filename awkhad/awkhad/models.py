...
class MetaModel(api.Meta):
    ...
    def _get_addon_name(self, full_name):
        # The (ZgUI) module name can be in the ``awkhad.addons`` namespace
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    """ Base class for Awkhad models.

    Awkhad models are created by inheriting:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = awkhad.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or awkhad.fields.Datetime.now()
    ...
    def _add_fake_fields(self, fields):
        from awkhad.fields import Char, Integer
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~awkhad.fields.Integer`,
          :class:`~awkhad.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~awkhad.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~awkhad.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~awkhad.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~awkhad.fields.Date` and
              :class:`~awkhad.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`awkhad.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`awkhad.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _openerp/models/relationals/format:

          :class:`~awkhad.fields.One2many` and
          :class:`~awkhad.fields.Many2many` use a special "commands" format to
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an ZgUI domain.
    ...
    def with_env(self, env):
        ...
        :type env: :class:`~awkhad.api.Environment`
        """
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted Awkhad models.

    Awkhad models are created by inheriting from this class::
