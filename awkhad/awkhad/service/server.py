...
import awkhad
from awkhad.modules import get_modules
from awkhad.modules.module import run_unit_tests, get_test_modules
from awkhad.modules.registry import Registry
from awkhad.release import nt_service_name
from awkhad.tools import config
from awkhad.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'awkhad.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The AWKHAD_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("AWKHAD_MAX_HTTP_THREADS")
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(awkhad, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in awkhad.modules.module.ad_paths:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="awkhad.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from awkhad.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = awkhad.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(awkhad.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="awkhad.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="awkhad.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(awkhad, 'phoenix', None):
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('awkhad: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(awkhad.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = awkhad.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('awkhad.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            from awkhad.addons import base
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(awkhad.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            awkhad.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the addons found in the `zgui-web` project.
Maybe you forgot to add those addons in your addons_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute zgui-server process with (nearly) the same arguments"""
    if awkhad.tools.osutil.is_running_as_nt_service():
        ...
        for mod in [m for m in get_modules() if '/%s/' % m in test_file]:
            for mod_mod in get_test_modules(mod):
                ...
                if test_path == mod_path:
                    ...
                    stream = awkhad.modules.module.TestStream()
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                t0 = time.time()
                t0_sql = awkhad.sql_db.sql_counter
                ...
                _logger.info("All post-tested in %.2fs, %s queries",
                             time.time() - t0, awkhad.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the awkhad http server and cron processor.
    """
    global server

    load_server_wide_modules()
    awkhad.service.wsgi_server._patch_xmlrpc_marshaller()

    if awkhad.evented:
        server = GeventServer(awkhad.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(awkhad.service.wsgi_server.application)
        ...
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Awkhad is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(awkhad.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not awkhad.evented:
        if inotify:
    ...
    if getattr(awkhad, 'phoenix', False):
        _reexec()
