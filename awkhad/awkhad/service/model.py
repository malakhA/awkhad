...
import awkhad
from awkhad.exceptions import UserError, ValidationError, QWebException
from awkhad.models import check_method_name
from awkhad.tools.translate import translate
...
def dispatch(method, params):
    ...
    registry = awkhad.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            try:
                cr = awkhad.sql_db.db_connect(dbname).cursor()
        ...
        while True:
            try:
                if awkhad.registry(dbname)._init and not awkhad.tools.config['test_enable']:
                    raise awkhad.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    awkhad.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = awkhad.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    return awkhad.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with awkhad.registry(db).cursor() as cr:
