...
# User code must use the exceptions defined in ``awkhad.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, awkhad.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, awkhad.tools.ustr(e.name))
    ...
    elif isinstance(e, awkhad.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = awkhad.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, awkhad.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(awkhad.tools.ustr(e), formatted_info)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(awkhad.tools.exception_to_unicode(e), formatted_info)
...
def _patch_xmlrpc_marshaller():
    ...
    # Historically, awkhad is sending binary as base64 string.
    ...
    def dump_bytes(marshaller, value, write):
        marshaller.dump_unicode(awkhad.tools.ustr(value), write)
...
def application_unproxied(environ, start_response):
    ...
    # web.session.ZgUISession.send() and at RPC dispatch in
    # awkhad.service.web_services.objects_proxy.dispatch().
    ...
    with awkhad.api.Environment.manage():
        result = awkhad.http.root(environ, start_response)
