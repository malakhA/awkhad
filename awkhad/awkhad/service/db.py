...
from awkhad.exceptions import AccessDenied
import awkhad.release
import awkhad.sql_db
import awkhad.tools
from awkhad.sql_db import db_connect
from awkhad.release import version_info
from awkhad.tools import pycompat
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not awkhad.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and awkhad.tools.config.verify_admin_password(passwd):
        return True
    raise awkhad.exceptions.AccessDenied()

# This should be moved to awkhad.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = awkhad.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            awkhad.modules.db.initialize(cr)
            awkhad.tools.config['load_language'] = lang
            cr.commit()

        registry = awkhad.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = awkhad.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = awkhad.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = awkhad.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = awkhad.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    awkhad.sql_db.close_db(db_original_name)
    db = awkhad.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = awkhad.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = awkhad.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = awkhad.tools.config.filestore(db_original_name)
    to_fs = awkhad.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    awkhad.modules.registry.Registry.delete(db_name)
    awkhad.sql_db.close_db(db_name)
    ...
    fs = awkhad.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'awkhad_dump': '1',
        'db_name': cr.dbname,
        'version': awkhad.release.version,
        'version_info': awkhad.release.version_info,
        'major_version': awkhad.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with awkhad.tools.osutil.tempdir() as dump_dir:
            filestore = awkhad.tools.config.filestore(db_name)
            ...
            awkhad.tools.exec_pg_command(*cmd)
            if stream:
                awkhad.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                awkhad.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = awkhad.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with awkhad.tools.osutil.tempdir() as dump_dir:
        ...
        if awkhad.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = awkhad.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = awkhad.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
            ...
            if awkhad.tools.config['unaccent']:
                try:
                    with cr.savepoint():
...
def exp_rename(old_name, new_name):
    awkhad.modules.registry.Registry.delete(old_name)
    awkhad.sql_db.close_db(old_name)
    ...
    old_fs = awkhad.tools.config.filestore(old_name)
    new_fs = awkhad.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    awkhad.tools.config.set_admin_password(new_password)
    awkhad.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        awkhad.tools.config['update']['base'] = True
        awkhad.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@awkhad.tools.mute_logger('awkhad.sql_db')
...
def list_dbs(force=False):
    if not awkhad.tools.config['list_db'] and not force:
        raise awkhad.exceptions.AccessDenied()

    if not awkhad.tools.config['dbfilter'] and awkhad.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Awkhad will not
        ...
        res = sorted(db.strip() for db in awkhad.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [awkhad.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of Awkhad
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if awkhad.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        awkhad.sql_db.close_db(database_name)
...
def exp_list_lang():
    return awkhad.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(awkhad.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
