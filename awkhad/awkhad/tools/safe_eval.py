...
ZgUI domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    an ZgUI domain expression from an untrusted source.
    ...
    except awkhad.exceptions.except_orm:
        raise
    except awkhad.exceptions.Warning:
        raise
    except awkhad.exceptions.RedirectWarning:
        raise
    except awkhad.exceptions.AccessDenied:
        raise
    except awkhad.exceptions.AccessError:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except awkhad.http.AuthenticationError:
        raise
