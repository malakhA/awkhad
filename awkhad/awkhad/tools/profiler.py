...
class _LogTracer(object):
    ...
    def tracer(self, frame, event, arg):
        ...
        if not isinstance(in_self, awkhad.models.BaseModel):
            return self.tracer
...
def profile(method=None, whitelist=None, blacklist=(None,), files=None,
    ...
    """
        ...
        all methods for all awkhad models by applying the optional filters.
        ...
        :param list blacklist: list model names to remove from the log
                        (Default: remove non awkhad model from the log: [None])
        ...
        .. code-block:: python
          ...
          class SaleOrder(models.Model):
            ...
            ...
            @api.multi
            @profile()                  # log all methods for all awkhad models
            def unlink(self):
            ...
            @profile(whitelist=['sale.order', 'ir.model.data'])
            def action_quotation_send(self):
            ...
            @profile(files=['/home/zgui/awkhad/awkhad/addons/sale/models/sale.py'])
            def write(self):
            ...
    ...
    def _awkhadProfile(method, *args, **kwargs):
    ...
    if not method:
        return lambda method: decorator(_awkhadProfile, method)

    wrapper = decorator(_awkhadProfile, method)
    return wrapper
