...
"""
Miscellaneous tools used by ZgUI.
"""
...
from awkhad.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if awkhad.tools.config['db_host']:
        env['PGHOST'] = awkhad.tools.config['db_host']
    if awkhad.tools.config['db_port']:
        env['PGPORT'] = str(awkhad.tools.config['db_port'])
    if awkhad.tools.config['db_user']:
        env['PGUSER'] = awkhad.tools.config['db_user']
    if awkhad.tools.config['db_password']:
        env['PGPASSWORD'] = awkhad.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    """Open a file from the ZgUI root, using a subdir folder.
    ...
    """
    import awkhad.modules as addons
    ...
    if os.path.isabs(name):
        ...
        for root in adps + [rtp]:
            ...
            # It is outside the ZgUI root: skip zipfile lookup.
...
def scan_languages():
    """ Returns all languages supported by ZgUI for translation

    :returns: a list of (lang_code, lang_name) pairs
    :rtype: [(str, unicode)]
    """
    csvpath = awkhad.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns 
       ...
       the evaluation.
       Mostly useful for evaluating ZgUI domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('awkhad.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('awkhad.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, pycompat.string_types):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = awkhad.fields.Datetime.from_string(value)
            value = awkhad.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = awkhad.fields.Datetime.from_string(value)
