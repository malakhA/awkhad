...
from awkhad.sql_db import TestCursor
from awkhad.tools import (assertion_report, config, existing_tables,
                        lazy_classproperty, lazy_property, table_exists,
                        topological_sort, OrderedSet)
from awkhad.tools.lru import LRU
...
class Registry(Mapping):
    ...
    def __new__(cls, db_name):
        ...
        with cls._lock:
            try:
                ...
                # awkhad.service.wsgi_server.application
        ...
        with cls._lock:
            with awkhad.api.Environment.manage():
                ...
                try:
                    ...
                    try:
                        awkhad.modules.load_modules(registry._db, force_demo, status, update_module)
                    except Exception:
                        awkhad.modules.reset_modules_state(db_name)
                        raise
                except Exception:
    ...
    def init(self, db_name):
        ...
        self._db = awkhad.sql_db.db_connect(db_name)
        ...
        with closing(self.cursor()) as cr:
            has_unaccent = awkhad.modules.db.has_unaccent(cr)
            if awkhad.tools.config['unaccent'] and not has_unaccent:
                _logger.warning("The option --unaccent was given but no unaccent() function was found in database.")
            self.has_unaccent = awkhad.tools.config['unaccent'] and has_unaccent
    ...
    def init_models(self, cr, model_names, context):
        ...
        env = awkhad.api.Environment(cr, SUPERUSER_ID, context)
