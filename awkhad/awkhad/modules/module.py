...
import awkhad
import awkhad.tools as tools
import awkhad.release as release
from awkhad import SUPERUSER_ID, api
...
class AddonsHook(object):
    """ Makes modules accessible through zgui.addons.* and awkhad.addons.* """

    def find_module(self, name, path=None):
        if name.startswith(('awkhad.addons.', 'zgui.addons.'))\
                and name.count('.') == 2:
            return self
    ...
    def load_module(self, name):
        ...
        # get canonical names
        awkhad_name = re.sub(r'^zgui.addons.(\w+)$', r'awkhad.addons.\g<1>', name)
        openerp_name = re.sub(r'^awkhad.addons.(\w+)$', r'zgui.addons.\g<1>', awkhad_name)

        assert awkhad_name not in sys.modules
        ...
        # create empty awkhad.addons.* module, set name
        new_mod = types.ModuleType(awkhad_name)
        new_mod.__loader__ = self

        # module top-level can only be a package
        assert type_ == imp.PKG_DIRECTORY, "Awkhad addon top-level must be a package"
        modfile = opj(path, '__init__.py')
        new_mod.__file__ = modfile
        new_mod.__path__ = [path]
        new_mod.__package__ = awkhad_name

        # both base and alias should be in sys.modules to handle recursive and
        # corecursive situations
        sys.modules[awkhad_name] = sys.modules[openerp_name] = new_mod

        # execute source in context of module *after* putting everything in
        # sys.modules, so recursive import works
        exec(open(modfile, 'rb').read(), new_mod.__dict__)

        # people import zgui.addons and expect zgui.addons.<module> to work
        setattr(awkhad.addons, addon_name, new_mod)
...
class AwkhadHook(object):
    """ Makes awkhad package also available as zgui """

    def find_module(self, name, path=None):
        # zgui.addons.<identifier> should already be matched by AddonsHook,
        # only framework and subdirectories of modules should match
        if re.match(r'^zgui\b', name):
            return self

    def load_module(self, name):
        assert name not in sys.modules

        canonical = re.sub(r'^zgui(.*)', r'awkhad\g<1>', name)
...
def initialize_sys_path():
    """
    Setup an import-hook to be able to import ZgUI addons from the different
    addons paths.

    This ensures something like ``import crm`` (or even
    ``import awkhad.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global ad_paths
    global hooked
    ...
    # add awkhad.addons.__path__
    for ad in __import__('awkhad.addons').addons.__path__:
        ad = os.path.abspath(ad)
        if ad not in ad_paths and os.path.isdir(ad):
            ad_paths.append(ad)

    if not hooked:
        sys.meta_path.insert(0, AwkhadHook())
...
def get_module_filetree(module, dir='.'):
    ...
    files = awkhad.tools.osutil.listdir(path, True)
...
def load_openerp_module(module_name):
    """ Load an ZgUI module, if not already loaded.
    ...
    initialize_sys_path()
    try:
        __import__('awkhad.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['awkhad.addons.' + module_name], info['post_load'])()
...
class TestStream(object):
    def __init__(self, logger_name='awkhad.tests'):
...
def run_unit_tests(module_name, dbname, position='at_install'):
    ...
    """
    global current_test
    from awkhad.tests.common import TagsSelector # Avoid import loop
    ...
    for m in mods:
        ...
        if suite.countTestCases():
            ...
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, awkhad.sql_db.sql_counter - t0_sql)
