...
import awkhad
import awkhad.modules.db
import awkhad.modules.graph
import awkhad.modules.migration
import awkhad.modules.registry
import awkhad.tools as tools

from awkhad import api, SUPERUSER_ID
from awkhad.modules.module import adapt_version, initialize_sys_path, load_openerp_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('awkhad.tests')
...
def force_demo(cr):
    ...
    """
    graph = awkhad.modules.graph.Graph()
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    def load_test(idref, mode):
        ...
        try:
            ...
            awkhad.registry(cr.dbname).clear_caches()
    ...
    migrations = awkhad.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph, 1):
        ...
        if new_install:
            py_module = sys.modules['awkhad.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = awkhad.tools.config["overwrite_existing_translations"]
            ...
            if demo_loaded:
                ...
                if tools.config.options['test_enable']:
                    ...
                    report.record_result(awkhad.modules.module.run_unit_tests(module_name, cr.dbname))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, awkhad.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    with db.cursor() as cr:
        if not awkhad.modules.db.is_initialized(cr):
        ...
        # awkhad.modules.registry.Registry.new().
        registry = awkhad.registry(cr.dbname)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['awkhad.addons.%s' % (pkg.name,)]
                ...
                registry = awkhad.modules.registry.Registry.new(
