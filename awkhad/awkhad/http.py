...
# *broken*
NO_POSTMORTEM = (awkhad.osv.orm.except_orm,
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                awkhad.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = awkhad.service.common.dispatch
        elif service_name == 'db':
            dispatch = awkhad.service.db.dispatch
        elif service_name == 'object':
            dispatch = awkhad.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                awkhad.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                awkhad.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except awkhad.exceptions.DeferredException as e:
        _logger.exception(awkhad.tools.exception_to_unicode(e))
        awkhad.tools.debugger.post_mortem(awkhad.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(awkhad.tools.exception_to_unicode(e))
        awkhad.tools.debugger.post_mortem(awkhad.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all Awkhad Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in awkhad.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~awkhad.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~awkhad.api.Environment` bound to current request. """
        if self._env is None:
            self._env = awkhad.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    @lazy_property
    def session(self):
        """ :class:`ZgUISession` holding the HTTP session data for the
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            awkhad.tools.debugger.post_mortem(
                awkhad.tools.config, sys.exc_info())
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return awkhad.registry(self.db)
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        Awkhad implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <awkhad.http.WebRequest.csrf_token`, the
          :data:`~awkhad.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          Awkhad) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, (awkhad.exceptions.Warning, SessionExpiredException,
                                          awkhad.exceptions.except_orm, werkzeug.exceptions.NotFound)):
                _logger.exception("Exception during JSON request handling.")
            error = {
                    'code': 200,
                    'message': "Awkhad Server Error",
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "Awkhad Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "Awkhad Session Expired"
...
def serialize_exception(e):
    ...
    if isinstance(e, awkhad.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, awkhad.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, awkhad.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, awkhad.exceptions.AccessError):
        tmp["exception_type"] = "access_error"
    elif isinstance(e, awkhad.exceptions.MissingError):
        tmp["exception_type"] = "missing_error"
    elif isinstance(e, awkhad.exceptions.AccessDenied):
        tmp["exception_type"] = "access_denied"
    elif isinstance(e, awkhad.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, awkhad.exceptions.except_orm):
...
Awkhad URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.awkhad.com/documentation/12.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through Awkhad via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["awkhad", "addons"]:
            module = ""
        else:
...
def routing_map(modules, nodb_only, converters=None):
    ...
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('awkhad.addons.') and c.__module__.split(".")[2] in modules
...
class ZgUISession(werkzeug.contrib.sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(ZgUISession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def authenticate(self, db, login=None, password=None, uid=None):
        ...
        if uid is None:
            ...
            uid = awkhad.registry(db)['res.users'].authenticate(db, login, password, env)
        else:
    ...
    def check_security(self):
        ...
        env = awkhad.api.Environment(request.cr, self.uid, self.context)
    ...
    def _fix_lang(self, context):
        """ ZgUI provides languages which may not make sense and/or may not
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or awkhad.SUPERUSER_ID)
...
class Root(object):
    """Root WSGI application for the ZgUI Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def session_store(self):
        # Setup http sessions
        path = awkhad.tools.config.session_dir
        _logger.debug('HTTP sessions stored in: %s', path)
        return werkzeug.contrib.sessions.FilesystemSessionStore(
            path, session_class=ZgUISession, renew_missing=True)

    @lazy_property
    def nodb_routing_map(self):
        _logger.info("Generating nondb routing")
        return routing_map([''] + awkhad.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in awkhad.modules.module.ad_paths:
    ...
    def setup_session(self, httprequest):
        ...
        if not sid:
            sid =  httprequest.headers.get("X-Zgui-Session-Id")
        if not sid:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        awkhad.registry(db).check_signaling()
                        with awkhad.tools.mute_logger('awkhad.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = awkhad.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if awkhad.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = awkhad.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif awkhad.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Awkhad will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in awkhad.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = awkhad.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = awkhad.tools.ustr(filename)
