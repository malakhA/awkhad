...
class Shell(Command):
    """Start awkhad in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        awkhad.cli.server.report_configuration()
        awkhad.service.server.start(preload=[], stop=True)
        ...
        with awkhad.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = awkhad.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = awkhad.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
