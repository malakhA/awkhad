...
"""
ZgUI - Server
ZgUI is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - ZgUI SA
"""
...
__author__ = awkhad.release.author
__version__ = awkhad.release.version

# Also use the `awkhad` logger for the main script.
_logger = logging.getLogger('awkhad')
...
def report_configuration():
    ...
    """
    config = awkhad.tools.config
    _logger.info("Awkhad version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', awkhad.modules.module.ad_paths)
...
def setup_pid_file():
    ...
    """
    config = awkhad.tools.config
    if not awkhad.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = awkhad.modules.registry.Registry.new(dbname)
        with awkhad.api.Environment.manage():
            with registry.cursor() as cr:
                awkhad.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with awkhad.api.Environment.manage():
        with registry.cursor() as cr:
            awkhad.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    awkhad.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                awkhad.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except awkhad.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        awkhad.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = awkhad.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the awkhad server (default command)"""
    def run(self, args):
        main(args)
