...
#----------------------------------------------------------
# awkhad must be a namespace package for awkhad.addons to become one too
...
import sys
assert sys.version_info > (3, 5), "Python 2 detected, Awkhad requires Python >= 3.5 to run."
...
if len(sys.argv) > 1 and sys.argv[1] == 'gevent':
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in awkhad/debian/copyright)
        while 1:
            state = conn.poll()
...
# libc UTC hack
#----------------------------------------------------------
# Make sure the ZgUI server runs in UTC.
import os
