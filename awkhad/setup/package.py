...
exec(open(os.path.join(ROOTDIR, 'awkhad', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/awkhad && \
    groupadd -g %(group_id)s awkhad && \
    useradd -u %(user_id)s -g awkhad awkhad -d /var/lib/awkhad && \
    mkdir /data && \
    chown awkhad:awkhad /var/lib/awkhad /data
USER awkhad
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/awkhad_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False):
    ...
    run_cmd(cmd + ['%s/' % args.awkhad_dir, args.build_dir])
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'awkhad/addons'))
...
#  Docker stuffs
class AwkhadTestTimeoutError(Exception):
    pass


class AwkhadTestError(Exception):
    pass
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'awkhad-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
    ...
    def run(self, cmd, build_dir, container_name, user='awkhad', exposed_port=None, detach=False, timeout=None):
    ...
    def test_awkhad(self):
        logging.info('Starting to test Awkhad install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'awkhad.pid')):
                _rpc_count_modules(port=self.exposed_port)
                self.stop()
                return
        if self.is_running():
            self.stop()
            raise AwkhadTestTimeoutError('Awkhad pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise AwkhadTestError('Error while installing/starting Awkhad after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'awkhad-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/awkhad-*.tar.gz' % self.args.build_dir)[0], '%s/awkhad_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/awkhad-*.zip' % self.args.build_dir)[0], '%s/awkhad_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'pip3 install /data/src/awkhad_%s.%s.tar.gz' % (VERSION, TSTAMP),
            'su postgres -s /bin/bash -c "createuser -s awkhad"',
            'su postgres -s /bin/bash -c "createdb mycompany"',
            'su awkhad -s /bin/bash -c "awkhad -d mycompany -i base --stop-after-init"',
            'su awkhad -s /bin/bash -c "awkhad -d mycompany --pidfile=/data/src/awkhad.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'awkhad-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_awkhad()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/awkhad (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../awkhad_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'awkhad-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            '/usr/bin/dpkg -i /data/src/awkhad_%s.%s_all.deb ; /usr/bin/apt-get install -f -y' % (VERSION, TSTAMP),
            'su awkhad -s /bin/bash -c "awkhad -d mycompany -i base --stop-after-init"',
            'su awkhad -s /bin/bash -c "awkhad -d mycompany --pidfile=/data/src/awkhad.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'awkhad-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_awkhad()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py --quiet bdist_rpm', self.args.build_dir, 'awkhad-rpm-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/awkhad-*.noarch.rpm' % self.args.build_dir)[0], '%s/awkhad_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'dnf install -d 0 -e 0 /data/src/awkhad_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su awkhad -s /bin/bash -c "awkhad -c /etc/awkhad/awkhad.conf -d mycompany -i base --stop-after-init"',
            'su awkhad -s /bin/bash -c "awkhad -c /etc/awkhad/awkhad.conf -d mycompany --pidfile=/data/src/awkhad.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'awkhad-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_awkhad()
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        remote_build_dir = '/cygdrive/c/awkhadbuild/server/'

        self.ssh("mkdir -p build")
        logging.info("Syncing Awkhad files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        setup_path = glob("%s/zgui-server-setup-*.exe" % self.args.build_dir)[0]
        setupfile = setup_path.split('/')[-1]
        setupversion = setupfile.split('zgui-server-setup-')[1].split('.exe')[0]

        self.rsync(['"%s"' % setup_path, '%s@127.0.0.1:' % self.login])
        self.ssh("TEMP=/tmp ./%s /S" % setupfile)
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"Awkhad %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"Awkhad {sv}"/python/python.exe \'c:\\Program Files\\Awkhad {sv}\\server\\awkhad-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
...
def build_exe(args):
    ...
    shutil.copy(glob('%s/zgui*.exe' % args.build_dir)[0], '%s/awkhad_%s.%s.exe' % (args.build_dir, VERSION, TSTAMP))
...
def parse_args():
    ...
    # Windows VM
    ap.add_argument("--vm-winxp-image", default='/home/awkhad/vm/win1036/win10_winpy36.qcow2', help="%(default)s")
    ap.add_argument("--vm-winxp-ssh-key", default='/home/awkhad/vm/win1036/id_rsa', help="%(default)s")
    ...
    parsed_args.awkhad_dir = ROOTDIR
    return parsed_args
