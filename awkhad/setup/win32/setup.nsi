...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Awkhad Server ${VERSION}"
...
OutFile "zgui-allinone-setup-${VERSION}.exe"
...
InstallDir "$PROGRAMFILES\Awkhad ${VERSION}"
...
#VIAddVersionKey "LegalTrademark" "ZgUI is a trademark of ${PUBLISHER}"
...
!define MUI_ICON "${PIXMAPS_PATH}\zgui-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\zgui-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\zgui-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\zgui-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LACATION "https://www.awkhad.com/page/contactus"
...
; English
LangString DESC_ZgUI_Server ${LANG_ENGLISH} "Install the Awkhad Server with all the Awkhad standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Awkhad."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Awkhad for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the Awkhad Server"
...
LangString TITLE_ZgUI_Server ${LANG_ENGLISH} "Awkhad Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start Awkhad"

; French
LangString DESC_ZgUI_Server ${LANG_FRENCH} "Installation du Serveur Awkhad avec tous les modules Awkhad standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par Awkhad."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Awkhad pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur Awkhad"
...
LangString TITLE_ZgUI_Server ${LANG_FRENCH} "Serveur Awkhad"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer Awkhad"

InstType $(Profile_AllInOne)
InstType $(Profile_Server)

Section $(TITLE_ZgUI_Server) SectionZgUI_Server
    SectionIn 1 2
...
# If there is a previous install of the ZgUI Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\awkhad.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\awkhad.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\awkhad.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\awkhad.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\awkhad.conf" "options" "addons_path" "$INSTDIR\server\awkhad\addons"
    WriteIniStr "$INSTDIR\server\awkhad.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\awkhad.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\awkhad-bin" --stop-after-init --logfile "$INSTDIR\server\awkhad.log" -s'
    ${If} ${RunningX64}
      nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe" "\"$INSTDIR\server\awkhad-bin\""'
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\${POSTGRESQL_EXE_FILENAME}" \
        ...
        --servicename "PostgreSQL_For_Awkhad" \
...
;    WriteRegDWORD HKLM     "${UNINSTALL_REGISTRY_KEY}" "VersionMinor" "${REVISION_VERSION}"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@awkhad.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.awkhad.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@awkhad.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionZgUI_Server} $(DESC_ZgUI_Server)
...
Function ShowPostgreSQL
    SectionGetFlags ${SectionZgUI_Server} $0
